...
class Simplificator {
    ...
    static private class SimplificatorAngleComparer extends AttributeStreamOfInt32.IntComparator {
    ...
    private boolean _processCrossOvers(Point2D ptCenter) {
        ...
        while (bContinue) {
            ...
            for (int i = 0, n = m_bunchEdgeIndices.size(); i < n && index1 != -1 && index2 != -1
                    && index1 != index2; i++) {
        ...
        if (!bFound) {// resolve all cross overs
            ...
            for (int i = 0, n = m_bunchEdgeIndices.size(); i < n && index1 != -1 && index2 != -1
                ...
                if (_detectAndResolveCrossOver(bDirection1, bDirection2, vertexB1, vertexA1, vertexC1, vertexB2,
                        vertexA2, vertexC2)) {
                    bFound = true;
                }
    ...
    static private class SimplificatorVertexComparer extends AttributeStreamOfInt32.IntComparator {
        private Simplificator m_parent;
        private Point2D pt1 = new Point2D();
        private Point2D pt2 = new Point2D();
    ...
    private boolean _simplify() {
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            m_shape.setUserIndex(vertex, m_userIndexSortedIndexToVertex, vertexlistIndex);// remember the sorted list
                                                                                          // element on the
                                                                                          // vertex.
        ...
        do// while (bNeedRepeat);
        {
            ...
            for (int vlistindex = m_sortedVertices.getFirst(m_sortedVerticesListIndex); vlistindex != IndexMultiDCList
                    .nullNode();) {
        ...
        bChanged |= RingOrientationFixer.execute(m_shape, m_geometry, m_sortedVertices, m_fixSelfTangency);
    ...
    private boolean _detectAndResolveCrossOver(boolean bDirection1, boolean bDirection2, int vertexB1, int vertexA1,
            int vertexC1, int vertexB2, int vertexA2, int vertexC2) {
    ...
    private void _resolveOverlap(boolean bDirection1, boolean bDirection2, int vertexA1, int vertexB1, int vertexA2,
            int vertexB2) {
        _resolveOverlapOddEven(bDirection1, bDirection2, vertexA1, vertexB1, vertexA2, vertexB2);
    }

    private void _resolveOverlapOddEven(boolean bDirection1, boolean bDirection2, int vertexA1, int vertexB1,
            int vertexA2, int vertexB2) {
        if (bDirection1 != bDirection2) {
            if (bDirection1) {
    ...
    private void _fixOrphanVertices() {
        int pathCount = 0;
        // clean any path info
        for (int node = m_sortedVertices.getFirst(m_sortedVertices.getFirstList()); node != -1; node = m_sortedVertices
                .getNext(node)) {
    ...
    public static boolean execute(EditShape shape, int geometry, int knownSimpleResult, boolean fixSelfTangency,
