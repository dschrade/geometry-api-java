...
final class InternalUtils {
    ...
    // corners)
    static int addPointsToArray(Point2D p0In, Point2D p1In, Point2D[] pointsArray, int idx, Envelope2D fullRange2D,
                                                  ...
                                                  // densifyDist)
    {
        ...
        if ((p0.x == p1.x || p0.y == p1.y && (p0.y == fullRange2D.ymin || p0.y == fullRange2D.ymax))
    ...
    static double calculateToleranceFromGeometry(SpatialReference sr, Envelope2D env2D, boolean bConservative) {
        ...
        double stolerance = sr != null ? sr.getTolerance(VertexDescription.Semantics.POSITION) : 0;
        if (bConservative) {
            gtolerance *= 4;
            stolerance *= 1.1;
        }
        return StrictMath.max(stolerance, gtolerance);
    }

    static double adjust_tolerance_for_TE_clustering(double tol) {
        return 2.0 * StrictMath.sqrt(2.0) * tol;
    }

    static double adjust_tolerance_for_TE_cracking(double tol) {
        return StrictMath.sqrt(2.0) * tol;
    }
    ...
    static double calculateZToleranceFromGeometry(SpatialReference sr, Geometry geometry, boolean bConservative) {
        Envelope1D env1D = geometry.queryInterval(VertexDescription.Semantics.Z, 0);
        double gtolerance = env1D._calculateToleranceFromEnvelope();
        double stolerance = sr != null ? sr.getTolerance(VertexDescription.Semantics.Z) : 0;
        if (bConservative) {
            gtolerance *= 4;
            stolerance *= 1.1;
        }
    ...
    double calculateZToleranceFromGeometry(SpatialReference sr, Geometry geometry) {
        ...
        return Math.max(sr != null ? sr.getTolerance(VertexDescription.Semantics.Z) : 0, tolerance);
    }
        ...
        do {
            for (int ipath = 0, npaths = multipathImpl.getPathCount(); ipath < npaths; ipath++) {
                ...
                if (hint_index == -1) {
                    ...
                    // This is usually happens because esri shape buffer contains geometry extent
                    // which is slightly different from the true extent.
                    // Recalculate extent
                    multipathImpl.calculateEnvelope2D(extent, false);
                    resized_extent = true;
                    quad_tree_impl.reset(extent, 8);
    ...
    static Envelope2DIntersectorImpl getEnvelope2DIntersector(MultiPathImpl multipathImplA,
            MultiPathImpl multipathImplB, double tolerance) {
    ...
    static Envelope2DIntersectorImpl getEnvelope2DIntersectorForParts(MultiPathImpl multipathImplA,
            MultiPathImpl multipathImplB, double tolerance, boolean bExteriorOnlyA, boolean bExteriorOnlyB) {
