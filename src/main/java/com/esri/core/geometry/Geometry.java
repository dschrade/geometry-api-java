...
public abstract class Geometry implements Serializable {
    ...
    static public enum Type {
        /**
         * Used to indicate that the geometry type is not known before executing a
         * method.
         */
     ...
     * <p>
     * This estimate doesn't include the size of the {@link VertexDescription}
     * object because instances of {@link VertexDescription} are shared among
     * geometry objects.
     * 
     ...
     * The Geometry will have a union of the current and the src descriptions.
     * 
     * @param src
     ...
     * 
     * @param semantics
     *            The VertexDescription.Semantics to check.
    ...
    /**
     * Drops an attribute from the Geometry. Dropping the attribute is equivalent to
     * setting the attribute to the default value for each vertex, However, it is
     * faster and the result Geometry has smaller memory footprint and smaller size
     * when persisted.
     * 
     * @param semantics
    ...
    public void dropAttribute(int semantics) {
        ...
        VertexDescription newvd = VertexDescriptionDesignerImpl.removeSemanticsFromVertexDescription(m_description,
                semantics);
        _assignVertexDescriptionImpl(newvd);
    }
    ...
    /**
     * Returns the conservative bbox of the Geometry in X, Y plane. This is a faster
     * method than QueryEnvelope2D. However, the bbox could be larger than the tight
     * box.
     * 
     * @param env
     *            The envelope to return the result in.
    ...
    /**
     * Returns tight conservative box of the Geometry in 3D. This is a faster method
     * than the QueryEnvelope3D. However, the box could be larger than the tight
     * box.
     * 
     * @param env
     *            The envelope to return the result in.
    ...
    /**
     * Returns the geometry to its original initialization state by releasing all
     ...
     * 
     * @param dst
     *            The geometry instance to copy to.
    ...
    /**
     * Calculates the area of the geometry. If the spatial reference is a Geographic
     * Coordinate System (WGS84) then the 2D area calculation is defined in angular
     * units.
     * 
     ...
     * Geographic Coordinate System (a system where coordinates are defined using
     * angular units such as longitude and latitude) then the 2D distance
    ...
    /**
     * Indicates if the integer value of the enumeration is a point type (dimension
     * 0).
     * 
     * @param type
     *            The integer value from geometry enumeration. You can use the
    ...
    /**
     * Indicates if the integer value of the enumeration is a multipath (ie, line or
     * area).
     * 
     * @param type
     *            The integer value from geometry enumeration. You can use the
    ...
    /**
     * Replaces NaNs in the attribute with the given value. If the geometry is not
     * empty, it adds the attribute if geometry does not have it yet, and replaces
     * the values. If the geometry is empty, it adds the attribute and does not set
     * any values.
     * 
     * @param semantics
     *            The semantics for which to replace the NaNs.
     * @param value
    ...
    /**
     * Describes the degree of acceleration of the geometry. Acceleration usually
     * builds a raster and a quadtree.
     */
    static public enum GeometryAccelerationDegree {
        ...
        enumMild,
        /**
         * medium acceleration, takes more memory and takes more time to accelerate, but
         * may work faster. (256x256x2 bit raster and a quad tree for segments)
         */
        enumMedium,
        /**
         * high acceleration, takes even more memory and may take longest time to
         * accelerate, but may work faster than the other two. (1024x1024x2 bit raster
         * and a quad tree for segments)
         */
        enumHot
    }
    ...
    /**
     * The output of this method can be only used for debugging. It is subject to
     ...
     * 
     * @param geom
     *            The geometry to get the vertex count for.
