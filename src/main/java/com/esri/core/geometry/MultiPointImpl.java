...
final class MultiPointImpl extends MultiVertexGeometryImpl {
    ...
    public void add(MultiVertexGeometryImpl src, int beginIndex, int endIndex) {
        ...
        for (int iattrib = 0, nattrib = src.getDescription().getAttributeCount(); iattrib < nattrib; iattrib++) {
            ...
            stream.insertRange(oldPointCount * ncomps, srcStream, beginIndex * ncomps, count * ncomps, true, 1,
                    oldPointCount * ncomps);
        }
    }
    ...
    public void insertPoint(int beforePointIndex, Point pt) {
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            ...
            if (pt.hasAttribute(semantics)) {
                m_vertexAttributes[iattr].insertAttributes(comp * beforePointIndex, pt, semantics,
                        comp * oldPointCount);
            } else {
