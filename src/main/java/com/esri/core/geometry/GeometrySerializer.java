...
final class GeometrySerializer implements Serializable {
    ...
    Object readResolve() throws ObjectStreamException {
        ...
        try {
            geometry = GeometryEngine.geometryFromEsriShape(geometryData.esriShape, geometryData.geometryType);
            if (Geometry.isMultiVertex(geometry.getType().value())) {
                ...
                if (!geometry.isEmpty() && Geometry.isMultiPath(geometry.getType().value())) {
                    ...
                    for (int i = 0, n = mpImpl.getPathCount(); i < n; i++) {
                        if (mpd.ogcFlags[i])
                            pathFlags.setBits(i, (byte) PathFlags.enumOGCStartPolygon);
                    }
                }
    ...
    public void setGeometryByValue(Geometry geometry) throws ObjectStreamException {
        try {
            ...
            geometryData.esriShape = GeometryEngine.geometryToEsriShape(geometry);
            ...
            if (Geometry.isMultiVertex(geometryData.geometryType.value())) {
                ...
                if (!geometry.isEmpty() && Geometry.isMultiPath(geometryData.geometryType.value())) {
                    MultiPathData mpd = (MultiPathData) geometryData;
                    MultiPathImpl mpImpl = (MultiPathImpl) geometry._getImpl();
