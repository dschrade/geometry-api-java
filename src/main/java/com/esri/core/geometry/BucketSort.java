...
final class BucketSort {
    ...
    /**
     * Executes sort on the Bucket_sort. The result is fed into the indices array in
     * the range between begin (inclusive) and end (exclusive). Uses user supplied
     * sorter to execute sort on each bucket. Users either supply the sorter and use
     * this method of Bucket_sort class, or use other methods to form the buckets
     * and take care of bucket sorting themselves.
     */
     ...
     * 
     * @param bucket_count
     *            - the number of buckets. Usually equal to the number of elements
     *            to sort.
     * @param min_value
     ...
     *         parameters. The method also can throw out of memory exception. In the
     *         later case, one should fall back to the regular sort.
     */
    private boolean reset(int bucket_count, double min_value, double max_value, int capacity) {
        ...
        int bc = StrictMath.min(MAXBUCKETS, bucket_count);
    ...
    /**
     * Adds new element to the bucket builder. The value must be between min_value
     * and max_value.
     * 
     * @param The
