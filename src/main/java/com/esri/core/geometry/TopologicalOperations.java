...
final class TopologicalOperations {
    ...
    private void collectPolygonPathsPreservingFrom_(int geometryFrom, int newGeometry, int visitedEdges,
            int visitedClusters, int geometry_dominant) {
        ...
        for (int path = shape.getFirstPath(geometryFrom); path != -1; path = shape.getNextPath(path)) {
            ...
            assert (m_topo_graph.getHalfEdgeTo(firstHalfEdge) == secondCluster
                    && m_topo_graph.getHalfEdgeOrigin(firstHalfEdge) == firstCluster);
            ...
            int dir = 1;
            // Walk the chain of half edges, preferably selecting vertices that belong to
            // the
            ...
            do {
                ...
                do {// move in a loop through coincident vertices (probably
                    // vertical segments).
                    v = dir == 1 ? shape.getNextVertex(vertex) : shape.getPrevVertex(vertex);// if we came to the
                                                                                             // polyline
                                                                                             // tail, the next may
                                                                                             // return
                                                                                             // -1.
                ...
                if (originCluster != cv) {
                    ...
                    do {// move in a loop through coincident vertices (probably
                        // vertical segments).
                        v = dir == 1 ? shape.getPrevVertex(vertex) : shape.getNextVertex(vertex);// if we came to the
                                                                                                 // polyline tail, the
                                                                                                 // next may return -1.
                        cv = v != -1 ? m_topo_graph.getClusterFromVertex(v) : -1;
                    } while (cv == cluster);
    ...
    private int topoOperationPolygonPolygon_(int geometry_a, int geometry_b, int geometry_dominant) {
        ...
        m_topo_graph.deleteUserIndexForHalfEdges(visitedEdges);
        Simplificator.execute(shape, newGeometry, MultiVertexGeometryImpl.GeometryXSimple.Weak, m_bOGCOutput, null);
        return newGeometry;
    }

    private void topoOperationPolygonPolygonHelper_(int geometry_a, int geometry_b, int newGeometryPolygon,
            int geometry_dominant, int visitedEdges, int visitedClusters) {
        collectPolygonPathsPreservingFrom_(geometry_a, newGeometryPolygon, visitedEdges, visitedClusters,
                geometry_dominant);
        if (geometry_b != -1)
            collectPolygonPathsPreservingFrom_(geometry_b, newGeometryPolygon, visitedEdges, visitedClusters,
                    geometry_dominant);
                        ...
                        do {
                            ...
                            if (viter != -1) {
                                ...
                                int viter1 = m_topo_graph
                                        .getHalfEdgeVertexIterator(m_topo_graph.getHalfEdgeTwin(faceHalfEdge));
                                assert (viter1 != -1);
                            ...
                            shape.addVertex(newPath, vertex_dominant);
                            assert (isGoodParentage(m_topo_graph.getHalfEdgeFaceParentage(faceHalfEdge)));
    ...
    int[] topoOperationPolygonPolygonEx_(int geometry_a, int geometry_b, int geometry_dominant) {
        ...
        topoOperationPolygonPolygonHelper_(geometry_a, geometry_b, newGeometryPolygon, geometry_dominant, visitedEdges,
                visitedClusters);
                ...
                if (visited == 2) {
                    ...
                    if (isGoodParentage(parentage)) {// Extract face.
                        ...
                        do {
                            ...
                            m_topo_graph.setHalfEdgeUserIndex(m_topo_graph.getHalfEdgeTwin(polyHalfEdge), visitedEdges,
                                    1);//
                            ...
                            visited2 = m_topo_graph.getHalfEdgeUserIndex(m_topo_graph.getHalfEdgeTwin(polyHalfEdge),
                                    visitedEdges);
                            ...
                            if (!isGoodParentage(parentage)) {
                                m_topo_graph.setHalfEdgeUserIndex(polyHalfEdge, visitedEdges, 1);
                                m_topo_graph.setHalfEdgeUserIndex(m_topo_graph.getHalfEdgeTwin(polyHalfEdge),
                                        visitedEdges, 1);
                                break;
                            }
        ...
        for (int cluster = m_topo_graph.getFirstCluster(); cluster != -1; cluster = m_topo_graph
                .getNextCluster(cluster)) {
            int visited = m_topo_graph.getClusterUserIndex(cluster, visitedClusters);
            if (visited == 1)
                continue;
        ...
        m_topo_graph.deleteUserIndexForHalfEdges(visitedEdges);
        Simplificator.execute(shape, newGeometryPolygon, MultiVertexGeometryImpl.GeometryXSimple.Weak, m_bOGCOutput,
                null);
    ...
    private double prevailingDirection_(EditShape shape, int half_edge) {
        ...
        for (int iterator = m_topo_graph.getClusterVertexIterator(cluster); iterator != -1; iterator = m_topo_graph
            ...
            } else if (prevVert != -1 && m_topo_graph.getClusterFromVertex(prevVert) == clusterTo) {
                signTotal--;
                if (isGoodParentage(geomID)) {
    ...
    int getCombinedHalfEdgeParentage_(int e) {
        return m_topo_graph.getHalfEdgeParentage(e) | m_topo_graph.getHalfEdgeFaceParentage(e)
                | m_topo_graph.getHalfEdgeFaceParentage(m_topo_graph.getHalfEdgeTwin(e));
    }
    ...
    private void restorePolylineParts_(int first_edge, int newGeometry, int visitedEdges, int visitedClusters,
        ...
        if (toEdge == -1) {
            ...
            if (m_from_edge_for_polylines != -1) {
                ...
                toEdge = m_topo_graph.getHalfEdgePrev(m_from_edge_for_polylines);// try
                                                                                 // simply
                                                                                 // getting
                                                                                 // prev
        ...
        while (true) {
            ...
            int halfEdgeNext = m_topo_graph.getHalfEdgeNext(half_edge);
            if (m_topo_graph.getHalfEdgePrev(m_topo_graph.getHalfEdgeTwin(half_edge)) != m_topo_graph
        ...
        for (int cluster = m_topo_graph.getFirstCluster(); cluster != -1; cluster = m_topo_graph
            ...
            do {
                ...
                if (visited != 1) {
                    int parentage = getCombinedHalfEdgeParentage_(clusterHalfEdge);
                    if (isGoodParentage(parentage)) {
                        restorePolylineParts_(clusterHalfEdge, newPolyline, visitedEdges, visitedClusters,
                                geometry_dominant);
                    } else {
                        //
                    }
    ...
    private int topoOperationMultiPoint_() {
        ...
        for (int cluster = m_topo_graph.getFirstCluster(); cluster != -1; cluster = m_topo_graph
            ...
            if (isGoodParentage(parentage)) {
                int vert = -1;
                for (int iterator = m_topo_graph.getClusterVertexIterator(
                        cluster); iterator != -1; iterator = m_topo_graph.incrementVertexIterator(iterator)) {
                    int vertex = m_topo_graph.getVertexFromVertexIterator(iterator);
                    if (vert == -1)
                        vert = vertex;
                    int geometry = shape.getGeometryFromPath(shape.getPathFromVertex(vertex));
    ...
    static MultiPoint processMultiPointIntersectOrDiff_(MultiPoint multi_point, Geometry intersector, double tolerance,
            boolean bClipIn) {
        ...
        for (int ipoints = 0; ipoints < npoints;) {
            ...
            if (bArea)
                PolygonUtils.testPointsInArea2D(intersector, input_points, (int) num, tolerance, test_results);
            else
                PolygonUtils.testPointsOnLine2D(intersector, input_points, (int) num, tolerance, test_results);
            int i0 = 0;
            for (int i = 0; i < num; i++) {
    ...
    static Point intersection(Point point, Point point2, double tolerance) {
        ...
        if (CrackAndCluster.non_empty_points_need_to_cluster(tolerance, point, point2)) {
            return CrackAndCluster.cluster_non_empty_points(point, point2, 1, 1, 1, 1);
        }
    ...
    MultiVertexGeometry planarSimplifyImpl_(MultiVertexGeometry input_geom, double tolerance,
            boolean b_use_winding_rule_for_polygons, boolean dirty_result, ProgressTracker progress_tracker) {
    ...
    MultiVertexGeometry planarSimplify(EditShape shape, int geom, double tolerance,
            boolean b_use_winding_rule_for_polygons, boolean dirty_result, ProgressTracker progress_tracker) {
        ...
        try {
            ...
            if (shape.getGeometryType(geom) == Geometry.Type.Polygon.value() || (b_use_winding_rule_for_polygons
                    && shape.getGeometryType(geom) != Geometry.Type.MultiPoint.value())) {
                ...
                if (!dirty_result) {
                    ((MultiVertexGeometryImpl) polygon._getImpl()).setIsSimple(GeometryXSimple.Strong, tolerance,
                            false);
                    ((MultiPathImpl) polygon._getImpl())._updateOGCFlags();
                } else
                    ((MultiVertexGeometryImpl) polygon._getImpl()).setIsSimple(GeometryXSimple.Weak, 0.0, false);// dirty
                                                                                                                 // result
                                                                                                                 // means
                                                                                                                 // simple
                                                                                                                 // but
                ...
                return polygon;
            } else if (shape.getGeometryType(geom) == Geometry.Type.Polyline.value()) {
                int resGeom = topoOperationPolylinePolylineOrPolygon_(-1);

                Polyline polyline = (Polyline) shape.getGeometry(resGeom);
                if (!dirty_result)
                    ((MultiVertexGeometryImpl) polyline._getImpl()).setIsSimple(GeometryXSimple.Strong, tolerance,
                            false);

                return polyline;
            } else if (shape.getGeometryType(geom) == Geometry.Type.MultiPoint.value()) {
                int resGeom = topoOperationMultiPoint_();

                MultiPoint mp = (MultiPoint) shape.getGeometry(resGeom);
                if (!dirty_result)
                    ((MultiVertexGeometryImpl) mp._getImpl()).setIsSimple(GeometryXSimple.Strong, tolerance, false);
    ...
    // static
    static MultiVertexGeometry planarSimplify(MultiVertexGeometry input_geom, double tolerance,
            boolean use_winding_rule_for_polygons, boolean dirty_result, ProgressTracker progress_tracker) {
        TopologicalOperations topoOps = new TopologicalOperations();
        return topoOps.planarSimplifyImpl_(input_geom, tolerance, use_winding_rule_for_polygons, dirty_result,
                progress_tracker);
    }

    boolean planarSimplifyNoCrackingAndCluster(boolean OGCoutput, EditShape shape, int geom,
        ...
        if (shape.getGeometryType(geom) == GeometryType.Polygon || (rule == Polygon.FillRule.enumFillRuleWinding
                && shape.getGeometryType(geom) != GeometryType.MultiPoint)) {
            // geom can be a polygon or a polyline.
            // It can be a polyline only when the winding rule is true.
            shape.setFillRule(geom, Polygon.FillRule.enumFillRuleOddEven);
            int resGeom = topoOperationPolygonPolygon_(geom, -1, -1);
    ...
    static MultiVertexGeometry simplifyOGC(MultiVertexGeometry input_geom, double tolerance, boolean dirty_result,
            ProgressTracker progress_tracker) {
        TopologicalOperations topoOps = new TopologicalOperations();
    ...
    public int intersection(int geometry_a, int geometry_b) {
        ...
        boolean b_vertex_dominance = (m_topo_graph.getShape().getVertexDescription().getAttributeCount() > 1);
    ...
    // static
    public static Geometry difference(Geometry geometry_a, Geometry geometry_b, SpatialReference sr,
            ProgressTracker progress_tracker) {
        ...
        double tolerance = InternalUtils.calculateToleranceFromGeometry(sr, envMerged, true);// conservative to have
                                                                                             // same effect as simplify
        ...
        if (Geometry.isMultiPath(res_geom.getType().value())) {
            ((MultiVertexGeometryImpl) res_geom._getImpl()).setIsSimple(GeometryXSimple.Strong, tolerance, false);
    ...
    public static Geometry dissolve(Geometry geometry_a, Geometry geometry_b, SpatialReference sr,
            ProgressTracker progress_tracker) {
        ...
        Geometry res_geom = normalizeResult_(edit_shape.getGeometry(result), geometry_a, geometry_b, '|');
    ...
    static Geometry dissolveDirty(ArrayList<Geometry> geometries, SpatialReference sr,
            ProgressTracker progress_tracker) {
        ...
        for (int i = 0, n = geometries.size(); i < n; i++) {
            dim = StrictMath.max(geometries.get(i).getDimension(), dim);
        }
        ...
        for (int i = 0, n = geometries.size(); i < n; i++) {
            if (geometries.get(i).getDimension() == dim) {
                if (!geometries.get(i).isEmpty()) {
                    any_index = i;
                    if (geom == -1)
                        geom = shape.addGeometry(normalizeInputGeometry_(geometries.get(i)));
                    else
                        shape.appendGeometry(geom, normalizeInputGeometry_(geometries.get(i)));
    ...
    // static
    public static Geometry intersection(Geometry geometry_a, Geometry geometry_b, SpatialReference sr,
            ProgressTracker progress_tracker) {
    ...
    static Geometry[] intersectionEx(Geometry geometry_a, Geometry geometry_b, SpatialReference sr,
            ProgressTracker progress_tracker) {
        ...
        {
            if (geometry_a.getDimension() <= geometry_b.getDimension()) {
                Geometry geom = normalizeResult_(normalizeInputGeometry_(geometry_a.createInstance()), geometry_a,
                        geometry_b, '&');
            ...
            if (geometry_a.getDimension() > geometry_b.getDimension()) {
                Geometry geom = normalizeResult_(normalizeInputGeometry_(geometry_b.createInstance()), geometry_a,
                        geometry_b, '&');
                res_vec[geom.getDimension()] = geom;
                return res_vec;
            }
        ...
        for (int i = 0; i < result_geom_handles.length; i++) {
            Geometry res_geom = normalizeResult_(edit_shape.getGeometry(result_geom_handles[i]), geometry_a, geometry_b,
                    '&');

            if (Geometry.isMultiPath(res_geom.getType().value())) {
                ((MultiVertexGeometryImpl) res_geom._getImpl())
                        .setIsSimple(MultiVertexGeometryImpl.GeometryXSimple.Strong, tolerance, false);
    ...
    // static
    public static Geometry symmetricDifference(Geometry geometry_a, Geometry geometry_b, SpatialReference sr,
            ProgressTracker progress_tracker) {
        if (geometry_a.getDimension() > geometry_b.getDimension())
            return normalizeResult_(normalizeInputGeometry_(geometry_a), geometry_a, geometry_b, '^');
        ...
        for (int cluster = m_topo_graph.getFirstCluster(); cluster != -1; cluster = m_topo_graph
            ...
            do {
                ...
                if (visited != 1) {
                    ...
                    if (bFoundCutter && m_topo_graph.getChainArea(m_topo_graph.getHalfEdgeChain(toHalfEdge)) > 0.0) {// if
                                                                                                                     // we
                                                                                                                     // found
                                                                                                                     // a
                                                                                                                     // cutter
                        ...
                        if (sideIndex != -1)
                            shape.setGeometryUserIndex(geometry, sideIndex, side); // what is that?
                    }
    ...
    // call this if EditShape instance has to survive the TopologicalOperations
    // life.
    void removeShape() {
        if (m_topo_graph != null) {
            m_topo_graph.removeShape();
