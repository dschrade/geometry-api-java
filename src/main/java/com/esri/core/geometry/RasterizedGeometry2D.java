...
public abstract class RasterizedGeometry2D {
     ...
     * 
     * @param geom
     *            The input geometry to rasterize. It has to be a
     *            MultiVertexGeometry instance.
     * @param toleranceXY
     *            The tolerance of the rasterization. Raster pixels that are closer
     *            than given tolerance to the Geometry will be set.
     * @param rasterSizeBytes
     *            The max size of the raster in bytes. The raster has size of
     *            rasterSize x rasterSize. Polygons are rasterized into 2 bpp (bits
     *            per pixel) rasters while other geometries are rasterized into 1
     *            bpp rasters. 32x32 pixel raster for a polygon would take 256 bytes
     *            of memory
     */
    ...
    public static int rasterSizeFromAccelerationDegree(GeometryAccelerationDegree accelDegree) {
        int value = 0;
        switch (accelDegree) {
        case enumMild:
            value = 64 * 64 * 2 / 8;// 1k
