...
final class PolygonUtils {
    ...
    /**
     * Tests if Point is inside the Polygon. Returns PiPOutside if not in polygon,
     * PiPInside if in the polygon, PiPBoundary is if on the border. It tests border
     * only if the tolerance is greater than 0, otherwise PiPBoundary cannot be
     * returned. Note: If the tolerance is not 0, the test is more expensive because
     * it calculates closest distance from a point to each segment.
     * 
    ...
    static PiPResult isPointInPolygon2D(Polygon polygon, double inputPointXVal, double inputPointYVal,
            double tolerance) {
     ...
     * border only if the tolerance is greater than 0, otherwise PiPBoundary cannot
     ...
     * PiPOutside if not in any outer ring, PiPInside if in the any outer ring, or
     * on the boundary. PiPBoundary is never returned. Note: If the tolerance is not
     * 0, the test is more expensive because it calculates closest distance from a
    ...
    /**
     * Tests point is inside the Polygon for an array of points. Returns PiPOutside
     * if not in polygon, PiPInside if in the polygon, PiPBoundary is if on the
     * border. It tests border only if the tolerance is greater than 0, otherwise
     * PiPBoundary cannot be returned. Note: If the tolerance is not 0, the test is
     * more expensive.
     * 
     * O(n*m) complexity, where n is the number of polygon segments, m is the number
     * of input points.
     */
    public static void testPointsInPolygon2D(Polygon polygon, Point2D[] inputPoints, int count, double tolerance,
    ...
    static void testPointsInPolygon2D(Polygon polygon, double[] xyStreamBuffer, int pointCount, double tolerance,
            PiPResult[] testResults) {
    ...
    /**
     * Tests point is inside an Area Geometry (Envelope, Polygon) for an array of
     * points. Returns PiPOutside if not in area, PiPInside if in the area,
     * PiPBoundary is if on the border. It tests border only if the tolerance is
     * greater than 0, otherwise PiPBoundary cannot be returned. Note: If the
     * tolerance is not 0, the test is more expensive.
     * 
     * O(n*m) complexity, where n is the number of polygon segments, m is the number
     * of input points.
     */
    public static void testPointsInArea2D(Geometry polygon, Point2D[] inputPoints, int count, double tolerance,
            PiPResult[] testResults) {
        if (polygon.getType() == Geometry.Type.Polygon)
            testPointsInPolygon2D((Polygon) polygon, inputPoints, count, tolerance, testResults);
    ...
    public static void testPointsInArea2D(Geometry polygon, double[] xyStreamBuffer, int count, double tolerance,
            PiPResult[] testResults) {
        if (polygon.getType() == Geometry.Type.Polygon)
            testPointsInPolygon2D((Polygon) polygon, xyStreamBuffer, count, tolerance, testResults);
        else if (polygon.getType() == Geometry.Type.Envelope) {
            Envelope2D env2D = new Envelope2D();
            ((Envelope) polygon).queryEnvelope2D(env2D);
    ...
    private static void _testPointsInEnvelope2D(Envelope2D env2D, Point2D[] inputPoints, int count, double tolerance,
            PiPResult[] testResults) {
        if (inputPoints.length < count || testResults.length < count)
    ...
    private static void _testPointsInEnvelope2D(Envelope2D env2D, double[] xyStreamBuffer, int pointCount,
            double tolerance, PiPResult[] testResults) {
        if (xyStreamBuffer.length / 2 < pointCount || testResults.length < pointCount)
            throw new IllegalArgumentException();
        ...
        envOut.inflate(tolerance * 0.5, tolerance * 0.5);
        for (int i = 0; i < pointCount; i++) {
            if (envIn.contains(xyStreamBuffer[i * 2], xyStreamBuffer[i * 2 + 1]))
                testResults[i] = PiPResult.PiPInside;
            else if (!envIn.contains(xyStreamBuffer[i * 2], xyStreamBuffer[i * 2 + 1]))
                testResults[i] = PiPResult.PiPOutside;
            else
                testResults[i] = PiPResult.PiPBoundary;
        }
    ...
    static void testPointsOnSegment_(Segment seg, Point2D[] input_points, int count, double tolerance,
    ...
    static void testPointsOnPolyline2D_(Polyline poly, Point2D[] input_points, int count, double tolerance,
            PolygonUtils.PiPResult[] test_results) {
    ...
    static void testPointsOnLine2D(Geometry line, Point2D[] input_points, int count, double tolerance,
            PolygonUtils.PiPResult[] test_results) {
        Geometry.Type gt = line.getType();
        if (gt == Geometry.Type.Polyline)
            testPointsOnPolyline2D_((Polyline) line, input_points, count, tolerance, test_results);
        else if (Geometry.isSegment(gt.value())) {
            testPointsOnSegment_((Segment) line, input_points, count, tolerance, test_results);
        } else
