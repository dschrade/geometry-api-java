...
class ConvexHull {
    ...
    private ConvexHull(AttributeStreamOfDbl stream, int n) {
        m_tree_hull = new Treap();
        m_tree_hull.setCapacity(StrictMath.min(20, n));
        m_stream = stream;
    ...
    Geometry getBoundingGeometry() {
        // Extracts the convex hull from the tree. Reading the tree in order from first
        // to last is the resulting convex hull.
     ...
     * Returns a Geometry. \param mvg The geometry used to create the convex hull.
     */
    ...
    static Geometry construct(MultiVertexGeometry mvg) {
        ...
        AttributeStreamOfDbl stream = (AttributeStreamOfDbl) mvg_impl
        ...
        if (tm < N) {
            ...
            for (int tp = tm + 1; tp < mvg_impl.getPointCount(); tp++) {// Dynamically insert into the current convex
                                                                        // hull
     ...
     * out_convex_hull array will be populated with the subset of index positions
     * which contribute to the convex hull. Returns the number of points in the
     * convex hull. \param points The points used to create the convex hull. \param
     * count The number of points in the input Point2D array. \param out_convex_hull
     * An index array allocated by the user at least as big as the size of the input
     * points array.
     */
     ...
     * convex. \param path_index The path of the MultiPath to check if its convex.
     */
    static boolean isPathConvex(MultiPath multi_path, int path_index, ProgressTracker progress_tracker) {
        ...
        // This matches the logic for case 1 of the tree hull algorithm. The idea is
        // inductive. We assume we have a convex hull pt_0,...,pt_m, and we see if
        // a new point (pt_pivot) is among the transitive tournament for pt_0, knowing
        // that pt_pivot comes after pt_m.

        // We check three conditions:
        // 1) pt_m->pt_pivot->pt_0 is clockwise (closure across the boundary is convex)
        // 2) pt_1->pt_pivot->pt_0 is clockwise (the first step forward is convex) (pt_1
        // is the next point after pt_0)
        // 3) pt_m->pt_pivot->pt_m_prev is clockwise (the first step backwards is
        // convex) (pt_m_prev is the previous point before pt_m)

        // If all three of the above conditions are clockwise, then pt_pivot is among
        // the transitive tournament for pt_0, and therefore the polygon pt_0, ...,
        // pt_m, pt_pivot is convex.
        ...
        // Assume that pt_0,...,pt_m is convex. Check if the next point, pt_pivot,
        // maintains the convex invariant.
    ...
    private int addPoint_(Point2D pt_p) {
        ...
        if (m_tree_hull.size(-1) == 1) {
            ...
            if (!pt_p.isEqual(pt_0, NumberUtils.doubleEps())) // We don't want to close the gap between t0 and tm.
                p = m_tree_hull.addBiggestElement(-5, -1); // set place holder to -5 to indicate the second element
                                                           // being added (tm).
    ...
    private int treeHull_(Point2D pt_pivot) {
        ...
        do {
            ...
            if (isCounterClockwise_(orient_m_p_0)) {// Case 2: tp->tm->t0 is clockwise
                int k = m_tree_hull.getRoot(-1), k_min = m_tree_hull.getFirst(-1), k_max = m_tree_hull.getLast(-1),
                        k_prev;
                int tk, tk_prev;
                Point2D pt_k = new Point2D();
                Point2D pt_k_prev = new Point2D();

                while (k_min != m_tree_hull.getPrev(k_max)) {// binary search to find k such that t0->tp->tj holds (i.e.
                                                             // clockwise) for j >= k. Hence, tj->tp->t0 is clockwise
                                                             // (or degenerate) for j < k.
                ...
                assert (isCounterClockwise_(Point2D.orientationRobust(pt_k, pt_pivot, pt_0))
                        && !isCounterClockwise_(Point2D.orientationRobust(pt_k_prev, pt_pivot, pt_0)));
                ...
                p = m_tree_hull.addElementAtPosition(k_prev, k, -2, true, false, -1); // set place holder to -2 for case
                                                                                      // 2.
            ...
            {// Case 3: degenerate
                ...
                if (between == -1) {
                    ...
                    p = m_tree_hull.addElementAtPosition(-1, j, -3, true, false, -1); // set place holder to -3 for case
                                                                                      // 3.
    ...
    private int treeHullWalkForward_(Point2D pt_pivot, int start, int end) {
        ...
        while (j != end && m_tree_hull.size(-1) > 2) {// Stops when we find a clockwise triple containting the pivot
                                                      // point, or when the tree_hull size is 2. Deletes non-clockwise
    ...
    private int treeHullWalkBackward_(Point2D pt_pivot, int start, int end) {
        ...
        while (l != end && m_tree_hull.size(-1) > 2) {// Stops when we find a clockwise triple containting the pivot
                                                      // point, or when the tree_hull size is 2. Deletes non-clockwise
                                                      // triples along the way.
    ...
    private static int isBetween_(Point2D pt_pivot, Point2D pt_m, Point2D pt_0) {
        ...
        if (pt_m.y == pt_0.y) {
            ...
            double diff_x = StrictMath.abs(pt_m.x - pt_0.x);
            double diff_y = StrictMath.abs(pt_m.y - pt_0.y);
