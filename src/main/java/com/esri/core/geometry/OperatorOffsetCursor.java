...
class OperatorOffsetCursor extends GeometryCursor {
    ...
    OperatorOffsetCursor(GeometryCursor inputGeometries, SpatialReference sr, double distance,
            OperatorOffset.JoinType joins, double bevelRatio, double flattenError, ProgressTracker progressTracker) {
        m_index = -1;
    ...
    Geometry Offset(Geometry geom) {
        double tolerance;
        if (m_flattenError <= 0)
            tolerance = InternalUtils.calculateToleranceFromGeometry(m_spatialReference, geom, false);
        else
            tolerance = m_flattenError;
        return ConstructOffset.execute(geom, m_distance, m_joins, m_miterLimit, tolerance, m_progressTracker);
    }
