...
class OperatorImportFromJsonCursor extends MapGeometryCursor {
        ...
        try {
            ...
            while (parser.nextToken() != JsonReader.Token.END_OBJECT) {
                ...
                if (!bFoundSpatial_reference && name.equals("spatialReference")) {
                    ...
                    continue;
                } else if (!bFoundX && name.equals("x")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Point)) {
                    bFoundX = true;
                    x = readDouble(parser);
                } else if (!bFoundY && name.equals("y")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Point)) {
                    bFoundY = true;
                    y = readDouble(parser);
                } else if (!bFoundZ && name.equals("z")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Point)) {
                    bFoundZ = true;
                    z = readDouble(parser);
                } else if (!bFoundM && name.equals("m")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Point)) {
                    bFoundM = true;
                    m = readDouble(parser);
                }
                if (!bFoundXMin && name.equals("xmin")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundXMin = true;
                    xmin = readDouble(parser);
                } else if (!bFoundYMin && name.equals("ymin")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundYMin = true;
                    ymin = readDouble(parser);
                } else if (!bFoundMMin && name.equals("mmin")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundMMin = true;
                    mmin = readDouble(parser);
                } else if (!bFoundZMin && name.equals("zmin")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundZMin = true;
                    zmin = readDouble(parser);
                } else if (!bFoundXMax && name.equals("xmax")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundXMax = true;
                    xmax = readDouble(parser);
                } else if (!bFoundYMax && name.equals("ymax")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundYMax = true;
                    ymax = readDouble(parser);
                } else if (!bFoundMMax && name.equals("mmax")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundMMax = true;
                    mmax = readDouble(parser);
                } else if (!bFoundZMax && name.equals("zmax")
                        && (gt == Geometry.GeometryType.Unknown || gt == Geometry.GeometryType.Envelope)) {
                    bFoundZMax = true;
            ...
            if (bFoundPolygon || bFoundPolyline || bFoundMultiPoint) {
                ...
                geometry = p;
            } else if (bFoundXMin || bFoundYMin || bFoundXMax || bFoundYMax || bFoundZMin || bFoundZMax || bFoundMMin
                    || bFoundMMax) {
    ...
    private static double readDouble(JsonReader parser) {
        if (parser.currentToken() == JsonReader.Token.VALUE_NULL
                || parser.currentToken() == JsonReader.Token.VALUE_STRING && parser.currentString().equals("NaN"))
            return NumberUtils.NaN();
        else
            return parser.currentDoubleValue();
    }

    private static Geometry importFromJsonMultiPoint(JsonReader parser, AttributeStreamOfDbl as,
            AttributeStreamOfDbl bs) throws Exception {
        ...
        AttributeStreamOfDbl position = (AttributeStreamOfDbl) (AttributeStreamBase.createDoubleStream(2, 0));
    ...
    private static Geometry importFromJsonMultiPath(boolean b_polygon, JsonReader parser, AttributeStreamOfDbl as,
            AttributeStreamOfDbl bs) throws Exception {
        if (parser.currentToken() != JsonReader.Token.START_ARRAY)
        ...
        while (parser.nextToken() != JsonReader.Token.END_ARRAY) {
            ...
            if (b_polygon && pathPointCount > requiredSize && sz == szstart && start[0] == buf[0] && start[1] == buf[1]
