...
final class OperatorUnionCursor extends GeometryCursor {
    ...
    ArrayList<TreeMap<Integer, Bin_type>> m_union_bins = new ArrayList<TreeMap<Integer, Bin_type>>();// for each
                                                                                                     // dimension there
                                                                                                     // is a list of
                                                                                                     // bins sorted by
                                                                                                     // level
    ...
    private Geometry get_result_geometry(int dim) {
        ...
        if (unioned) {
            resG = OperatorSimplify.local().execute(resG, m_spatial_reference, false, m_progress_tracker);
        ...
        if (geom != null) {
            ...
            if (dim >= m_max_dimension && !m_b_union_all_dimensions) {
                ...
                if (dim > m_max_dimension && !m_b_union_all_dimensions) {
                    ...
                    // Therefore we delete all lower dimensions (unless m_b_union_all_dimensions is
                    // true).
                ...
                while (m_dim_geom_counts[dim] > 1) {
                    ...
                    if (serial_execution) {
                        if (batch_to_union.size() != 0) {
                            Geometry geomRes = TopologicalOperations.dissolveDirty(batch_to_union, m_spatial_reference,
                                    m_progress_tracker);
    ...
    ArrayList<Geometry> collect_geometries_to_union(int dim) {
        ...
        Set<Map.Entry<Integer, Bin_type>> set = m_union_bins.get(dim).entrySet();
    ...
    private void add_geom(int dimension, boolean unioned, Geometry geom) {
        ...
        if (dimension + 1 > (int) m_union_bins.size()) {
            for (int i = 0, n = StrictMath.max(2, dimension + 1); i < n; i++) {
        ...
        m_added_geoms++;
        m_max_dimension = StrictMath.max(m_max_dimension, dimension);
    }

    private static int get_level_(int sz) {// calculates logarithm of sz to base
                                           // 4.
        return sz > 0 ? (int) (StrictMath.log((double) sz) / StrictMath.log(4.0) + 0.5) : (int) 0;
    }
