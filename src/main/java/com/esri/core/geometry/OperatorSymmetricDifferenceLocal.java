...
class OperatorSymmetricDifferenceLocal extends OperatorSymmetricDifference {
    ...
    @Override
    public GeometryCursor execute(GeometryCursor inputGeometries, GeometryCursor rightGeometry, SpatialReference sr,
            ProgressTracker progressTracker) {
        return new OperatorSymmetricDifferenceCursor(inputGeometries, rightGeometry, sr, progressTracker);
    }

    @Override
    public Geometry execute(Geometry leftGeometry, Geometry rightGeometry, SpatialReference sr,
            ProgressTracker progressTracker) {
        SimpleGeometryCursor leftGeomCurs = new SimpleGeometryCursor(leftGeometry);
        SimpleGeometryCursor rightGeomCurs = new SimpleGeometryCursor(rightGeometry);
        GeometryCursor geometryCursor = execute(leftGeomCurs, rightGeomCurs, sr, progressTracker);
        return geometryCursor.next();
    }

    static Geometry symmetricDifference(Geometry geometry_a, Geometry geometry_b, SpatialReference spatial_reference,
        ...
        if (type_a == Geometry.GeometryType.Point && type_b == Geometry.GeometryType.Point)
            return pointSymDiffPoint_((Point) (geometry_a), (Point) (geometry_b), tolerance, progress_tracker);
        ...
        if (type_a != type_b) {
            ...
            if (type_a == Geometry.GeometryType.MultiPoint)
                return multiPointSymDiffPoint_((MultiPoint) (geometry_a), (Point) (geometry_b), tolerance,
                        progress_tracker);

            return multiPointSymDiffPoint_((MultiPoint) (geometry_b), (Point) (geometry_a), tolerance,
                    progress_tracker);
        }

        return TopologicalOperations.symmetricDifference(geometry_a, geometry_b, spatial_reference, progress_tracker);
    }

    static Geometry pointSymDiffPoint_(Point point_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    static Geometry multiPointSymDiffPoint_(MultiPoint multi_point, Point point, double tolerance,
            ProgressTracker progress_tracker) {
