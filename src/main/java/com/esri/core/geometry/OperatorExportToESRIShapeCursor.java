...
class OperatorExportToESRIShapeCursor extends ByteBufferCursor {
    ...
    static int exportToESRIShape(int exportFlags, Geometry geometry, ByteBuffer shapeBuffer) {
        ...
        case Geometry.GeometryType.Polygon:
            return exportMultiPathToESRIShape(true, exportFlags, (MultiPath) geometry, shapeBuffer);
        case Geometry.GeometryType.Polyline:
            return exportMultiPathToESRIShape(false, exportFlags, (MultiPath) geometry, shapeBuffer);
        case Geometry.GeometryType.MultiPoint:
            return exportMultiPointToESRIShape(exportFlags, (MultiPoint) geometry, shapeBuffer);
        case Geometry.GeometryType.Point:
            return exportPointToESRIShape(exportFlags, (Point) geometry, shapeBuffer);
        case Geometry.GeometryType.Envelope:
            return exportEnvelopeToESRIShape(exportFlags, (Envelope) geometry, shapeBuffer);
        default: {
    ...
    private static int exportEnvelopeToESRIShape(int exportFlags, Envelope envelope, ByteBuffer shapeBuffer) {
        ...
        int size = (4 /* type */) + (4 * 8 /* envelope */) + (4 /* part count */) + (4 /* point count */)
                + (partCount * 4 /* start indices */) + pointCount * 2 * 8 /* xy coordinates */;
        ...
        if (!bExportZs && !bExportMs) {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralPolygon | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapePolygon;
        } else if (bExportZs && !bExportMs) {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralPolygon | ShapeModifiers.ShapeHasZs | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapePolygonZ;
        } else if (bExportMs && !bExportZs) {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralPolygon | ShapeModifiers.ShapeHasMs | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapePolygonM;
        } else {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralPolygon | ShapeModifiers.ShapeHasZs | ShapeModifiers.ShapeHasMs
    ...
    private static int exportPointToESRIShape(int exportFlags, Point point, ByteBuffer shapeBuffer) {
        boolean bExportZ = point.hasAttribute(Semantics.Z) && (exportFlags & ShapeExportFlags.ShapeExportStripZs) == 0;
        boolean bExportM = point.hasAttribute(Semantics.M) && (exportFlags & ShapeExportFlags.ShapeExportStripMs) == 0;
        ...
        if (!bExportZ && !bExportM) {
            ...
            if (bExportID)
                type = ShapeType.ShapeGeneralPoint | ShapeModifiers.ShapeHasZs | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapePointZ;
        } else if (bExportM && !bExportZ) {
            if (bExportID)
                type = ShapeType.ShapeGeneralPoint | ShapeModifiers.ShapeHasMs | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapePointM;
        } else {
            if (bExportID)
                type = ShapeType.ShapeGeneralPoint | ShapeModifiers.ShapeHasZs | ShapeModifiers.ShapeHasMs
                        | ShapeModifiers.ShapeHasIDs;
            else
        ...
        shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(x) : x);
        offset += 8;
        shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(y) : y);
        offset += 8;
        ...
        if (bExportM) {
            ...
            shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(m) : m);
            offset += 8;
        }
    ...
    private static int exportMultiPointToESRIShape(int exportFlags, MultiPoint multipoint, ByteBuffer shapeBuffer) {
        ...
        if (!bExportZs && !bExportMs) {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralMultiPoint | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapeMultiPoint;
        } else if (bExportZs && !bExportMs) {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralMultiPoint | ShapeModifiers.ShapeHasZs | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapeMultiPointZ;
        } else if (bExportMs && !bExportZs) {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralMultiPoint | ShapeModifiers.ShapeHasMs | ShapeModifiers.ShapeHasIDs;
            else
                type = ShapeType.ShapeMultiPointM;
        } else {
            if (bExportIDs)
                type = ShapeType.ShapeGeneralMultiPoint | ShapeModifiers.ShapeHasZs | ShapeModifiers.ShapeHasMs
                        | ShapeModifiers.ShapeHasIDs;
            else
        ...
        if (pointCount > 0) {
            // write xy coordinates
            AttributeStreamBase positionStream = multipointImpl.getAttributeStreamRef(Semantics.POSITION);
        ...
        if (bExportZs) {
            ...
            offset += 8;
            shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(zInterval.vmax) : zInterval.vmax);
            offset += 8;

            if (pointCount > 0) {
                if (multipointImpl._attributeStreamIsAllocated(Semantics.Z)) {
                    AttributeStreamOfDbl zs = (AttributeStreamOfDbl) multipointImpl.getAttributeStreamRef(Semantics.Z);
                    for (int i = 0; i < pointCount; i++) {
                        double z = zs.read(i);
                        shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(z) : z);
                        offset += 8;
        ...
        if (bExportMs) {
            ...
            offset += 8;
            shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(mInterval.vmax) : mInterval.vmax);
            offset += 8;

            if (pointCount > 0) {
                if (multipointImpl._attributeStreamIsAllocated(Semantics.M)) {
                    AttributeStreamOfDbl ms = (AttributeStreamOfDbl) multipointImpl.getAttributeStreamRef(Semantics.M);
                    for (int i = 0; i < pointCount; i++) {
                        double m = ms.read(i);
                        shapeBuffer.putDouble(offset, bArcViewNaNs ? Interop.translateToAVNaN(m) : m);
                        offset += 8;
        ...
        if (bExportIDs) {
            if (pointCount > 0) {
                if (multipointImpl._attributeStreamIsAllocated(Semantics.ID)) {
                    ...
                    int id = (int) VertexDescription.getDefaultValue(Semantics.ID);
                    for (int i = 0; i < pointCount; i++)
                        shapeBuffer.putInt(offset, id);
                    offset += 4;
                }
    ...
    private static int exportMultiPathToESRIShape(boolean bPolygon, int exportFlags, MultiPath multipath,
        ...
        if (!bExportZs && !bExportMs) {
            if (bExportIDs || bHasCurves) {
                type = bPolygon ? ShapeType.ShapeGeneralPolygon : ShapeType.ShapeGeneralPolyline;
                if (bExportIDs)
                    type |= ShapeModifiers.ShapeHasIDs;
                if (bHasCurves)
                    type |= ShapeModifiers.ShapeHasCurves;
            } else
                type = bPolygon ? ShapeType.ShapePolygon : ShapeType.ShapePolyline;
        } else if (bExportZs && !bExportMs) {
            if (bExportIDs || bHasCurves) {
                type = bPolygon ? ShapeType.ShapeGeneralPolygon : ShapeType.ShapeGeneralPolyline;
                ...
                type = bPolygon ? ShapeType.ShapePolygonZ : ShapeType.ShapePolylineZ;
        } else if (bExportMs && !bExportZs) {
            if (bExportIDs || bHasCurves) {
                type = bPolygon ? ShapeType.ShapeGeneralPolygon : ShapeType.ShapeGeneralPolyline;
                ...
                type = bPolygon ? ShapeType.ShapePolygonM : ShapeType.ShapePolylineM;
        } else {
            if (bExportIDs || bHasCurves) {
                type = bPolygon ? ShapeType.ShapeGeneralPolygon : ShapeType.ShapeGeneralPolyline;
                ...
                type = bPolygon ? ShapeType.ShapePolygonZM : ShapeType.ShapePolylineZM;
        }
        ...
        if (pointCount > 0) {
            // write xy coordinates
            AttributeStreamBase positionStream = multipathImpl.getAttributeStreamRef(Semantics.POSITION);
            AttributeStreamOfDbl position = (AttributeStreamOfDbl) positionStream;
        ...
        if (bExportZs) {
            ...
            if (pointCount > 0) {
                if (multipathImpl._attributeStreamIsAllocated(Semantics.Z)) {
                    AttributeStreamOfDbl zs = (AttributeStreamOfDbl) multipathImpl.getAttributeStreamRef(Semantics.Z);
                    for (int ipart = 0; ipart < partCount; ipart++) {
                        int partStart = multipathImpl.getPathStart(ipart);
                        int partEnd = multipathImpl.getPathEnd(ipart);
                        for (int i = partStart; i < partEnd; i++) {
        ...
        if (bExportMs) {
            ...
            if (pointCount > 0) {
                if (multipathImpl._attributeStreamIsAllocated(Semantics.M)) {
                    AttributeStreamOfDbl ms = (AttributeStreamOfDbl) multipathImpl.getAttributeStreamRef(Semantics.M);
                    for (int ipart = 0; ipart < partCount; ipart++) {
                        int partStart = multipathImpl.getPathStart(ipart);
                        int partEnd = multipathImpl.getPathEnd(ipart);
                        for (int i = partStart; i < partEnd; i++) {
