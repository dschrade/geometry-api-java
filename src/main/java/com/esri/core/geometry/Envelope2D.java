...
public final class Envelope2D implements Serializable {
    ...
    /**
     * Sets the envelope from the array of points. The envelope will be set to empty
     * if the array is null.
     * 
     * @param points
     *            The points to set the envelope from. No element in the array can
     *            be null.
     */
    ...
    public boolean isIntersecting(Envelope2D other) {
        ...
        // (IEEE math)
        return ((xmin <= other.xmin) ? xmax >= other.xmin : other.xmax >= xmin) && // check that x projections overlap
     ...
     * @param other
     *            The other envelope.
     * @return True if this envelope intersects the other. Assumes this and other
     *         envelopes are not empty.
     */
    ...
    /**
     * Queries corners into a given array. The array length must be at least 4.
     * Starts from the lower left corner and goes clockwise.
     * 
     * @param corners
    ...
    /**
     * Queries corners into a given array in reversed order. The array length must
     * be at least 4. Starts from the lower left corner and goes counterclockwise.
     * 
     * @param corners
     *            The array of four points.
     */
    ...
    public void normalize() {
        ...
        double min = StrictMath.min(xmin, xmax);
        double max = StrictMath.max(xmin, xmax);
        xmin = min;
        xmax = max;
        min = StrictMath.min(ymin, ymax);
        max = StrictMath.max(ymin, ymax);
        ymin = min;
        ymax = max;
    }
    ...
    /**
     * Returns True if this envelope is valid (empty, or has xmin less or equal to
     * xmax, or ymin less or equal to ymax).
     * 
    ...
    double _calculateToleranceFromEnvelope() {
        if (isEmpty())
            return NumberUtils.doubleEps() * 100.0; // GEOMTERYX_EPSFACTOR
                                                    // 100.0;
        double r = StrictMath.abs(xmin) + StrictMath.abs(xmax) + StrictMath.abs(ymin) + StrictMath.abs(ymax) + 1;
    ...
    // 4 - the segment is complitely inside of the clipping window
    {
        ...
        do {
            ...
            if (bDX) {
                ...
                /*
                 * if (c1) //original code. Faster, but less robust numerically. ( //The
                 * Cohen-Sutherland Line-Clipping Algorithm) { if (c1 & XLESSXMIN) { p1.y += dy
                 * * (xmin - p1.x) / dx; p1.x = xmin; } else if (c1 & XGREATERXMAX) { p1.y += dy
                 * * (xmax - p1.x) / dx; p1.x = xmax; } else if (c1 & YLESSYMIN) { p1.x += dx *
                 * (ymin - p1.y) / dy; p1.y = ymin; } else if (c1 & YGREATERYMAX) { p1.x += dx *
                 * (ymax - p1.y) / dy; p1.y = ymax; }
                 * 
                 * c1 = _clipCode(p1, ClipRect); } else { if (c2 & XLESSXMIN) { p2.y += dy *
                 * (xmin - p2.x) / dx; p2.x = xmin; } else if (c2 & XGREATERXMAX) { p2.y += dy *
                 * (xmax - p2.x) / dx; p2.x = xmax; } else if (c2 & YLESSYMIN) { p2.x += dx *
                 * (ymin - p2.y) / dy; p2.y = ymin; } else if (c2 & YGREATERYMAX) { p2.x += dx *
    ...
    int clipLine(Point2D p0, Point2D p1, int lineExtension, double[] segParams, double[] boundaryDistances) {
        ...
        if (clipLineAuxiliary(delta.x, xmin - p0.x, segParams) && clipLineAuxiliary(-delta.x, p0.x - xmax, segParams)
    ...
    public boolean isPointOnBoundary(Point2D pt, double tolerance) {
        return StrictMath.abs(pt.x - xmin) <= tolerance || StrictMath.abs(pt.x - xmax) <= tolerance
                || StrictMath.abs(pt.y - ymin) <= tolerance || StrictMath.abs(pt.y - ymax) <= tolerance;
    }

    /**
     * Calculates minimum distance from this envelope to the other. Returns 0 for
     * empty envelopes.
     * 
     * @param other
     *            The other envelope.
    ...
    public double distance(Envelope2D other) {
        return StrictMath.sqrt(sqrDistance(other));
    }

    /**
     * Calculates minimum distance from this envelope to the point. Returns 0 for
     * empty envelopes.
     * 
     * @param pt2D
     *            The other point.
    ...
    public double distance(Point2D pt2D) {
        return StrictMath.sqrt(sqrDistance(pt2D));
    }

    /**
     * Calculates minimum squared distance from this envelope to the other. Returns
     * 0 for empty envelopes.
     * 
     * @param other
     *            The other envelope.
    ...
    /**
     * Returns squared max distance between two bounding boxes. This is furthest
     * distance between points on the two envelopes.
     *
     * @param other
     *            The bounding box to calculate the max distance two.
    ...
    /**
     * Calculates minimum squared distance from this envelope to the point. Returns
     * 0 for empty envelopes.
     * 
     * @param pt2D
     *            The point.
