...
class Bufferer {
    ...
    /**
     * Result is always a polygon. For non positive distance and non-areas returns
     * an empty polygon. For points returns circles.
     */
    Geometry buffer(Geometry geometry, double distance, SpatialReference sr, double densify_dist,
        ...
        m_geometry = geometry;
        m_tolerance = InternalUtils.calculateToleranceFromGeometry(sr, env2D, true);// conservative to have same effect
                                                                                    // as simplify
        m_small_tolerance = InternalUtils.calculateToleranceFromGeometry(null, env2D, true);// conservative
                                                                                            // to have
                                                                                            // same
                                                                                            // effect as
                                                                                            // simplify
        ...
        m_distance = distance;
        m_abs_distance = StrictMath.abs(m_distance);
        ...
        double max_dd = StrictMath.abs(distance) * (1 - StrictMath.cos(StrictMath.PI / max_vertex_in_complete_circle));

        if (max_dd > densify_dist)
            densify_dist = max_dd;// the densify distance has to agree with the
                                  // max_vertex_in_complete_circle
        else {
            double vertex_count = StrictMath.PI / StrictMath.acos(1.0 - densify_dist / StrictMath.abs(distance));
            if (vertex_count < (double) max_vertex_in_complete_circle - 1.0) {
                max_vertex_in_complete_circle = (int) vertex_count;
                if (max_vertex_in_complete_circle < 12) {
                    max_vertex_in_complete_circle = 12;
                    densify_dist = StrictMath.abs(distance)
                            * (1 - StrictMath.cos(StrictMath.PI / max_vertex_in_complete_circle));
                }
            }
        }
        ...
        m_filter_tolerance = StrictMath.min(m_small_tolerance, densify_dist * 0.25);
    ...
    private void generateCircleTemplate_() {
        ...
        assert (N >= 4);
        int real_size = (N + 3) / 4;
        double dA = (StrictMath.PI * 0.5) / real_size;
        m_dA = dA;

        for (int i = 0; i < real_size * 4; i++)
            m_circle_template.add(null);

        double dcos = StrictMath.cos(dA);
        double dsin = StrictMath.sin(dA);
    ...
    private static final class GeometryCursorForMultiPoint extends GeometryCursor {
        ...
        GeometryCursorForMultiPoint(Bufferer parent, MultiPoint mp, double distance, SpatialReference sr,
                double densify_dist, int max_vertex_in_complete_circle, ProgressTracker progress_tracker) {
            m_parent = parent;
            m_index = 0;
            m_mp = mp;
            m_x = 0;
        ...
        public Geometry next() {
            ...
            if (m_buffered_polygon == null) {
                ...
                m_buffered_polygon = m_parent.buffer(point, m_distance, m_spatialReference, m_densify_dist,
            ...
            if (m_current_path_index < npaths) {
                ...
                if (!mp.isClosedPathInXYPlane(ind)) {
                    ...
                    while (m_current_path_index < mp.getPathCount()) {
                        Point2D start = mp.getXY(mp.getPathStart(m_current_path_index));
                ...
                Polyline tmp_polyline = new Polyline(m_polyline.getDescription());
                ...
                if (false) {
                    OperatorFactoryLocal.saveGeometryToEsriShapeDbg("c:/temp/_geom.bin", tmp_polyline);
                }
    ...
    private static final class GeometryCursorForPolyline extends GeometryCursor {
        ...
        public Geometry next() {
            ...
            if (m_index < mp.getPathCount()) {
                int ind = m_index;
                m_index++;
                return m_bufferer.bufferPolylinePath_((Polyline) m_geometry, ind, m_bfilter);
            }
    ...
    private Geometry bufferPolyline_() {
        ...
        GeometryCursor glueing_cursor = new GlueingCursorForPolyline(poly);// glues paths together if they connect at
                                                                           // one point
        poly = null;
        GeometryCursor generalized_paths = OperatorGeneralize.local().execute(glueing_cursor, m_densify_dist * 0.25,
                false, m_progress_tracker);
        GeometryCursor simple_paths = OperatorSimplifyOGC.local().execute(generalized_paths, null, true,
                m_progress_tracker);// make a planar graph.
        generalized_paths = null;
        GeometryCursor path_buffering_cursor = new GeometryCursorForPolyline(this, simple_paths, m_bfilter);
        simple_paths = null;
        GeometryCursor union_cursor = OperatorUnion.local().execute(path_buffering_cursor, m_spatialReference,
                m_progress_tracker);// (int)Operator_union::Options::enum_disable_edge_dissolver
    ...
    private Geometry bufferPolygon_() {
        ...
        OperatorSimplify simplify = (OperatorSimplify) OperatorFactoryLocal.getInstance()
        ...
        if (m_distance < 0) {
            ...
            Polygon buffered_result = bufferPolygonImpl_(poly, 0, poly.getPathCount());
            ...
            GeometryCursor union_cursor = ((OperatorUnion) OperatorFactoryLocal.getInstance()
                    .getOperator(Operator.Type.Union)).execute(cursor, m_spatialReference, m_progress_tracker);
            Geometry result = union_cursor.next();
            if (result != null) {
                return result;
            } else {
    ...
    private Polygon bufferPolygonImpl_(Polygon input_geom, int ipath_begin, int ipath_end) {
            ...
            if (m_distance > 0) {
                if (path_area > 0) {
                    if (isDegeneratePath_(mp_impl, ipath)) {// if a path is
                        ...
                        point.setXY(env2D.getCenter());
                        addCircle_((MultiPathImpl) intermediate_polygon._getImpl(), point);
                    } else {
                        Polyline result_polyline = new Polyline(input_geom.getDescription());
                        ...
                        boolean bConvex = ConvexHull.isPathConvex((Polygon) (m_geometry), ipath, m_progress_tracker);
                        if (bConvex || bufferClosedPath_(m_geometry, ipath, result_mp, true, 1) == 2) {
                            Polygon buffered_path = bufferConvexPath_(input_mp, ipath);
                            intermediate_polygon.add(buffered_path, false);
                        } else {
                            Polygon buffered_path = bufferCleanup_(result_polyline, false);
                            intermediate_polygon.add(buffered_path, false);
                        }
                    }
                } else {
                ...
                if (path_area > 0) {
                    ...
                    if (!result_polyline.isEmpty()) {
                        ...
                        result_mp.addEnvelope(env, false);// add an envelope
                                                          // exterior shell
                        Polygon buffered_path = bufferCleanup_(result_polyline, false);// simplify with winding rule
        ...
        if (m_distance > 0) {
            if (intermediate_polygon.getPathCount() > 1) {
                Polygon cleaned_polygon = bufferCleanup_(intermediate_polygon, false);
    ...
    private Geometry bufferMultiPoint_() {
        assert (m_distance > 0);
        GeometryCursorForMultiPoint mpCursor = new GeometryCursorForMultiPoint(this, (MultiPoint) (m_geometry),
                m_distance, m_spatialReference, m_densify_dist, m_max_vertex_in_complete_circle, m_progress_tracker);
        GeometryCursor c = ((OperatorUnion) OperatorFactoryLocal.getInstance().getOperator(Operator.Type.Union))
                .execute(mpCursor, m_spatialReference, m_progress_tracker);
        return c.next();
    }
    ...
    private Polygon bufferPolylinePath_(Polyline polyline, int ipath, boolean bfilter) {
        ...
        if (b_closed) {
            ...
            ((MultiPathImpl) tmpPoly._getImpl()).addSegmentsFromPath((MultiPathImpl) input_multi_path._getImpl(), ipath,
                    0, input_multi_path.getSegmentCount(ipath), false);
    ...
    private Polygon bufferCleanup_(MultiPath multi_path, boolean simplify_result) {
        ...
        Polygon resultPolygon = (Polygon) (TopologicalOperations.planarSimplify(multi_path, tol, true, !simplify_result,
                m_progress_tracker));
    ...
    private int calcN_() {
        ...
        double r = m_densify_dist * StrictMath.abs(m_abs_distance_reversed);
        double cos_a = 1 - r;
        double N;
        if (cos_a < -1)
            N = minN;
        else
            N = 2.0 * StrictMath.PI / StrictMath.acos(cos_a) + 0.5;
    ...
    private void addJoin_(MultiPathImpl dst, Point2D center, Point2D fromPt, Point2D toPt, boolean bStartPath,
        ...
        double angle_from = StrictMath.atan2(v_1.y, v_1.x);
        ...
        double angle_to = StrictMath.atan2(v_2.y, v_2.x);
        ...
        int index_from = (int) StrictMath.ceil(dindex_from);
        ...
        for (int i = 0,
                j = index_from % m_circle_template.size(); i < count; i++, j = (j + 1) % m_circle_template.size()) {
    ...
    private int bufferClosedPath_(Geometry input_geom, int ipath, MultiPathImpl result_mp, boolean bfilter, int dir) {
        ...
        if (bfilter) {
            ...
            int res_filter = filterPath_(edit_shape, geom, dir, true, m_abs_distance, m_filter_tolerance,
                    m_densify_dist);
            assert (res_filter == 1);
        ...
        Point2D pt_current = new Point2D(), pt_after = new Point2D(), pt_before = new Point2D(),
                pt_left_prev = new Point2D(), pt = new Point2D(), pt1 = new Point2D();
        ...
        for (int index = 0; index < ncount; index++) {
            ...
            if (bDoJoin) {
                m_buffer_commands.add(new BufferCommand(pt_left_prev, pt, pt_current, BufferCommand.Flags.enum_arc,
                        m_buffer_commands.size() + 1, m_buffer_commands.size() - 1));
            } else if (!pt_left_prev.isEqual(pt)) {
                m_buffer_commands.add(new BufferCommand(pt_left_prev, pt_current, m_buffer_commands.size() + 1,
                        m_buffer_commands.size() - 1, "dummy"));
                m_buffer_commands.add(new BufferCommand(pt_current, pt, m_buffer_commands.size() + 1,
                        m_buffer_commands.size() - 1, "dummy"));
            }

            pt1.add(pt_after, v_left);
            m_buffer_commands.add(new BufferCommand(pt, pt1, pt_current, BufferCommand.Flags.enum_line,
                    m_buffer_commands.size() + 1, m_buffer_commands.size() - 1));
    ...
    private static void protectExtremeVertices_(EditShape edit_shape, int protection_index, int geom, int path) {
        ...
        for (int i = 0, n = edit_shape.getPathSize(path), v = edit_shape.getFirstVertex(path); i < n; ++i) {
            ...
            if (vprev != -1) {
                ...
                if (d < -0.99 && StrictMath.abs(v_after.crossProduct(v_before)) < 1e-7) {
    ...
    static private int filterPath_(EditShape edit_shape, int geom, int dir, boolean closed, double abs_distance,
            double filter_tolerance, double densify_distance) {
        int path = edit_shape.getFirstPath(geom);
        ...
        for (int iter = 0; iter < 100; ++iter) {
            ...
            for (int i = 0; i < nvertices && ivert != -1; i++) {
                ...
                for (int filter = 1, n = (int) StrictMath.min(nfilter, nvertices - i); filter < n; filter++) {
                    v = edit_shape.getNextVertex(v, dir);
                    if (filter > 1) {
                        int num = clipFilter_(edit_shape, fixed_vertices_index, ivert, v, dir, abs_distance,
                                densify_distance, nfilter);
                        if (num == -1)
                            break;
    ...
    private static int clipFilter_(EditShape edit_shape, int fixed_vertices_index, int from_vertex, int to_vertex,
            int dir, double abs_distance, double densify_distance, final int max_filter) {
        ...
        double center_to_chord = StrictMath.sqrt(sqr_center_to_chord); // distance
                                                                       // from
                                                                       // circle
                                                                       // center to
                                                                       // the
        ...
        double corrected_chord_to_corner = chord_to_corner - densify_distance_delta;// using slightly smaller than
                                                                                    // needed
                                                                                    // distance let us filter more.
        corner.scaleAdd(StrictMath.max(0.0, corrected_chord_to_corner), chord_midpoint);
        ...
        if (cnt == 3) {
            ...
            if (case1 || case2) {
                ...
                double wsina = StrictMath.abs(p3.crossProduct(vec) / veclen);
                double z = 2 * abs_distance - wsina;
                if (z < 0)
                    return 0;

                double x = wcosa + StrictMath.sqrt(wsina * z);
                if (x > veclen)
                    return 0;
        ...
        while (v_next != to_vertex) {
            ...
            if (prev_location != -1) {
                ...
                if (cur_location == 3 && prev_location != 0 && next_location != 0) {
                    ...
                    if (next_location == 0 && cur_location != 0 || next_location != 0 && cur_location == 0
                            || ((next_location | cur_location) == 3 && next_location != 3 && cur_location != 3)) {
                        // clip
                    }
                }
            }
    ...
    private boolean isDegeneratePath_(MultiPathImpl mp_impl, int ipath) {
        ...
        if (StrictMath.max(env.getWidth(), env.getHeight()) < m_densify_dist * 0.5)
            return true;
    ...
    private Polyline preparePolyline_(Polyline input_geom) {
        ...
        // criterion.
        Polyline generalized_polyline = (Polyline) ((OperatorGeneralize) OperatorFactoryLocal.getInstance()
                .getOperator(Operator.Type.Generalize)).execute(input_geom, m_densify_dist * 0.25, false,
                        m_progress_tracker);

        int path_point_count = 0;
        for (int i = 0, npath = generalized_polyline.getPathCount(); i < npath; i++) {
            path_point_count = StrictMath.max(generalized_polyline.getPathSize(i), path_point_count);
        }
        ...
        if (path_point_count < 32) {
            ...
            // self intersections.
            Polyline simple_polyline = (Polyline) (TopologicalOperations.planarSimplify(generalized_polyline,
                    m_small_tolerance, false, true, m_progress_tracker));
    ...
    private Polygon setStrongSimple_(Polygon poly) {
        ((MultiPathImpl) poly._getImpl()).setIsSimple(MultiVertexGeometryImpl.GeometryXSimple.Strong, m_tolerance,
                false);
