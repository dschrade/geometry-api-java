...
class OperatorSimplifyLocalHelper {
    ...
    private boolean checkDegenerateSegments_(boolean bTestZs) {
        ...
        // Envelope2D env2D;
        boolean bHasZ = multiPathImpl.hasAttribute(VertexDescription.Semantics.Z);
        double ztolerance = !bHasZ ? 0 : InternalUtils.calculateZToleranceFromGeometry(m_sr, multiPathImpl, false);
        while (segIter.nextPath()) {
            while (segIter.hasNextSegment()) {
                ...
                if (bTestZs && bHasZ) {
                    double z0 = seg.getStartAttributeAsDbl(VertexDescription.Semantics.Z, 0);
                    double z1 = seg.getStartAttributeAsDbl(VertexDescription.Semantics.Z, 0);
                    if (StrictMath.abs(z1 - z0) > ztolerance)
                        continue;
                }

                m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.DegenerateSegments,
    ...
    private boolean checkClustering_() {
        MultiVertexGeometryImpl multiVertexImpl = (MultiVertexGeometryImpl) m_geometry._getImpl();

        MultiPathImpl multiPathImpl = null;
        if (Geometry.isMultiPath(m_geometry.getType().value()))
            multiPathImpl = (MultiPathImpl) m_geometry._getImpl();

        boolean get_paths = (m_bPlanarSimplify || m_bOGCRestrictions) && multiPathImpl != null;

        int pointCount = multiVertexImpl.getPointCount();
        m_xy = (AttributeStreamOfDbl) multiVertexImpl.getAttributeStreamRef(VertexDescription.Semantics.POSITION);
        ...
        sorter.sort(m_pairIndices, 0, 2 * pointCount, new IndexSorter(this, get_paths));
        ...
        for (int index = 0, n = pointCount * 2; index < n; index++) {
            ...
            if ((pair & 1) == 0) {// bottom element
                ...
                if (leftneighbour != Treap.nullNode()
                        && !testToleranceDistance_(m_AET.getElement(leftneighbour), xyindex)) {
                    m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.Clustering, xyindex,
                            m_AET.getElement(leftneighbour));
                    return false;
                }
                int rightneighbour = m_AET.getNext(aetNode);
                if (rightneighbour != Treap.nullNode()
                        && !testToleranceDistance_(m_AET.getElement(rightneighbour), xyindex)) {
                    m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.Clustering, xyindex,
                ...
                if (leftneighbour != Treap.nullNode() && rightneighbour != Treap.nullNode()
                        && !testToleranceDistance_(m_AET.getElement(leftneighbour), m_AET.getElement(rightneighbour))) {
                    m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.Clustering,
                            m_AET.getElement(leftneighbour), m_AET.getElement(rightneighbour));
                    return false;
                }
            }
        }
                                               ...
                                               // algorithm.
    {
        ...
        boolean bNonSimple = Cracker.needsCracking(false, editShape, m_toleranceIsSimple, result, m_progressTracker);
        if (bNonSimple) {
                    ...
                    do {
                        while (segIter2.hasNextSegment()) {
                            ...
                            if (res != 0) {
                                ...
                                m_nonSimpleResult = new NonSimpleResult(reason, segIter1.getStartPointIndex(),
    ...
    boolean checkSelfIntersectionsPolylinePlanar_() {
        ...
        for (int index = 1, n = m_pairIndices.size(); index < n; index++) {
            ...
            boolean boundary;
            boolean end_point = (xyindex == path_start) || (xyindex == path_last);
            if (m_bOGCRestrictions)
                ...
                if (m_bOGCRestrictions) {
                    if (!vi.boundary || !vi_prev.boundary) {
                        ...
                        if ((vi.ipath != vi_prev.ipath) || (!vi.end_point && !vi_prev.end_point)) {
                            ...
                            m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.OGCPolylineSelfTangency,
                    ...
                    if (!vi.end_point || !vi_prev.end_point) {
                        ...
                        m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.CrossOver, vi.ivertex,
                                vi_prev.ivertex);
                        return false;// common point not on the boundary
                    }
                }
    ...
    final static class Vertex_info_pg {
        ...
        boolean is_equal(Vertex_info_pg other) {
            return x == other.x && y == other.y && ipath == other.ipath && ivertex == other.ivertex
                    && ipolygon == other.ipolygon;
        }
    };
    ...
    boolean check_self_intersections_polygons_OGC_() {
        ...
        for (int index = 1, n = m_pairIndices.size(); index < n; index++) {
            ...
            m_xy.read(2 * xyindex, pt);
            int ipath = m_paths_for_OGC_tests.get(xyindex);
            Vertex_info_pg vi = new Vertex_info_pg(pt.x, pt.y, ipath, xyindex, ring_to_polygon[ipath]);

            if (vi.x == vi_prev.x && vi.y == vi_prev.y) {
                if (vi.ipath == vi_prev.ipath) {// the ring has self tangency
                    m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.OGCPolygonSelfTangency, vi.ivertex,
                            vi_prev.ivertex);
                    return false;
        ...
        for (int node = graph.getFirstList(); node != -1; node = graph.getNextList(node)) {
            ...
            if (bad_rnode != -1) {
                ...
                m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.OGCDisconnectedInterior, bad_ring_index,
                        -1);
                return false;
            }
        }
    ...
    private int checkValidRingOrientation_() {
        ...
        if (totalArea <= 0) {
            m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.RingOrientation,
        ...
        m_pathOrientations = new AttributeStreamOfInt32(multiPathImpl.getPathCount(), 0);

        m_pathParentage = new AttributeStreamOfInt32(multiPathImpl.getPathCount(), -1);
        ...
        for (int ipath = 0, n = multiPathImpl.getPathCount(); ipath < n; ipath++) {
            ...
            if (area > 0) {
                ...
                m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.RingOrientation, ipath, -1);
                return 0;
            } else {
                // area < 0: this is a hole.
                // We write the parent exterior
                // ring for it (assumed to be first previous exterior ring)
                if (parent_ring < 0 || exteriorArea < StrictMath.abs(area)) {
                    ...
                    m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.RingOrder, ipath, -1);
                    if (m_bOGCRestrictions)
                        return 0;
                }
    ...
    private boolean processBunchForSelfIntersectionTest_(AttributeStreamOfInt32 bunch) {
        ...
        if (listSize > 0) {
            ...
            m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.CrossOver, i1, i2);
            return false;
        }
    ...
    private boolean processBunchForRingOrientationTest_(AttributeStreamOfInt32 bunch) {
        ...
        for (int i = 0, n = bunch.size(); i < n; i++) {
            ...
            // segments in here
            {
                ...
                Edge edge = createEdge_(seg1, xyindex, m_recycledSegIter.getPathIndex(), true);
                int edgeIndex;
            ...
            // segments in here
            {
                ...
                Edge edge = createEdge_(seg2, xyindex, m_recycledSegIter.getPathIndex(), false);
                int edgeIndex;
                if (m_FreeEdges.size() > 0) {
                    edgeIndex = m_FreeEdges.getLast();
                    m_FreeEdges.removeLast();
        ...
        for (int i = 0, n = m_newEdges.size(); i < n && m_unknownOrientationPathCount > 0; i++) {
                ...
                do {
                    ...
                    if ((direction & 3) == 0) {
                        if (oddEven != edge.getRightSide()) {
                            m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.RingOrientation, pathIndex,
                                    -1);
                        ...
                        if (dir == 2 && m_nonSimpleResult.m_reason == NonSimpleResult.Reason.NotDetermined) {
                            // check that this hole has a correct parent
                            // exterior ring.
                            int parent = m_pathParentage.read(pathIndex);
                            if (parent != prevExteriorPath) {
                                m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.RingOrder, pathIndex,
                                        -1);
                                if (m_bOGCRestrictions)
                                    return false;
                            }
    ...
    private Edge createEdge_(/* const */Segment seg, int xyindex, int pathIndex, boolean bReversed) {
        Edge edge;
    ...
    private static final class RingOrientationTestComparator extends Treap.Comparator {
    ...
    int multiPointIsSimpleAsFeature_() {
        ...
        for (int i = 1; i < pointCount; i++) {
            if (compareVerticesMultiPoint_(indices.get(i - 1), indices.get(i)) == 0) {
                m_nonSimpleResult = new NonSimpleResult(NonSimpleResult.Reason.Clustering, indices.get(i - 1),
                        indices.get(i));
    ...
    MultiPoint multiPointSimplifyAsFeature_() {
        ...
        ((MultiVertexGeometryImpl) dst._getImpl()).setIsSimple(GeometryXSimple.Strong, m_toleranceSimplify, false);
        return dst;
    }
    ...
    Polyline polylineSimplifyAsFeature_() {
        ...
        // Envelope2D env2D;
        boolean bHasZ = multiPathImpl.hasAttribute(VertexDescription.Semantics.Z);
        double ztolerance = !bHasZ ? 0.0 : InternalUtils.calculateZToleranceFromGeometry(m_sr, multiPathImpl, true);
        ...
        while (segIterFwd.nextPath()) {
            ...
            while (segIterFwd.hasNextSegment()) {
                ...
                if (lengthFwd > m_toleranceSimplify) {
                    ...
                    if (bHasZ) {
                        double z0 = multiPathImpl.getAttributeAsDbl(VertexDescription.Semantics.Z, fwdStack.getLast(),
                                0);
                        double z1 = segFwd.getEndAttributeAsDbl(VertexDescription.Semantics.Z, 0);
                        if (StrictMath.abs(z1 - z0) > ztolerance) {
                            fwdStack.add(segIterFwd.getEndPointIndex());
                            lengthFwd = 0;
                        }
                ...
                if (lengthBwd > m_toleranceSimplify) {
                    ...
                    if (bHasZ) {
                        double z0 = multiPathImpl.getAttributeAsDbl(VertexDescription.Semantics.Z, bwdStack.getLast(),
                                0);
                        double z1 = segBwd.getEndAttributeAsDbl(VertexDescription.Semantics.Z, 0);
                        if (StrictMath.abs(z1 - z0) > ztolerance) {
                            bwdStack.add(segIterBwd.getStartPointIndex());
                            lengthBwd = 0;
                        }
    ...
    MultiVertexGeometry simplifyPlanar_() {
        ...
        if (m_geometry.getType() == Geometry.Type.Polygon) {
            if (((Polygon) m_geometry).getFillRule() == Polygon.FillRule.enumFillRuleWinding) {
                ...
                // method.
                return TopologicalOperations.planarSimplify((MultiVertexGeometry) m_geometry, m_toleranceSimplify, true,
                        false, m_progressTracker);
            }
        }
        ...
        if (m_editShape.getTotalPointCount() != 0) {
            ...
            if (m_knownSimpleResult != GeometryXSimple.Weak) {
                CrackAndCluster.execute(m_editShape, m_toleranceSimplify, m_progressTracker, true);
            }

            if (m_geometry.getType().equals(Geometry.Type.Polygon)) {
                Simplificator.execute(m_editShape, m_editShape.getFirstGeometry(), m_knownSimpleResult, false,
                        m_progressTracker);
            }
        }
        ...
        ((MultiVertexGeometryImpl) m_geometry._getImpl()).setIsSimple(GeometryXSimple.Strong, m_toleranceSimplify,
                false);
    ...
    OperatorSimplifyLocalHelper(Geometry geometry, SpatialReference spatialReference, int knownSimpleResult,
        ...
        m_dbgCounter = 0;
        m_toleranceIsSimple = InternalUtils.calculateToleranceFromGeometry(m_sr, geometry, false);
        m_toleranceSimplify = InternalUtils.calculateToleranceFromGeometry(m_sr, geometry, true);
        // m_toleranceCluster = m_toleranceSimplify * StrictMath.sqrt(2.0) * 1.00001;
     ...
     * MultiPoint-check for clustering. Polyline -check for clustering and cracking.
     * Polygon -check for clustering,cracking,absence of self-intersections,and
    ...
    static protected int isSimplePlanar(/* const */Geometry geometry, /* const */
        ...
        if (gt == Geometry.Type.Point)
            ...
            boolean bReturnValue = !env2D
                    .isDegenerate(InternalUtils.calculateToleranceFromGeometry(spatialReference, geometry, false));
        ...
        OperatorSimplifyLocalHelper helper = new OperatorSimplifyLocalHelper(geometry, spatialReference,
                knownSimpleResult, progressTracker, false);
    ...
    static int isSimpleOGC(/* const */Geometry geometry, /* const */
        ...
        OperatorSimplifyLocalHelper helper = new OperatorSimplifyLocalHelper(geometry, spatialReference,
                knownSimpleResult, progressTracker, true);
     ...
     * clusters using cluster tolerance and resolves all self intersections, orients
     * rings properly and arranges the rings in the OGC order.
     * 
    ...
    static protected Geometry simplifyAsFeature(/* const */Geometry geometry, /* const */
        ...
        if (knownSimpleResult == GeometryXSimple.Strong) {
            if (gt == Geometry.Type.Polygon
                    && ((Polygon) geometry).getFillRule() != Polygon.FillRule.enumFillRuleOddEven) {
    ...
    static Geometry simplifyOGC(/* const */Geometry geometry, /* const */
        ...
        MultiVertexGeometry result = TopologicalOperations.simplifyOGC((MultiVertexGeometry) geometry, tolerance, false,
                progressTracker);
    ...
    private static final class EdgeComparerForSelfIntersection implements Comparator<Edge> {
        OperatorSimplifyLocalHelper parent;
    ...
    private static final class MultiPointVertexComparer extends AttributeStreamOfInt32.IntComparator {
        OperatorSimplifyLocalHelper parent;
    ...
    private static final class MultiPointVertexComparer2 extends AttributeStreamOfInt32.IntComparator {
        OperatorSimplifyLocalHelper parent;
    ...
    private int edgeAngleCompare_(/* const */Edge edge1, /* const */Edge edge2) {
        ...
        if (q2 == q1) {
            ...
            double crossError = 4 * NumberUtils.doubleEps()
                    * (StrictMath.abs(v2.x * v1.y) + StrictMath.abs(v2.y * v1.x));
            if (StrictMath.abs(cross) <= crossError) {
                cross--; // To avoid warning of "this line has no effect" from
                         // cross = cross.
                cross++;
            }
            assert (StrictMath.abs(cross) > crossError);
