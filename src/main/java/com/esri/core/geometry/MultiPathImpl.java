...
final class MultiPathImpl extends MultiVertexGeometryImpl {
    ...
    public long estimateMemorySize() {
        long size = SIZE_OF_MULTI_PATH_IMPL + +(m_envelope != null ? m_envelope.estimateMemorySize() : 0)
    ...
    protected void _beforeNewSegment(int resizeBy) {
        ...
        if (m_bPathStarted) {
            ...
            if (m_paths == null) {
                m_paths = (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(2);
                m_paths.write(0, 0);
                m_pathFlags = (AttributeStreamOfInt8) AttributeStreamBase.createByteStream(2, (byte) 0);
            } else {
    ...
    protected void _initSegmentData(int sz) {
        if (m_segmentParamIndex == null) {
            m_segmentFlags = (AttributeStreamOfInt8) AttributeStreamBase.createByteStream(m_pointCount,
                    (byte) SegmentFlags.enumLineSeg);
            m_segmentParamIndex = (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(m_pointCount, -1);
        }

        int size = m_curveParamwritePoint + sz;
        if (m_segmentParams == null) {
            m_segmentParams = (AttributeStreamOfDbl) AttributeStreamBase
                    .createAttributeStreamWithPersistence(VertexDescription.Persistence.enumDouble, size);
        } else {
    ...
    protected void _finishBezierTo() {
        ...
        m_segmentFlags.write(m_pointCount - 2, (byte) SegmentFlags.enumBezierSeg);
    }

    // Reviewed vs. Native Jan 11, 2011
    /**
     * adds a Cubic Bezier Segment to the current Path. The Bezier Segment connects
     * the current last Point and the given endPoint.
     */
    public void bezierTo(Point2D controlPoint1, Point2D controlPoint2, Point2D endPoint) {
        ...
        double z;
        _initSegmentData(6);
        m_pathFlags.setBits(m_pathFlags.size() - 1, (byte) PathFlags.enumHasNonlinearSegments);
    ...
    public void openPathAndDuplicateStartVertex(int pathIndex) {
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
                                                  ...
                                                  // (m_vertexAttributes[iattr])
            {
                ...
                m_vertexAttributes[iattr].insertRange(comp * pathIndexEnd, m_vertexAttributes[iattr],
                        comp * pathIndexStart, comp, true, 1, comp * oldPointCount);
            }
        }
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            if (m_vertexAttributes[iattr] != null) {
                ...
                for (int i = m_pointCount - 1; i >= 0; i--) {
                    if (i + 1 == m_paths.read(ipath)) {
                        ...
                        if (m_pathFlags.read(ipath) == (byte) PathFlags.enumClosed) {
                            ...
                            for (int c = 0; c < comp; c++) {
                                ...
                                m_vertexAttributes[iattr].writeAsDbl(comp * (offset + i) + c, v);
                            }
                    ...
                    for (int c = 0; c < comp; c++) {
                        double v = m_vertexAttributes[iattr].readAsDbl(comp * i + c);
                        m_vertexAttributes[iattr].writeAsDbl(comp * (offset + i) + c, v);
                    }
                }
            }
    ...
    // Reviewed vs. Native Jan 11, 2011
    /**
     * Closes all open curves by adding an implicit line segment from the end point
     * to the start point.
     */
    ...
    public void closePathWithBezier(Point2D controlPoint1, Point2D controlPoint2) {
        ...
        m_pathFlags.write(pathIndex, (byte) (pf | PathFlags.enumClosed | PathFlags.enumHasNonlinearSegments));
        _initSegmentData(6);

        byte oldType = m_segmentFlags.read((byte) ((m_pointCount - 1) & SegmentFlags.enumSegmentMask));
        m_segmentFlags.write(m_pointCount - 1, (byte) (SegmentFlags.enumBezierSeg));
    ...
    public void addSegmentsFromPath(MultiPathImpl src, int src_path_index, int src_segment_from, int src_segment_count,
        ...
        if (src_path_index >= src.getPathCount() || src_segment_from < 0 || src_segment_count < 0
        ...
        boolean bIncludesClosingSegment = src.isClosedPath(src_path_index)
                && src_segment_from + src_segment_count == src.getSegmentCount(src_path_index);
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            ...
            if (isrcAttr < 0 || src.m_vertexAttributes[isrcAttr] == null) {// The
                ...
                m_vertexAttributes[iattr].insertRange(comp * oldPointCount, v, src_point_count * comp,
                        comp * oldPointCount);
                continue;
            }
            ...
            m_vertexAttributes[iattr].insertRange(comp * oldPointCount, src.m_vertexAttributes[isrcAttr],
                    comp * srcFromPoint, src_point_count * comp, b_forward, comp, comp * oldPointCount);
        }
    ...
    public void reversePath(int pathIndex) {
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            if (m_vertexAttributes[iattr] != null) {
                int semantics = m_description._getSemanticsImpl(iattr);
                int comp = VertexDescription.getComponentCount(semantics);
                m_vertexAttributes[iattr].reverseRange(comp * (reversedPathStart + offset),
                        comp * (reversedPathSize - offset), comp);
            }
        }
    ...
    public void removePath(int pathIndex) {
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            if (m_vertexAttributes[iattr] != null) {
                int semantics = m_description._getSemanticsImpl(iattr);
                int comp = VertexDescription.getComponentCount(semantics);
                m_vertexAttributes[iattr].eraseRange(comp * removedPathStart, comp * removedPathSize,
                        comp * m_pointCount);
            }
        }
    ...
    public void insertPath(int pathIndex, MultiPathImpl src, int srcPathIndex, boolean bForward) {
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            ...
            if (isrcAttr >= 0 && src.m_vertexAttributes[isrcAttr] != null) {
                if (offset != 0)
                    m_vertexAttributes[iattr].insertRange(pathIndexStart * comp, src.m_vertexAttributes[isrcAttr],
                            comp * srcPathIndexStart, comp, true, comp, comp * oldPointCount);
                m_vertexAttributes[iattr].insertRange((pathIndexStart + offset) * comp,
                        src.m_vertexAttributes[isrcAttr], comp * (srcPathIndexStart + offset),
                        comp * (srcPathSize - offset), bForward, comp, comp * (oldPointCount + offset));
            } else {
                // Need to make room for the attributes, so we copy default
                // values in

                double v = VertexDescription.getDefaultValue(semantics);
                m_vertexAttributes[iattr].insertRange(pathIndexStart * comp, v, comp * srcPathSize,
                        comp * oldPointCount);
            }
        }
        ...
        if (points != null) {
            ...
            for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
                ...
                if (semantics == VertexDescription.Semantics.POSITION) {
                    ...
                    m_vertexAttributes[iattr].writeRange(2 * (pathStart + count), 2 * (oldPointCount - pathIndex),
                            m_vertexAttributes[iattr], 2 * pathStart, true, 2);

                    AttributeStreamOfDbl position = (AttributeStreamOfDbl) (AttributeStreamBase) getAttributeStreamRef(
                            semantics);
    ...
    public void insertPoints(int pathIndex, int beforePointIndex, MultiPathImpl src, int srcPathIndex,
        ...
        if (pathIndex > getPathCount() || beforePointIndex >= 0 && beforePointIndex > getPathSize(pathIndex)
                || srcPathIndex >= src.getPathCount() || srcPointCount > src.getPathSize(srcPathIndex))
            throw new GeometryException("index out of bounds");
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            ...
            {
                double v = VertexDescription.getDefaultValue(semantics);
                m_vertexAttributes[iattr].insertRange(comp * absoluteIndex, v, srcAbsoluteIndex * comp,
                        comp * oldPointCount);
                continue;
            }
            ...
            m_vertexAttributes[iattr].insertRange(comp * (pathStart + beforePointIndex),
                    src.m_vertexAttributes[isrcAttr], comp * (srcPathStart + srcPointIndexFrom), srcPointCount * comp,
                    bForward, comp, comp * oldPointCount);
        }

        if (hasNonLinearSegments()) {// TODO: probably a bug here when a new
                                     // path is added.
            m_segmentFlags.writeRange((getPathStart(pathIndex) + beforePointIndex + srcPointCount),
                    (oldPointCount - getPathStart(pathIndex) - beforePointIndex), m_segmentFlags,
                    (getPathStart(pathIndex) + beforePointIndex), true, 1);
            m_segmentParamIndex.writeRange((getPathStart(pathIndex) + beforePointIndex + srcPointCount),
                    (oldPointCount - getPathStart(pathIndex) - beforePointIndex), m_segmentParamIndex,
                    (getPathStart(pathIndex) + beforePointIndex), true, 1);
            for (int i = getPathStart(pathIndex) + beforePointIndex,
                    n = getPathStart(pathIndex) + beforePointIndex + srcPointCount; i < n; i++) {
    ...
    public void insertPoints(int pathIndex, int beforePointIndex, Point2D[] src, int srcPointIndexFrom,
            int srcPointCount, boolean bForward) {
        if (pathIndex < 0)
            pathIndex = getPathCount();

        if (pathIndex > getPathCount() || beforePointIndex > getPathSize(pathIndex) || srcPointIndexFrom < 0
                || srcPointCount > src.length)
            throw new GeometryException("index out of bounds");
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            ...
            int comp = VertexDescription.getComponentCount(semantics);
            // copy range to make place for new vertices
            m_vertexAttributes[iattr].writeRange(comp * (getPathStart(pathIndex) + beforePointIndex + srcPointCount),
                    (oldPointCount - getPathStart(pathIndex) - beforePointIndex) * comp, m_vertexAttributes[iattr],
                    comp * (getPathStart(pathIndex) + beforePointIndex), true, comp);

            if (iattr == 0) {
                // add vertices to the given stream
                ((AttributeStreamOfDbl) (AttributeStreamBase) m_vertexAttributes[iattr]).writeRange(
                        comp * (getPathStart(pathIndex) + beforePointIndex), srcPointCount, src, srcPointIndexFrom,
                        bForward);
            } else {
                double v = VertexDescription.getDefaultValue(semantics);
                m_vertexAttributes[iattr].setRange(v, (getPathStart(pathIndex) + beforePointIndex) * comp,
                        srcPointCount * comp);
            }
        }
        ...
        if (hasNonLinearSegments()) {
            ...
            m_segmentParamIndex.writeRange((getPathStart(pathIndex) + beforePointIndex + srcPointCount),
                    (oldPointCount - getPathStart(pathIndex) - beforePointIndex), m_segmentParamIndex,
                    (getPathStart(pathIndex) + beforePointIndex), true, 1);
            m_segmentFlags.setRange((byte) SegmentFlags.enumLineSeg, getPathStart(pathIndex) + beforePointIndex,
                    srcPointCount);
    ...
    public void insertPoint(int pathIndex, int beforePointIndex, Point2D pt) {
        ...
        ((AttributeStreamOfDbl) (AttributeStreamBase) m_vertexAttributes[0]).insert(2 * (pathStart + beforePointIndex),
                pt, 2 * oldPointCount);
    ...
    public void insertPoint(int pathIndex, int beforePointIndex, Point pt) {
        ...
        for (int iattr = 0, nattr = m_description.getAttributeCount(); iattr < nattr; iattr++) {
            ...
            if (pt.hasAttribute(semantics)) {
                m_vertexAttributes[iattr].insertAttributes(comp * (pathStart + beforePointIndex), pt, semantics,
                        comp * oldPointCount);
            } else {
                // Need to make room for the attribute, so we copy a default
                // value in
                double v = VertexDescription.getDefaultValue(semantics);
                m_vertexAttributes[iattr].insertRange(comp * (pathStart + beforePointIndex), v, comp,
                        comp * oldPointCount);
            }
        }
    ...
    void interpolateAttributes(int from_path_index, int from_point_index, int to_path_index, int to_point_index) {
        ...
        for (int iattr = 1; iattr < nattr; iattr++) {
            ...
            for (int ordinate = 0; ordinate < components; ordinate++)
                interpolateAttributes_(semantics, from_path_index, from_point_index, to_path_index, to_point_index,
                        sub_length, ordinate);
        }
    }

    // TODO: Add code for interpolation type (none and angular)
    void interpolateAttributesForSemantics(int semantics, int from_path_index, int from_point_index, int to_path_index,
            int to_point_index) {
        if (semantics == VertexDescription.Semantics.POSITION)
            return;
    ...
    // TODO: Add code fore interpolation type (none and angular)
    void interpolateAttributes_(int semantics, int from_path_index, int from_point_index, int to_path_index,
            int to_point_index, double sub_length, int ordinate) {
        SegmentIteratorImpl seg_iter = querySegmentIterator();
    ...
    void interpolateAttributes_(int semantics, int path_index, int from_point_index, int to_point_index,
    ...
    protected void _verifyStreamsImpl() {
        if (m_paths == null) {
            m_paths = (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(1, 0);
            m_pathFlags = (AttributeStreamOfInt8) AttributeStreamBase.createByteStream(1, (byte) 0);
        }

        if (m_segmentFlags != null) {
            m_segmentFlags.resize(m_reservedPointCount, (byte) SegmentFlags.enumLineSeg);
    ...
    void _copyToImpl(MultiVertexGeometryImpl dst) {
        ...
        if (m_segmentParamIndex != null)
            dstPoly.m_segmentParamIndex = new AttributeStreamOfInt32(m_segmentParamIndex);
        else
    ...
    /**
     * Returns a SegmentIterator that set to a specific vertex of the MultiPathImpl.
     * The call to NextSegment will return the segment that starts at the vertex.
     * Call to PreviousSegment will return the segment that starts at the previous
     * vertex.
     */
    ...
    int getHighestPointIndex(int path_index) {
        ...
        AttributeStreamOfDbl position = (AttributeStreamOfDbl) (getAttributeStreamRef(
    ...
    /**
     * Returns a reference to the AttributeStream of MultiPathImpl parts (Paths).
     * 
     * For the non empty MultiPathImpl, that stream contains start points of the
     * MultiPathImpl curves. In addition, the last element is the total point count.
     * The number of vertices in a given part is parts[i + 1] - parts[i].
     */
     ...
     * 
     * Segment flags indicate what kind of segment originates (starts) on the given
     * point. The last vertices of open Path parts has enumNone flag.
     */
    ...
    /**
     * Returns a reference to the AttributeStream of Path flags (PathFlags flags).
     * 
     * Each start point of a path has a flag set to indicate if the Path is open or
     * closed.
     */
    ...
    @Override
    public boolean _buildRasterizedGeometryAccelerator(double toleranceXY, GeometryAccelerationDegree accelDegree) {
    ...
    void queryPathEnvelope2D(int path_index, Envelope2D envelope) {
        ...
        if (hasNonLinearSegments(path_index)) {
            ...
            AttributeStreamOfDbl stream = (AttributeStreamOfDbl) getAttributeStreamRef(
                    VertexDescription.Semantics.POSITION);
            Point2D pt = new Point2D();
