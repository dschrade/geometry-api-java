...
public final class Transformation2D {
    ...
    void transform(Point2D[] points, int start, int count) {
        int n = StrictMath.min(points.length, start + count);
    ...
    /**
     * Transforms an array of points stored in an array of doubles as interleaved XY
     * coordinates.
     * 
     ...
     *            transformed.
     * @param start
     *            The start point index to transform from (the actual element index
     *            is 2 * start).
     * @param count
     *            The number of points to transform (the actual element count is 2 *
     *            count).
     */
    public void transform(double[] pointsXYInterleaved, int start, int count) {
        int n = StrictMath.min(pointsXYInterleaved.length, (start + count) * 2) / 2;
        for (int i = count; i < n; i++) {
    ...
    /**
     * Multiplies this matrix on the left with the "left" matrix. Stores the result
     * into this matrix and returns a reference to it. <br>
    ...
    /**
     * Performs multiplication of matrices a and b and places the result into this
     * matrix. The a, b, and result could point to same objects. <br>
    ...
    /**
     * Writes the matrix coefficients in the order XX, XY, XD, YX, YY, YD into the
     * given array.
     * 
     * @param coefs
     ...
     * Tangent vector must have unity length
     */
     ...
     * Transforms size.
     * 
     * Creates an AABB with width of SizeSrc.x and height of SizeSrc.y. Transforms
     * that AABB and gets a quadrangle in new coordinate system. The result x
     * contains the length of the quadrangle edge, which were parallel to X in the
     * original system, and y contains the length of the edge, that were parallel to
     * the Y axis in the original system.
     */
    Point2D transformSize(Point2D SizeSrc) {
        Point2D pt = new Point2D();
        pt.x = StrictMath.sqrt(xx * xx + yx * yx) * SizeSrc.x;
        pt.y = StrictMath.sqrt(xy * xy + yy * yy) * SizeSrc.y;
        return pt;
    }
    ...
    public double transform(double tolerance) {
        ...
        /*
         * pt[0].Set(0, 0); pt[1].Set(1, 0); pt[2].Set(0, 1); Transform(pt); pt[1] -=
         * pt[0]; pt[2] -= pt[0];
         */
        ...
        return tolerance * ((d1 > d2) ? StrictMath.sqrt(d1) : StrictMath.sqrt(d2));
    }
    ...
    /**
     * Returns TRUE for reflective transformations. It inverts the sign of vector
     * cross product.
     */
     ...
     * 
     * The uniform transformation is a transformation, which transforms a square to
     * a square.
     */
    public boolean isUniform(double eps) {
        ...
        double e = (v1 + v2) * eps;
        return StrictMath.abs(v1 - v2) <= e && StrictMath.abs(xx * xy + yx * yy) <= e;
    }
     ...
     * tolerance. The orthonormal: Rotation or rotoinversion and shift (preserves
    ...
    /**
     * Returns TRUE if this matrix is degenerated (does not have an inverse) within
     * the given tolerance.
     * 
     * @param tol
     *            The tolerance value.
    ...
    public boolean isDegenerate(double tol) {
        return StrictMath.abs(xx * yy - yx * xy) <= 2 * tol * (StrictMath.abs(xx * yy) + StrictMath.abs(yx * xy));
    }

    /**
     * Returns TRUE, if this transformation does not have rotation and shear within
     * the given tolerance.
     * 
     * @param tol
     *            The tolerance value.
     ...
     * 
     * When the axis Y is directed up and X is directed to the right, the positive
     * angle corresponds to the anti-clockwise rotation. When the axis Y is directed
     * down and X is directed to the right, the positive angle corresponds to the
     * clockwise rotation.
     * 
    ...
    public void setRotate(double angle_in_Radians) {
        setRotate(StrictMath.cos(angle_in_Radians), StrictMath.sin(angle_in_Radians));
    }

    /**
     * Produces a transformation that swaps x and y coordinate values. xx = 0.0; xy
     * = 1.0; xd = 0; yx = 1.0; yy = 0.0; yd = 0;
     */
    ...
    void setRotate(double angle_in_Radians, Point2D rotationCenter) {
        setRotate(StrictMath.cos(angle_in_Radians), StrictMath.sin(angle_in_Radians), rotationCenter);
    }
    ...
    public void extractScaleTransform(Transformation2D scale, Transformation2D rotateNshearNshift) {
        ...
        scale.setScale(StrictMath.sqrt(xx * xx + xy * xy), StrictMath.sqrt(yx * yx + yy * yy));
