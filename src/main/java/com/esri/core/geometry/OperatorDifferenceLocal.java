...
class OperatorDifferenceLocal extends OperatorDifference {
    ...
    @Override
    public GeometryCursor execute(GeometryCursor inputGeometries, GeometryCursor subtractor, SpatialReference sr,
            ProgressTracker progressTracker) {
        return new OperatorDifferenceCursor(inputGeometries, subtractor, sr, progressTracker);
    }

    @Override
    public Geometry execute(Geometry inputGeometry, Geometry subtractor, SpatialReference sr,
            ProgressTracker progressTracker) {
        ...
        GeometryCursor geometryCursor = execute(inputGeomCurs, subractorCurs, sr, progressTracker);

        return geometryCursor.next();
    }

    static Geometry difference(Geometry geometry_a, Geometry geometry_b, SpatialReference spatial_reference,
        ...
        double tolerance_cluster = tolerance * StrictMath.sqrt(2.0) * 1.00001;
        ...
        if (type_a == Geometry.GeometryType.Point) {
            ...
            case Geometry.GeometryType.Polygon:
                return pointMinusPolygon_((Point) (geometry_a), (Polygon) (geometry_b_), tolerance, progress_tracker);
            case Geometry.GeometryType.Polyline:
                return pointMinusPolyline_((Point) (geometry_a), (Polyline) (geometry_b_), tolerance, progress_tracker);
            case Geometry.GeometryType.MultiPoint:
                return pointMinusMultiPoint_((Point) (geometry_a), (MultiPoint) (geometry_b_), tolerance,
                        progress_tracker);
            case Geometry.GeometryType.Envelope:
                return pointMinusEnvelope_((Point) (geometry_a), (Envelope) (geometry_b_), tolerance, progress_tracker);
            case Geometry.GeometryType.Point:
                return pointMinusPoint_((Point) (geometry_a), (Point) (geometry_b_), tolerance, progress_tracker);
            default:
            ...
            switch (type_b) {
            case Geometry.GeometryType.Polygon:
                return multiPointMinusPolygon_((MultiPoint) (geometry_a), (Polygon) (geometry_b), tolerance,
                        progress_tracker);
            case Geometry.GeometryType.Envelope:
                return multiPointMinusEnvelope_((MultiPoint) (geometry_a), (Envelope) (geometry_b), tolerance,
                        progress_tracker);
            case Geometry.GeometryType.Point:
                return multiPointMinusPoint_((MultiPoint) (geometry_a), (Point) (geometry_b), tolerance,
                        progress_tracker);
            default:
                break;
            }
    ...
    static Geometry pointMinusPolygon_(Point point, Polygon polygon, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    static Geometry pointMinusPolyline_(Point point, Polyline polyline, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    static Geometry pointMinusMultiPoint_(Point point, MultiPoint multi_point, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    static Geometry pointMinusEnvelope_(Point point, Envelope envelope, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env = new Envelope2D();
    ...
    static Geometry multiPointMinusPolygon_(MultiPoint multi_point, Polygon polygon, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env = new Envelope2D();
        ...
        for (int i = 0; i < point_count; i++) {
            ...
            PolygonUtils.PiPResult result = PolygonUtils.isPointInPolygon2D(polygon, pt, tolerance);
    ...
    static Geometry multiPointMinusEnvelope_(MultiPoint multi_point, Envelope envelope, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env = new Envelope2D();
        envelope.queryEnvelope2D(env);
        env.inflate(tolerance, tolerance);
    ...
    static Geometry multiPointMinusPoint_(MultiPoint multi_point, Point point, double tolerance,
            ProgressTracker progress_tracker) {
        MultiPointImpl multipointImpl = (MultiPointImpl) (multi_point._getImpl());
        AttributeStreamOfDbl position = (AttributeStreamOfDbl) (multipointImpl
                .getAttributeStreamRef(VertexDescription.Semantics.POSITION));
