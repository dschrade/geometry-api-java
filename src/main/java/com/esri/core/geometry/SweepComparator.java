...
class SweepComparator extends Treap.Comparator {
    ...
    SimpleEdge tryCreateCachedEdge_(int value) {
        ...
        if (se == null) {
            if (m_simple_edges_recycle.isEmpty()) {
                ...
                se = m_simple_edges_buffer.get(m_simple_edges_buffer.size() - 1);
            } else {
                se = m_simple_edges_recycle.get(m_simple_edges_recycle.size() - 1);
                m_simple_edges_recycle.remove(m_simple_edges_recycle.size() - 1);
            }
    ...
    int compareHorizontal1Case1_(Line line_1, Line line_2) {
        ...
        if (line_1.getEndX() > line_2.getEndX()) {
            ...
            // /
            // +------------------
            if (line_2.getEndX() > line_2.getStartX() && line_2.getEndY() - line_2.getStartY() < 2 * m_tolerance
                    && line_1._isIntersectingPoint(line_2.getEndXY(), m_tolerance, true))
                return errorCracking();
        } else {
            // /
            // /
    ...
    int compareHorizontal1Case3_(Line line_1, Line line_2) {
        ...
        double ad1 = StrictMath.abs(d_1);
        double ad2 = StrictMath.abs(d_2);
    ...
    int compareHorizontal2_(Line line_1, Line line_2) {
        if (line_1.getEndY() == line_2.getEndY() && line_1.getEndX() == line_2.getEndX()
                && line_1.getStartY() == line_2.getStartY() && line_1.getStartX() == line_2.getStartX()) {// both lines
                                                                                                          // coincide
            if (m_b_is_simple)
                return errorCoincident();
            return 0;
    ...
    int errorCoincident() {// two segments coincide.
        ...
        m_non_simple_result = new NonSimpleResult(reason, m_vertex_1, m_vertex_2);
        return -1;
    }
    ...
    int compareSegments_(int left, int right, SimpleEdge segLeft, SimpleEdge segRight) {
        ...
        Envelope1D envLeft = segLeft.m_segment.queryInterval(VertexDescription.Semantics.POSITION, 0);
        Envelope1D envRight = segRight.m_segment.queryInterval(VertexDescription.Semantics.POSITION, 0);
        ...
        if (StrictMath.abs(xleft - xright) <= m_tolerance) {
    ...
    SweepComparator(EditShape shape, double tol, boolean bIsSimple) {
        ...
        int s = StrictMath.min(shape.getTotalPointCount() * 3 / 2, (int) (67 /* SIMPLEDGE_CACHESIZE */));
        int cache_size = StrictMath.min((int) 7, s);
