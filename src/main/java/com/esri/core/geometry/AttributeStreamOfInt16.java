...
final class AttributeStreamOfInt16 extends AttributeStreamBase {
    ...
    public void addRange(AttributeStreamBase src, int start, int count, boolean bForward, int stride) {
        ...
        if (bForward) {
            System.arraycopy(((AttributeStreamOfInt16) src).m_buffer, start, m_buffer, oldSize, count);
        } else {
            int n = count;
            ...
            for (int i = 0; i < count; i += stride) {
                ...
                for (int s = 0; s < stride; s++) {
                    m_buffer[oldSize + i + s] = ((AttributeStreamOfInt16) src).m_buffer[start + n + s];
                }
            }
        }
    }
    ...
    public void insertRange(int start, AttributeStreamBase src, int srcStart, int count, boolean bForward, int stride,
        ...
        if (bForward) {
            System.arraycopy(((AttributeStreamOfInt16) src).m_buffer, srcStart, m_buffer, start, count);
        } else {
            int n = count;
            ...
            for (int i = 0; i < count; i += stride) {
                ...
                for (int s = 0; s < stride; s++) {
                    m_buffer[start + i + s] = ((AttributeStreamOfInt16) src).m_buffer[srcStart + n + s];
                }
            }
        }
    }
