...
class OperatorImportFromGeoJsonLocal extends OperatorImportFromGeoJson {
    ...
    public MapGeometry execute(int importFlags, Geometry.Type type, String geoJsonString,
            ProgressTracker progressTracker) throws JsonGeometryException {
        MapGeometry map_geometry = OperatorImportFromGeoJsonHelper.importFromGeoJson(importFlags, type,
                JsonParserReader.createFromString(geoJsonString), progressTracker, false);
        return map_geometry;
    }

    @Override
    public MapGeometry execute(int importFlags, Geometry.Type type, JsonReader jsonReader,
            ProgressTracker progressTracker) throws JsonGeometryException {
        if (jsonReader == null)
            return null;
    ...
    static final class OperatorImportFromGeoJsonHelper {
        ...
        static MapGeometry importFromGeoJson(int importFlags, Geometry.Type type, JsonReader json_iterator,
                ProgressTracker progress_tracker, boolean skip_coordinates) throws JsonGeometryException {
            OperatorImportFromGeoJsonHelper geo_json_helper = new OperatorImportFromGeoJsonHelper();
            MapOGCStructure ms = geo_json_helper.importFromGeoJsonImpl(importFlags, type, json_iterator,
                    progress_tracker, skip_coordinates, 0);
        ...
        static MapOGCStructure importFromGeoJson(int importFlags, Geometry.Type type, JsonReader json_iterator,
                ProgressTracker progress_tracker, boolean skip_coordinates, int recursion)
                throws JsonGeometryException {
            OperatorImportFromGeoJsonHelper geo_json_helper = new OperatorImportFromGeoJsonHelper();
            MapOGCStructure ms = geo_json_helper.importFromGeoJsonImpl(importFlags, type, json_iterator,
                    progress_tracker, skip_coordinates, recursion);
        ...
        MapOGCStructure importFromGeoJsonImpl(int importFlags, Geometry.Type type, JsonReader json_iterator,
                ProgressTracker progress_tracker, boolean skip_coordinates, int recursion)
                throws JsonGeometryException {
                        ...
                        if (current_token == JsonReader.Token.START_ARRAY) {
                            ...
                            while (current_token != JsonReader.Token.END_ARRAY) {
                                MapOGCStructure child = importFromGeoJson(
                                        ...
                                        progress_tracker, false, recursion + 1);
            ...
            if ((!b_geometry_collection && b_geometries_found) || (b_geometry_collection && !b_geometries_found)) {
                throw new JsonGeometryException("parsing error");// found "geometries" but did not see
                                                                 // "GeometryCollection"
            }
        ...
        private double readDouble_(JsonReader json_iterator) throws JsonGeometryException {
            ...
            if (current_token == JsonReader.Token.VALUE_NULL || (current_token == JsonReader.Token.VALUE_STRING
                    && json_iterator.currentString().equals("NaN"))) {
        ...
        private Geometry createGeometry_(GeoJsonType geo_json_type, int type) throws JsonGeometryException {
            ...
            if (type != Geometry.GeometryType.Unknown) {
                ...
                case Geometry.GeometryType.Polygon:
                    if (geo_json_type != GeoJsonType.MultiPolygon && geo_json_type != GeoJsonType.Polygon) {
                        throw new GeometryException("invalid shape type");
                    }
                    break;
                case Geometry.GeometryType.Polyline:
                    if (geo_json_type != GeoJsonType.MultiLineString && geo_json_type != GeoJsonType.LineString) {
                        throw new GeometryException("invalid shape type");
                    }
                    break;
    ...
    public MapOGCStructure executeOGC(int import_flags, String geoJsonString, ProgressTracker progress_tracker)
            throws JsonGeometryException {
        return executeOGC(import_flags, JsonParserReader.createFromString(geoJsonString), progress_tracker);
    }

    public MapOGCStructure executeOGC(int import_flags, JsonReader json_iterator, ProgressTracker progress_tracker)
            throws JsonGeometryException {
        MapOGCStructure mapOGCStructure = OperatorImportFromGeoJsonHelper.importFromGeoJson(import_flags,
                Geometry.Type.Unknown, json_iterator, progress_tracker, false, 0);

        // This is to restore legacy behavior when we always return a geometry
