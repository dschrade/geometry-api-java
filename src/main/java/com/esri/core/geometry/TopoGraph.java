...
final class TopoGraph {
    ...
    static final class PlaneSweepComparator extends Treap.Comparator {
        ...
        int compare(Treap treap, int left, int node) {
            ...
            if (bLeftHorz || bRightHorz) {
                ...
                else if (segLeft.getEndY() == segRight.getEndY() && segLeft.getEndX() == segRight.getEndX())
                    return bLeftHorz ? -1 : 1;
            }
            ...
            if (xLeft == xRight) {
                ...
                double miny = StrictMath.min(yLeft, yRight);
    ...
    static final class ClusterSweepMonikerComparator extends Treap.MonikerComparator {
        TopoGraph m_parent;
    ...
    void updateVertexToHalfEdgeConnectionHelper_(int half_edge, boolean bClear) {
        ...
        if (viter != -1) {
            ...
            for (int viter_ = getHalfEdgeVertexIterator(half_edge); viter_ != -1; viter_ = incrementVertexIterator(
                    viter_)) {
    ...
    void planeSweepParentage_(int inputMode, ProgressTracker progress_tracker) {
        ...
        for (int cluster = getFirstCluster(); cluster != -1; cluster = getNextCluster(cluster)) {
            ...
            if (firstHalfEdge != -1) {
                ...
                if (!tryOptimizedInsertion_(aet, treeNodeIndex, new_edges, cluster, firstHalfEdge))// optimized
                                                                                                   // insertion is for a
                                                                                                   // simple chain, in
                                                                                                   // that case we
                                                                                                   // simply replace an
                ...
                {// This is more complex than a simple chain of edges
                    ...
                    do {// edges that end at the cluster have been assigned an
                        ...
                        if (attachedTreeNode != -1) {
                            ...
                            setHalfEdgeUserIndex(clusterHalfEdge, treeNodeIndex,
                                    StridedIndexTypeCollection.impossibleIndex2());// set it to -2
                        }
    ...
    void planeSweepParentagePropagateParentage_(Treap aet, int treeNode, int inputMode) {
        ...
        if (inputMode == EnumInputMode.enumInputModeBuildGraph) {
            propagate_parentage_build_graph_(aet, treeNode, edge, leftEdge, edgeChain, edgeChainParent,
                    twinHalfEdgeChain);
        } else if (inputMode == EnumInputMode.enumInputModeSimplifyWinding) {
            propagate_parentage_winding_(aet, treeNode, edge, leftEdge, twinEdge, edgeChain, edgeChainParent,
                    twinHalfEdgeChain);
        } else if (inputMode == EnumInputMode.enumInputModeSimplifyAlternate) {
            propagate_parentage_alternate_(aet, treeNode, edge, leftEdge, twinEdge, edgeChain, edgeChainParent,
                    twinHalfEdgeChain);
        }

    }

    void propagate_parentage_build_graph_(Treap aet, int treeNode, int edge, int leftEdge, int edgeChain,
            int edgeChainParent, int twinHalfEdgeChain) {
        ...
        if (leftEdge != -1) {
            ...
            int spikeParentage = chainParentage & twinChainParentage & leftChainParentage; // parentage that needs to
                                                                                           // stay
        ...
        for (int rightNode = aet.getNext(treeNode); rightNode != -1; rightNode = aet.getNext(rightNode)) {
            ...
            int spikeParentage = rightTwinChainParentage & rightChainParentage & chainParentage; // parentage
                                                                                                 // that needs to
                                                                                                 // stay
            chainParentage = chainParentage ^ (chainParentage & rightEdgeParentage);// only
                                                                                    // parentage
                                                                                    // that is
                                                                                    // abscent in
                                                                                    // the twin is
    ...
    void propagate_parentage_winding_(Treap aet, int treeNode, int edge, int leftEdge, int twinEdge, int edgeChain,
            int edgeChainParent, int twinHalfEdgeChain) {
    ...
    void propagate_parentage_alternate_(Treap aet, int treeNode, int edge, int leftEdge, int twinEdge, int edgeChain,
            int edgeChainParent, int twinHalfEdgeChain) {
        // Now do specific sweep calculations
        ...
        if (leftEdge == -1) {
            ...
            assert (getChainParentage(twinHalfEdgeChain) == 0
                    || getChainParentage(twinHalfEdgeChain) == m_universe_geomID);
    ...
    boolean tryOptimizedInsertion_(Treap aet, int treeNodeIndex, AttributeStreamOfInt32 new_edges, int cluster,
            int firstHalfEdge) {
        int clusterHalfEdge = firstHalfEdge;
    ...
    void createHalfEdges_(int inputMode, AttributeStreamOfInt32 sorted_vertices) {
        ...
        for (int i = 0, nvert = sorted_vertices.size(); i < nvert; i++) {
            ...
            if (Geometry.isMultiPath(gt)) {
                ...
                if (inputMode == EnumInputMode.enumInputModeBuildGraph) {
                    setHalfEdgeUserIndex(twinEdge, m_tmpHalfEdgeParentageIndex, 0); // Hole is always to the left. left
                                                                                    // side here is
                                                                                    // the twin.
                    setHalfEdgeUserIndex(half_edge, m_tmpHalfEdgeParentageIndex,
                            gt == Geometry.GeometryType.Polygon ? geometryID : 0);
                } else if (inputMode == EnumInputMode.enumInputModeSimplifyWinding) {
                ...
                int edgeBit = gt == Geometry.GeometryType.Polygon ? c_edgeBitMask : 0;
            ...
            if (first != -1) {
                ...
                if (angleSorter.size() > 1) {
                    ...
                    if (angleSorter.size() > 2) {
                        angleSorter.Sort(0, angleSorter.size(), tgac); // std::sort(angleSorter.get_ptr(),
                                                                       // angleSorter.get_ptr()
                                                                       // +
                                                                       // angleSorter.size(),
                        ...
                        // no need to sort most two edge cases. we only need to make sure that edges
                        // going up are sorted
                        ...
                        {// remove duplicate edge. Before removing, propagate
                         // the parentage to the remaning edge
                            if (inputMode == EnumInputMode.enumInputModeBuildGraph) {
                                ...
                                setHalfEdgeUserIndex(ePrev, m_tmpHalfEdgeParentageIndex,
                                        getHalfEdgeUserIndex(ePrev, m_tmpHalfEdgeParentageIndex)
                                                | getHalfEdgeUserIndex(e, m_tmpHalfEdgeParentageIndex));
                                setHalfEdgeUserIndex(ePrevTwin, m_tmpHalfEdgeParentageIndex,
                                        getHalfEdgeUserIndex(ePrevTwin, m_tmpHalfEdgeParentageIndex)
                                                | getHalfEdgeUserIndex(eTwin, m_tmpHalfEdgeParentageIndex));
                                ...
                                // are added.
                                int newHalfEdgeWinding = getHalfEdgeUserIndex(ePrev, m_tmpHalfEdgeWindingNumberIndex)
                                        + getHalfEdgeUserIndex(e, m_tmpHalfEdgeWindingNumberIndex);
                                int newTwinEdgeWinding = getHalfEdgeUserIndex(ePrevTwin,
                                        m_tmpHalfEdgeWindingNumberIndex)
                                        + getHalfEdgeUserIndex(eTwin, m_tmpHalfEdgeWindingNumberIndex);
                                ...
                                m_non_simple_result = new NonSimpleResult(NonSimpleResult.Reason.CrossOver, cluster,
                                        -1);
                                return;
                            } else if (m_tmpHalfEdgeOddEvenNumberIndex != -1) {
                                int newHalfEdgeWinding = getHalfEdgeUserIndex(ePrev, m_tmpHalfEdgeOddEvenNumberIndex)
                                        + getHalfEdgeUserIndex(e, m_tmpHalfEdgeOddEvenNumberIndex);
                                int newTwinEdgeWinding = getHalfEdgeUserIndex(ePrevTwin,
                                        m_tmpHalfEdgeOddEvenNumberIndex)
                                        + getHalfEdgeUserIndex(eTwin, m_tmpHalfEdgeOddEvenNumberIndex);
    ...
    void buildChains_(int inputMode) {
                    ...
                    {// if we have not visited this halfedge yet, then we have
                     // not created a chain for it yet.
                        ...
                        do {
                            ...
                            assert (getHalfEdgeUserIndex(e, visitedHalfEdgeIndex) != 1);
                        ...
                        assert (inputMode != EnumInputMode.enumInputModeIsSimplePolygon
                                || parentage != (1 | m_universe_geomID));
        ...
        m_chainAreas = new AttributeStreamOfDbl(m_chainData.size(), NumberUtils.TheNaN);
        m_chainPerimeters = new AttributeStreamOfDbl(m_chainData.size(), NumberUtils.TheNaN);
    ...
    void setEditShapeImpl_(EditShape shape, int inputMode, AttributeStreamOfInt32 editShapeGeometries,
        ...
        verticesSorter.reserve(editShapeGeometries != null ? m_shape.getPointCount(editShapeGeometries.get(0))
                : m_shape.getTotalPointCount());
        int path_count = 0;
        int geomID = 1;
        {// scope
        ...
        m_chainData.setCapacity(StrictMath.max((int) 32, path_count));
    ...
    TopoGraph() {
        c_edgeParentageMask = ((int) -1) ^ ((int) 1 << (NumberUtils.sizeOf((int) 0) * 8 - 1));
    ...
    void setEditShape(EditShape shape, ProgressTracker progress_tracker) {
        setEditShapeImpl_(shape, EnumInputMode.enumInputModeBuildGraph, null, progress_tracker, true);
    }

    void setEditShape(EditShape shape, ProgressTracker progress_tracker, boolean bBuildChains) {
        setEditShapeImpl_(shape, EnumInputMode.enumInputModeBuildGraph, null, progress_tracker, bBuildChains);
    }
    ...
    void setAndSimplifyEditShapeAlternate(EditShape shape, int geometry, ProgressTracker progressTracker) {
        ...
        geoms.add(geometry);
        setEditShapeImpl_(shape, EnumInputMode.enumInputModeSimplifyAlternate, geoms, progressTracker,
                shape.getGeometryType(geometry) == Geometry.Type.Polygon.value());
    }

    void setAndSimplifyEditShapeWinding(EditShape shape, int geometry, ProgressTracker progressTracker) {
        AttributeStreamOfInt32 geoms = new AttributeStreamOfInt32(0);
        geoms.add(geometry);
        setEditShapeImpl_(shape, EnumInputMode.enumInputModeSimplifyWinding, geoms, progressTracker, true);
    }
    ...
    int createUserIndexForClusters() {
        ...
        AttributeStreamOfInt32 new_stream = new AttributeStreamOfInt32(m_clusterData.capacity(), -1);
    ...
    int createUserIndexForHalfEdges() {
        ...
        AttributeStreamOfInt32 new_stream = new AttributeStreamOfInt32(m_halfEdgeData.capacity(), -1);
    ...
    int createUserIndexForChains() {
        ...
        AttributeStreamOfInt32 new_stream = new AttributeStreamOfInt32(m_chainData.capacity(), -1);
                ...
                do {
                    ...
                    if (edge != prev) {
                        ...
                        double cross = v_1.crossProduct(v_2); // cross_prod =
                                                              ...
                                                              // / (len1 *
                                                              // len2);
                        double sqr_sinA = (cross * cross) / (sqr_len1 * sqr_len2); // sqr_sinA is
                                                                                   // approximately A^2
                                                                                   // especially for
                                                                                   // smaller angles
                        double sqr_dist = StrictMath.min(sqr_len1, sqr_len2) * sqr_sinA;
                        if (sqr_dist <= sqr_tol) {
