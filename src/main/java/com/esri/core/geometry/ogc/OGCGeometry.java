...
public abstract class OGCGeometry {
    ...
    public int coordinateDimension() {
        int d = 2;
        if (getEsriGeometry().getDescription().hasAttribute(VertexDescription.Semantics.M))
            d++;
        if (getEsriGeometry().getDescription().hasAttribute(VertexDescription.Semantics.Z))
            d++;
    ...
    public ByteBuffer asBinary() {
        OperatorExportToWkb op = (OperatorExportToWkb) OperatorFactoryLocal.getInstance()
                .getOperator(Operator.Type.ExportToWkb);
    ...
    public double MinZ() {
        Envelope1D e = getEsriGeometry().queryInterval(VertexDescription.Semantics.Z, 0);
        return e.vmin;
    }
    ...
    public double MinMeasure() {
        Envelope1D e = getEsriGeometry().queryInterval(VertexDescription.Semantics.M, 0);
        return e.vmin;
    }
    ...
    /**
     * Returns true if this geometric object has no anomalous geometric points, such
     * as self intersection or self tangency. See the "Simple feature access - Part
     * 1" document (OGC 06-103r4) for meaning of "simple" for each geometry type.
     * 
     * The method has O(n log n) complexity when the input geometry is simple. For
     * non-simple geometries, it terminates immediately when the first issue is
     * encountered.
     * 
    ...
    public boolean isSimple() {
        return OperatorSimplifyOGC.local().isSimpleOGC(getEsriGeometry(), esriSR, true, null, null);
    }
     ...
     * 
     *         Note: If isSimpleRelaxed is true, then isSimple is either true or
     *         false. Geodatabase has more relaxed requirements for simple
     *         geometries than OGC.
     */
    ...
    public OGCGeometry makeSimpleRelaxed(boolean forceProcessing) {
        OperatorSimplify op = (OperatorSimplify) OperatorFactoryLocal.getInstance().getOperator(Operator.Type.Simplify);
        return OGCGeometry.createFromEsriGeometry(op.execute(getEsriGeometry(), esriSR, forceProcessing, null), esriSR);
    }

    /**
     * Resolves topological issues in this geometry and makes it Simple according to
     * OGC specification.
     * 
     * @return Returns simplified geometry.
     * 
     *         Note: isSimple and isSimpleRelaxed should return true after this
     *         operation.
     */
    ...
    public boolean is3D() {
        return getEsriGeometry().getDescription().hasAttribute(VertexDescription.Semantics.Z);
    }

    public boolean isMeasured() {
        return getEsriGeometry().getDescription().hasAttribute(VertexDescription.Semantics.M);
    }

    abstract public OGCGeometry boundary();

    /**
     * OGC equals. Performs topological comparison with tolerance. This is different
     * from equals(Object), that uses exact comparison.
     */
    public boolean Equals(OGCGeometry another) {
        ...
        return com.esri.core.geometry.GeometryEngine.equals(geom1, geom2, getEsriSpatialReference());
    }
    ...
    public boolean disjoint(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.disjoint(geom1, geom2, getEsriSpatialReference());
    }
    ...
    public boolean touches(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.touches(geom1, geom2, getEsriSpatialReference());
    }
    ...
    public boolean crosses(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.crosses(geom1, geom2, getEsriSpatialReference());
    }
    ...
    public boolean contains(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.contains(geom1, geom2, getEsriSpatialReference());
    }
    ...
    public boolean overlaps(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.overlaps(geom1, geom2, getEsriSpatialReference());
    }
    ...
    public boolean relate(OGCGeometry another, String matrix) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.relate(geom1, geom2, getEsriSpatialReference(), matrix);
    }
    ...
    public double distance(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom1 = getEsriGeometry();
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return com.esri.core.geometry.GeometryEngine.distance(geom1, geom2, getEsriSpatialReference());
    }
    ...
    private OGCGeometry simplifyBunch_(GeometryCursor gc) {
        ...
        for (com.esri.core.geometry.Geometry g = gc.next(); g != null; g = gc.next()) {
            switch (g.getType()) {
            case Point:
        ...
        if (dstMultiPoint != null) {
            Geometry resMP = OperatorSimplifyOGC.local().execute(dstMultiPoint, esriSR, true, null);
            result.add(resMP);
        }

        if (dstPolylines.size() > 0) {
            if (dstPolylines.size() == 1) {
                Geometry resMP = OperatorSimplifyOGC.local().execute(dstPolylines.get(0), esriSR, true, null);
                result.add(resMP);
            } else {
                GeometryCursor res = OperatorUnion.local().execute(new SimpleGeometryCursor(dstPolylines), esriSR,
                        null);
                Geometry resPolyline = res.next();
                Geometry resMP = OperatorSimplifyOGC.local().execute(resPolyline, esriSR, true, null);
                result.add(resMP);
            }
        }
        ...
        if (dstPolygons.size() > 0) {
            if (dstPolygons.size() == 1) {
                Geometry resMP = OperatorSimplifyOGC.local().execute(dstPolygons.get(0), esriSR, true, null);
                result.add(resMP);
            } else {
                ...
                Geometry resMP = OperatorSimplifyOGC.local().execute(resPolygon, esriSR, true, null);
                result.add(resMP);
            }
        }
    ...
    public OGCGeometry buffer(double distance) {
        ...
        double d[] = {distance};
        com.esri.core.geometry.GeometryCursor cursor = op.execute(getEsriGeometryCursor(), getEsriSpatialReference(), d,
                true, null);
    ...
    public OGCGeometry convexHull() {
        com.esri.core.geometry.GeometryCursor cursor = OperatorConvexHull.local().execute(getEsriGeometryCursor(),
                false, null);
    ...
    public OGCGeometry intersection(OGCGeometry another) {
        ...
        com.esri.core.geometry.OperatorIntersection op = (OperatorIntersection) OperatorFactoryLocal.getInstance()
                .getOperator(Operator.Type.Intersection);
        com.esri.core.geometry.GeometryCursor cursor = op.execute(getEsriGeometryCursor(),
                another.getEsriGeometryCursor(), getEsriSpatialReference(), null, 7);
    ...
    public OGCGeometry union(OGCGeometry another) {
        ...
        if (thisType != anotherType || thisType == OGCConcreteGeometryCollection.TYPE) {
            // heterogeneous union.
            // We make a geometry collection, then process to union parts and remove
            // overlaps.
        ...
        GeometryCursorAppend ap = new GeometryCursorAppend(getEsriGeometryCursor(), another.getEsriGeometryCursor());
        com.esri.core.geometry.GeometryCursor cursor = op.execute(ap, getEsriSpatialReference(), null);
        return OGCGeometry.createFromEsriCursor(cursor, esriSR);
    }
    ...
    public OGCGeometry difference(OGCGeometry another) {
        ...
        return createFromEsriGeometry(
                com.esri.core.geometry.GeometryEngine.difference(geom1, geom2, getEsriSpatialReference()), esriSR);
    }
    ...
    public OGCGeometry symDifference(OGCGeometry another) {
        ...
        com.esri.core.geometry.Geometry geom2 = another.getEsriGeometry();
        return createFromEsriGeometry(
                com.esri.core.geometry.GeometryEngine.symmetricDifference(geom1, geom2, getEsriSpatialReference()),
                esriSR);
    }
    ...
    public static OGCGeometry fromText(String text) {
        OperatorImportFromWkt op = (OperatorImportFromWkt) OperatorFactoryLocal.getInstance()
    ...
    public static OGCGeometry fromBinary(ByteBuffer binary) {
        OperatorImportFromWkb op = (OperatorImportFromWkb) OperatorFactoryLocal.getInstance()
    ...
    public static OGCGeometry fromGeoJson(String string) {
        OperatorImportFromGeoJson op = (OperatorImportFromGeoJson) OperatorFactoryLocal.getInstance()
    ...
    public static OGCGeometry createFromOGCStructure(OGCStructure ogcStructure, SpatialReference sr) {
        ...
        while (!structureStack.isEmpty()) {
            ...
            if (indices.get(indices.size() - 1) == lastStructure.m_structures.size()) {
            ...
            OGCConcreteGeometryCollection lastCollection = collectionStack.get(collectionStack.size() - 1);
            OGCGeometry g;
            ...
            switch (type) {
            case 1:
                g = new OGCPoint((Point) lastStructure.m_structures.get(i).m_geometry, sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
                break;
            case 2:
                g = new OGCLineString((Polyline) lastStructure.m_structures.get(i).m_geometry, 0, sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
                break;
            case 3:
                g = new OGCPolygon((Polygon) lastStructure.m_structures.get(i).m_geometry, 0, sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
                break;
            case 4:
                g = new OGCMultiPoint((MultiPoint) lastStructure.m_structures.get(i).m_geometry, sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
                break;
            case 5:
                g = new OGCMultiLineString((Polyline) lastStructure.m_structures.get(i).m_geometry, sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
                break;
            case 6:
                g = new OGCMultiPolygon((Polygon) lastStructure.m_structures.get(i).m_geometry, sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
                break;
            case 7:
                geometries = new OGCGeometry[lastStructure.m_structures.get(i).m_structures.size()];
                g = new OGCConcreteGeometryCollection(Arrays.asList(geometries), sr);
                lastCollection.geometries.set(i, g);
                indices.set(indices.size() - 1, i + 1);
    ...
    /**
     * For the geometry collection types, when it has 1 or 0 elements, converts a
     * MultiPolygon to Polygon, MultiPoint to Point, MultiLineString to a
     * LineString, and OGCConcretGeometryCollection to the reduced element it
     * contains.
     * 
