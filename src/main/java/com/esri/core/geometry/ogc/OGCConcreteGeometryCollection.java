...
public class OGCConcreteGeometryCollection extends OGCGeometryCollection {
    ...
    public int dimension() {
        ...
        for (int i = 0, n = numGeometries(); i < n; i++)
            maxD = StrictMath.max(geometryN(i).dimension(), maxD);
    ...
    public double MinZ() {
        double z = Double.NaN;
        for (int i = 0, n = numGeometries(); i < n; i++)
            z = i == 0 ? geometryN(i).MinZ() : StrictMath.min(geometryN(i).MinZ(), z);
        return z;
    }
    ...
    public double MaxZ() {
        double z = Double.NaN;
        for (int i = 0, n = numGeometries(); i < n; i++)
            z = i == 0 ? geometryN(i).MaxZ() : StrictMath.min(geometryN(i).MaxZ(), z);
        return z;
    }
    ...
    public double MinMeasure() {
        double z = Double.NaN;
        for (int i = 0, n = numGeometries(); i < n; i++)
            z = i == 0 ? geometryN(i).MinMeasure() : StrictMath.min(geometryN(i).MinMeasure(), z);
        return z;
    }
    ...
    public double MaxMeasure() {
        double z = Double.NaN;
        for (int i = 0, n = numGeometries(); i < n; i++)
            z = i == 0 ? geometryN(i).MaxMeasure() : StrictMath.min(geometryN(i).MaxMeasure(), z);
        return z;
    }
    ...
    public double distance(OGCGeometry another) {
        ...
        for (int i = 0, n = numGeometries(); i < n; ++i) {
            // TODO Skip expensive distance computation if bounding boxes are further away
            // than minD
    ...
    public boolean Equals(OGCGeometry another) {
        ...
        // TODO Assuming input geometries are simple and valid, remove-overlaps would be
        // a no-op.
    ...
    public OGCGeometry intersection(OGCGeometry another) {
        ...
        for (Geometry geometry : geometries) {
            for (Geometry otherGeometry : otherGeometries) {
                GeometryCursor intersectionCursor = OperatorIntersection.local().execute(
                        new SimpleGeometryCursor(geometry), new SimpleGeometryCursor(otherGeometry), esriSR, null, 7);
     ...
     * 
     * @return True for the flattened collection. A flattened collection contains up
     *         to three non-empty geometries: an OGCMultiPoint, an OGCMultiPolygon,
     *         and an OGCMultiLineString.
     */
    ...
    /**
     * Flattens Geometry Collection. The result collection contains up to three
     * geometries: an OGCMultiPoint, an OGCMultilineString, and an OGCMultiPolygon
     * (in that order).
     * 
    ...
    public OGCConcreteGeometryCollection flatten() {
        ...
        if (polygons != null) {
            GeometryCursor unionedPolygons = OperatorUnion.local().execute(new SimpleGeometryCursor(polygons), esriSR,
                    null);
    ...
    /**
     * Fixes topological overlaps in the GeometryCollecion. This is equivalent to
     * union of the geometry collection elements.
     *
     * TODO "flattened" collection is supposed to contain only mutli-geometries, but
     * this method may return single geometries e.g. for GEOMETRYCOLLECTION
     * (LINESTRING (...)) it returns GEOMETRYCOLLECTION (LINESTRING (...)) and not
     * GEOMETRYCOLLECTION (MULTILINESTRING (...))
     * 
     * @return A geometry collection that is flattened and has no overlapping
     *         elements.
     */
    ...
    // Collectively processes group of geometry collections (intersects all segments
    // and clusters points).
    // Flattens collections, removes overlaps.
    // Once done, the result collections would work well for topological and
    // relational operations.
    private GeometryCursor prepare_for_ops_(OGCConcreteGeometryCollection collection) {
        ...
        GeometryCursor prepared = OGCStructureInternal.prepare_for_ops_(collection.flatten().getEsriGeometryCursor(),
                esriSR);
