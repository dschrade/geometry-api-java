...
public abstract class Operator {
    ...
    public enum Type {
        ...
        ExportToJson, ImportFromJson, ExportToESRIShape, ImportFromESRIShape,
     ...
     * copies remain accelerated until modified. The acceleration of Geometry can be
     * a time consuming operation. The accelerated geometry also takes more memory.
     * Some operators share the same accelerator, some require a different one. If
     * the accelerator is built for the given parameters, the method returns
     * immediately.
     * 
     * @param geometry
    ...
    public static void deaccelerateGeometry(Geometry geometry) {
        ...
        if (Geometry.isMultiVertex(gt.value())) {
            GeometryAccelerators accel = ((MultiVertexGeometryImpl) geometry._getImpl())._getAccelerators();
            if (accel != null) {
