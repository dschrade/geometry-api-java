...
/**
 * Simple scanline rasterizer. Caller provides a callback to draw pixels to
 * actual surface.
 *
 */
public class SimpleRasterizer {
    ...
    public interface ScanCallback {
         ...
         * 
         * @param scans
         *            array of scans. Scans are triplets of numbers. The start X
         *            coordinate for the scan (inclusive), the end X coordinate of the
         *            scan (exclusive), the Y coordinate for the scan.
         * @param scanCount3
         *            The number of initialized elements in the scans array. The scan
         *            count is scanCount3 / 3.
         */
     ...
     * startAddingEdges(); addRing(...); renderEdges(Rasterizer.EVEN_ODD);
     * addRing(...); renderEdges(Rasterizer.EVEN_ODD);
     *
     * For example to render a polygon consisting of three rings:
     * startAddingEdges(); addRing(...); addRing(...); addRing(...);
    ...
    /**
     * Renders all edges added so far, and removes them. Calls startAddingEdges
     * after it's done.
     * 
     * @param fillMode
     *            Fill mode for the polygon fill can be one of two values: EVEN_ODD
     *            or WINDING.
     *
     *            Note, as any other graphics algorithm, the scan line rasterizer
     *            doesn't require polygons to be topologically simple, or have
    ...
    public final void addEdge(double x1, double y1, double x2, double y2) {
        ...
        int bigX = StrictMath.max(width_ + 1, 0x7fffff);
        if (x1 < -0x7fffff) {
    ...
    final boolean addSegmentStroke(double x1, double y1, double x2, double y2, double half_width, boolean skip_short,
        ...
        if (veryShort) {
            ...
            double f = half_width / StrictMath.sqrt(sqr_len);
            vec_x *= f;
            vec_y *= f;
        }
    ...
    public long estimateMemorySize() {
        // callback_ is only a pointer, the actual size is accounted for in the caller
        // of setup()
        long size = SIZE_OF_SIMPLE_RASTERIZER + (activeEdgesTable_ != null ? activeEdgesTable_.estimateMemorySize() : 0)
    ...
    static class Edge {
        ...
        long estimateMemorySize() {
            // next is only a pointer, the actual size is accounted for in
            // SimpleRasterizer#estimateMemorySize
            return SIZE_OF_EDGE;
        }
    }
    ...
    private final Edge sortAET_(Edge aet) {
        ...
        if (sortBuffer_ == null)
            sortBuffer_ = new Edge[StrictMath.max(num, 16)];

        else if (sortBuffer_.length < num)
            sortBuffer_ = new Edge[StrictMath.max(num, sortBuffer_.length * 2)];
