...
public final class Line extends Segment implements Serializable {
    ...
    boolean isDegenerate(double tolerance) {
        double dx = m_xStart - m_xEnd;
        double dy = m_yStart - m_yEnd;
        return StrictMath.sqrt(dx * dx + dy * dy) <= tolerance;
    }
    ...
    public void queryEnvelope(Envelope env) {
        ...
        for (int i = 1, n = m_description.getAttributeCount(); i < n; i++) {
            ...
            for (int iord = 0, nord = VertexDescription.getComponentCount(semantics); i < nord; i++) {
    ...
    @Override
    public int intersectionWithAxis2D(boolean b_axis_x, double ordinate, double[] result_ordinates,
            double[] parameters) {
        if (b_axis_x) {
    ...
    // inside clipEnv2D.
    int intersectionWithEnvelope2D(Envelope2D clipEnv2D, boolean includeEnvBoundary, double[] segParams,
    ...
    /**
     * Returns True if point and the segment intersect (not disjoint) for the given
     * tolerance.
     */
    @Override
    ...
    int _side(double ptX, double ptY) {
        ...
        double crossError = 4 * NumberUtils.doubleEps() * (StrictMath.abs(v2.x * v1.y) + StrictMath.abs(v2.y * v1.x));
    ...
    double _intersection(Point2D pt, double tolerance, boolean bExcludeExactEndPoints) {
        ...
        if (vlength <= StrictMath.max(tolerance, vLengthError)) {
        ...
        if (len > 0) {
            ...
            double projectionError = 8 * relativePoint._dotProductAbs(v) * NumberUtils.doubleEps();// See Error
                                                                                                   // Estimation Rules
                                                                                                   // In
                                                                                                   // Borg.docx
            v.leftPerpendicular();// get left normal to v
            double distance = relativePoint.dotProduct(v);
            double distanceError = 8 * relativePoint._dotProductAbs(v) * NumberUtils.doubleEps();// See Error Estimation
                                                                                                 // Rules In
                                                                                                 // Borg.docx

            double perror = StrictMath.max(tolerance, projectionError);
            if (projection < -perror || projection > len + perror)
                return NumberUtils.TheNaN;

            double merror = StrictMath.max(tolerance, distanceError);
            if (StrictMath.abs(distance) <= merror) {
    ...
    boolean _projectionIntersectHelper(Line other, Point2D v, boolean bStart) {
        ...
        if (dot > dotError) {
            ...
            double dotError2 = 3 * NumberUtils.doubleEps() * v._dotProductAbs(m);
    ...
    static Point2D _intersectHelper1(Line line1, Line line2, double tolerance) {
        ...
        double errdet = 4 * NumberUtils.doubleEps() * (StrictMath.abs(k2x * k1y) + StrictMath.abs(k1x * k2y));

        double bx = line2.m_xStart - line1.m_xStart;
        double by = line2.m_yStart - line1.m_yStart;

        double a0 = (k2x * by - bx * k2y);
        double a0error = 4 * NumberUtils.doubleEps() * (StrictMath.abs(k2x * by) + StrictMath.abs(bx * k2y));
        double t0 = a0 / det;
        double absdet = StrictMath.abs(det);
        double t0error = (a0error * absdet + errdet * StrictMath.abs(a0)) / (det * det)
                + NumberUtils.doubleEps() * StrictMath.abs(t0);
        if (t0 < -t0error || t0 > 1.0 + t0error)
            return result;

        double a1 = (k1x * by - bx * k1y);
        double a1error = 4 * NumberUtils.doubleEps() * (StrictMath.abs(k1x * by) + StrictMath.abs(bx * k1y));
        double t1 = a1 / det;
        double t1error = (a1error * absdet + errdet * StrictMath.abs(a1)) / (det * det)
                + NumberUtils.doubleEps() * StrictMath.abs(t1);
    ...
    static int _isIntersectingLineLine(Line line1, Line line2, double tolerance, boolean bExcludeExactEndpoints) {
        ...
        if (line1.m_xStart == line2.m_xStart && line1.m_yStart == line2.m_yStart
                || line1.m_xStart == line2.m_xEnd && line1.m_yStart == line2.m_yEnd) {
            counter++;
    ...
    int _intersectLineLineExact(Line line1, Line line2, Point2D[] intersectionPoints, double[] param1,
            double[] param2) {
        int counter = 0;
        if (line1.m_xStart == line2.m_xStart && line1.m_yStart == line2.m_yStart) {
    ...
    static int _intersectLineLine(Line line1, Line line2, Point2D[] intersectionPoints, double[] param1,
            double[] param2, double tolerance) {
