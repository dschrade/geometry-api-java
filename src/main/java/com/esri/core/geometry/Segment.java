...
public abstract class Segment extends Geometry implements Serializable {
    ...
    /**
     * Returns value of the start vertex attribute's ordinate. Throws if the Point
     * is empty.
     * 
     * @param semantics
    ...
    /**
     * Returns the value of the start vertex attribute's ordinate. The ordinate is
     ...
     *            The attribute semantics.
     * @param value
     *            is the array to write values to. The attribute type and the number
     *            of elements must match the persistence type, as well as the number
     *            of components of the attribute.
     */
    ...
    /**
     * Returns value of the end vertex attribute's ordinate. Throws if the Point is
     * empty.
     * 
     * @param semantics
     *            The attribute semantics.
     ...
     * @param tolerance
     *            The tolerance value for the intersection calculation. Can be 0.
     * @return The number of intersection points, 0 when no intersection points
     *         exist.
     */
    int intersect(Segment other, Point2D[] intersectionPoints, double[] paramThis, double[] paramOther,
            double tolerance) {
        return _intersect(other, intersectionPoints, paramThis, paramOther, tolerance);
    }

    /**
     * Returns TRUE if this segment intersects with the other segment with the given
     * tolerance.
     */
    ...
    /**
     * Returns TRUE if the point and segment intersect (not disjoint) for the given
     * tolerance.
     */
    ...
    private void _get(int endPoint, Point outPoint) {
        ...
        for (int attributeIndex = 0; attributeIndex < m_description.getAttributeCount(); attributeIndex++) {
            ...
            for (int icomp = 0, ncomp = VertexDescription.getComponentCount(semantics); icomp < ncomp; icomp++) {
    ...
    double _getAttributeAsDbl(int endPoint, int semantics, int ordinate) {
        ...
        if (attributeIndex >= 0) {
            ...
            return m_attributes[_getEndPointOffset(m_description, endPoint)
                    + m_description._getPointAttributeOffset(attributeIndex) - 2 + ordinate];
        } else
    ...
    void _setAttribute(int endPoint, int semantics, int ordinate, double value) {
        ...
        m_attributes[_getEndPointOffset(m_description, endPoint)
                + m_description._getPointAttributeOffset(attributeIndex) - 2 + ordinate] = value;
    ...
    boolean _equalsImpl(Segment other) {
        ...
        if (m_xStart != other.m_xStart || m_xEnd != other.m_xEnd || m_yStart != other.m_yStart
                || m_yEnd != other.m_yEnd)
            return false;
    ...
    /**
     * Returns true, when this segment is a closed curve (start point is equal to
     * end point exactly).
     * 
     * Note, this will return true for lines, that are degenerate to a point too.
     */
    boolean isClosed() {
    ...
    void reverse() {
        ...
        for (int i = 1, n = m_description.getAttributeCount(); i < n; i++) {
            ...
            for (int iord = 0, nord = VertexDescription.getComponentCount(semantics); iord < nord; iord++) {
    ...
    int _intersect(Segment other, Point2D[] intersectionPoints, double[] paramThis, double[] paramOther,
            double tolerance) {
        int gtThis = getType().value();
        int gtOther = other.getType().value();
        switch (gtThis) {
        case Geometry.GeometryType.Line:
            if (gtOther == Geometry.GeometryType.Line)
                return Line._intersectLineLine((Line) this, (Line) other, intersectionPoints, paramThis, paramOther,
                        tolerance);
            else
                throw GeometryException.GeometryInternalError();
        default:
    ...
    /**
     * A helper function for area calculation. Calculates the Integral(y(t) * x'(t)
     * * dt) for t = [0, 1]. The area of a ring is caluclated as a sum of the
     * results of CalculateArea2DHelper.
     */
    ...
    /**
     * Returns the coordinate of the point on this segment for the given parameter
     * value.
     */
    ...
    /**
     * Returns the coordinate of the point on this segment for the given parameter
     * value (segments are parametric curves).
     * 
     * @param t
     *            the parameter coordinate along the segment from 0.0 to 1.0. Value
     *            of 0 returns the start point, 1 returns end point.
     * @param dst
     ...
     * @param bExtrapolate
     *            TRUE if the segment is extrapolated at the end points along the
     *            end point tangents. Otherwise the result is limited to values
     *            between 0 and 1.
     ...
     *         obtain the 2D coordinate on the segment from t. To find the distance,
     *         call (inputPoint.sub(seg.getCoord2D(t))).length();
     */
     ...
     *            The in/out array of SegmentBuffer structures that will be filled
     *            with the monotonic parts. The monotonicSegments array must contain
     *            at least 3 elements.
     * @return The number of monotonic parts if the split had happened. Returns 0 if
     *         the segment is already monotonic.
     */
     ...
     *            The value of ordinate in the intersection points One ordinate is
     *            equal to the ordinate parameter. This parameter can be NULL.
     * @param parameters
     *            The value of the parameter in the intersection points (between 0
     *            and 1). This parameter can be NULL.
     * @return The number of intersection points, 0 when no intersection points
     *         exist, -1 when the segment coincides with the line (infinite number
     *         of intersection points).
     */
    public abstract int intersectionWithAxis2D(boolean bAxisX, double ordinate, double[] resultOrdinates,
            double[] parameters);
     ...
     * given tolerance. For Lines this means the line length is not longer than the
     * tolerance. For the curves, the distance between the segment endpoints should
     * not be longer than the tolerance and the distance from the line, connecting
     * the endpoints to the furtherst point on the segment is not larger than the
     * tolerance.
     */
    ...
    /**
     * Calculates the subsegment between parameters t1 and t2, and stores the result
     * in subSegmentBuffer. The attributes are interpolated along the length of the
     * curve.
     */
    ...
    abstract boolean _isIntersectingPoint(Point2D pt, double tolerance, boolean bExcludeExactEndpoints);

    /**
     * Calculates intersection point of this segment with an infinite line, parallel
     * to axis X. This segment must be to be y-monotonic (or horizontal).
     * 
     * @param y
    ...
    public double distance(/* const */Segment otherSegment, boolean bSegmentsKnownDisjoint) {
