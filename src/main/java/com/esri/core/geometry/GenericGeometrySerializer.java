...
public class GenericGeometrySerializer implements Serializable {
    ...
    public Object readResolve() throws ObjectStreamException {
        ...
        try {
            geometry = GeometryEngine.geometryFromEsriShape(esriShape, Geometry.Type.intToType(geometryType));

            if (Geometry.isMultiVertex(geometryType)) {
                MultiVertexGeometryImpl mvImpl = (MultiVertexGeometryImpl) geometry._getImpl();
                if (!geometry.isEmpty() && Geometry.isMultiPath(geometryType)) {
    ...
    public void setGeometryByValue(Geometry geometry) throws ObjectStreamException {
        try {
            esriShape = GeometryEngine.geometryToEsriShape(geometry);
