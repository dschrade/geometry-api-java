...
class RelationalOperations {
    ...
    static boolean relate(Geometry geometry_a, Geometry geometry_b, SpatialReference sr, int relation,
        ...
        if (type_a == Geometry.GeometryType.Envelope) {
            if (type_b == Geometry.GeometryType.Envelope) {
                return relate((Envelope) geometry_a, (Envelope) geometry_b, sr, relation, progress_tracker);
                ...
                return relate((Point) geometry_b, (Envelope) geometry_a, sr, relation, progress_tracker);
            } else {
                // proceed below
            }
        } else if (type_a == Geometry.GeometryType.Point) {
            if (type_b == Geometry.GeometryType.Envelope) {
                return relate((Point) geometry_a, (Envelope) geometry_b, sr, relation, progress_tracker);
            } else if (type_b == Geometry.GeometryType.Point) {
                return relate((Point) geometry_a, (Point) geometry_b, sr, relation, progress_tracker);
            } else {
                // proceed below
            }
        } else {
        ...
        if (type_a != Geometry.GeometryType.Envelope && type_b != Geometry.GeometryType.Envelope) {
            ...
            if (type_a != Geometry.GeometryType.Polygon && type_b != Geometry.GeometryType.Envelope) { // we will
                                                                                                       // switch
                                                                                                       // the order
                                                                                                       // of the
                                                                                                       // geometries
        ...
        case Geometry.GeometryType.Polygon:
            switch (type_b) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelatePolygon_((Polygon) (_geometry_a), (Polygon) (_geometry_b), tolerance, relation,
                        progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polygonRelatePolyline_((Polygon) (_geometry_a), (Polyline) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = polygonRelatePoint_((Polygon) (_geometry_a), (Point) (_geometry_b), tolerance, relation,
                        progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = polygonRelateMultiPoint_((Polygon) (_geometry_a), (MultiPoint) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Envelope:
                bRelation = polygonRelateEnvelope_((Polygon) (_geometry_a), (Envelope) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;
        ...
        case Geometry.GeometryType.Polyline:
            switch (type_b) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelatePolyline_((Polygon) (_geometry_b), (Polyline) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelatePolyline_((Polyline) (_geometry_a), (Polyline) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = polylineRelatePoint_((Polyline) (_geometry_a), (Point) (_geometry_b), tolerance, relation,
                        progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = polylineRelateMultiPoint_((Polyline) (_geometry_a), (MultiPoint) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Envelope:
                bRelation = polylineRelateEnvelope_((Polyline) (_geometry_a), (Envelope) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;
        ...
        case Geometry.GeometryType.Point:
            switch (type_b) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelatePoint_((Polygon) (_geometry_b), (Point) (_geometry_a), tolerance, relation,
                        progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelatePoint_((Polyline) (_geometry_b), (Point) (_geometry_a), tolerance, relation,
                        progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = multiPointRelatePoint_((MultiPoint) (_geometry_b), (Point) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;
        ...
        case Geometry.GeometryType.MultiPoint:
            switch (type_b) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelateMultiPoint_((Polygon) (_geometry_b), (MultiPoint) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelateMultiPoint_((Polyline) (_geometry_b), (MultiPoint) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = multiPointRelateMultiPoint_((MultiPoint) (_geometry_a), (MultiPoint) (_geometry_b),
                        tolerance, relation, progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = multiPointRelatePoint_((MultiPoint) (_geometry_a), (Point) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Envelope:
                bRelation = multiPointRelateEnvelope_((MultiPoint) (_geometry_a), (Envelope) (_geometry_b), tolerance,
                        relation, progress_tracker);
                break;
        ...
        case Geometry.GeometryType.Envelope:
            switch (type_b) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelateEnvelope_((Polygon) (_geometry_b), (Envelope) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelateEnvelope_((Polyline) (_geometry_b), (Envelope) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = multiPointRelateEnvelope_((MultiPoint) (_geometry_b), (Envelope) (_geometry_a), tolerance,
                        relation, progress_tracker);
                break;
    ...
    // relation.
    private static boolean relate(Envelope envelope_a, Envelope envelope_b, SpatialReference sr, int relation,
            ProgressTracker progress_tracker) {
    ...
    private static boolean relate(Point point_a, Envelope envelope_b, SpatialReference sr, int relation,
            ProgressTracker progress_tracker) {
    ...
    private static boolean relate(Point point_a, Point point_b, SpatialReference sr, int relation,
            ProgressTracker progress_tracker) {
    ...
    private static boolean polygonRelatePolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance, int relation,
            ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean polygonRelatePolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean polygonRelatePoint_(Polygon polygon_a, Point point_b, double tolerance, int relation,
            ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds
    private static boolean polygonRelateMultiPoint_(Polygon polygon_a, MultiPoint multipoint_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds
    private static boolean polygonRelateEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
    ...
    // Returns true if the relation holds.
    private static boolean polylineRelatePolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean polylineRelatePoint_(Polyline polyline_a, Point point_b, double tolerance, int relation,
            ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean polylineRelateMultiPoint_(Polyline polyline_a, MultiPoint multipoint_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean polylineRelateEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
    ...
    // Returns true if the relation holds.
    private static boolean multiPointRelateMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b,
            double tolerance, int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean multiPointRelatePoint_(MultiPoint multipoint_a, Point point_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    // Returns true if the relation holds.
    private static boolean multiPointRelateEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            int relation, ProgressTracker progress_tracker) {
        switch (relation) {
        case Relation.disjoint:
    ...
    private static boolean polygonEqualsPolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progress_tracker) {
        ...
        if (relation == Relation.disjoint || relation == Relation.contains || relation == Relation.within)
            return false;
        ...
        int max_vertices = StrictMath.max(polygon_a.getPointCount(), polygon_b.getPointCount());

        if (StrictMath.abs(length_a - length_b) > max_vertices * 4.0 * tolerance)
            return false;
    ...
    private static boolean polygonDisjointPolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        ...
        if (relation == Relation.contains || relation == Relation.within || relation == Relation.intersects)
            return false;
    ...
    private static boolean polygonTouchesPolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, polygon_b, tolerance, false);
    ...
    private static boolean polygonOverlapsPolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, polygon_b, tolerance, false);
    ...
    private static boolean polygonContainsPolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
        polygon_b.queryEnvelope2D(env_b);
    ...
    private static boolean polygonDisjointPolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
    ...
    private static boolean polygonTouchesPolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
    ...
    private static boolean polygonCrossesPolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, polyline_b, tolerance, false);
    ...
    private static boolean polygonContainsPolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
    ...
    private static boolean polygonDisjointPoint_(Polygon polygon_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    private static boolean polygonTouchesPoint_(Polygon polygon_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        Point2D pt_b = point_b.getXY();
    ...
    private static boolean polygonContainsPoint_(Polygon polygon_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        Point2D pt_b = point_b.getXY();
    ...
    private static boolean polygonDisjointMultiPoint_(Polygon polygon_a, MultiPoint multipoint_b, double tolerance,
        ...
        for (int i = 0; i < multipoint_b.getPointCount(); i++) {
            ...
            PolygonUtils.PiPResult result = PolygonUtils.isPointInPolygon2D(polygon_a, ptB, tolerance);
    ...
    private static boolean polygonTouchesMultiPoint_(Polygon polygon_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, multipoint_b, tolerance, false);
        ...
        for (int i = 0; i < multipoint_b.getPointCount(); i++) {
            ...
            if (!b_checked_polygon_a_quad_tree) {
                if (PointInPolygonHelper.quadTreeWillHelp(polygon_a, multipoint_b.getPointCount() - 1)
                        && (polygon_a_impl._getAccelerators() == null
                                || polygon_a_impl._getAccelerators().getQuadTree() == null)) {
                    pa = new Polygon();
                    polygon_a.copyTo(pa);
                    ((MultiPathImpl) pa._getImpl())
                            ._buildQuadTreeAccelerator(Geometry.GeometryAccelerationDegree.enumMedium);
                    p_polygon_a = pa;
                } else {
    ...
    private static boolean polygonCrossesMultiPoint_(Polygon polygon_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, multipoint_b, tolerance, false);
    ...
    private static boolean polygonContainsMultiPoint_(Polygon polygon_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
        multipoint_b.queryEnvelope2D(env_b);
    ...
    private static boolean polygonEqualsEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polygonDisjointEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
    ...
    private static boolean polygonTouchesEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, envelope_b, tolerance, false);
    ...
    private static boolean polygonOverlapsEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint,
        // or if one is contained in the other.
        int relation = tryRasterizedContainsOrDisjoint_(polygon_a, envelope_b, tolerance, false);
    ...
    private static boolean polygonWithinEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polygonContainsEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    private static boolean polygonCrossesEnvelope_(Polygon polygon_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineEqualsPolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        ...
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, polyline_b, tolerance, false) == Relation.disjoint)
            return false;
    ...
    private static boolean polylineDisjointPolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, polyline_b, tolerance, false) == Relation.disjoint)
            return true;
        ...
        PairwiseIntersectorImpl intersector_paths = new PairwiseIntersectorImpl(multi_path_impl_a, multi_path_impl_b,
                tolerance, true);
    ...
    private static boolean polylineTouchesPolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, polyline_b, tolerance, false) == Relation.disjoint)
            return false;
    ...
    private static boolean polylineCrossesPolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, polyline_b, tolerance, false) == Relation.disjoint)
            return false;
    ...
    private static boolean polylineOverlapsPolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, polyline_b, tolerance, false) == Relation.disjoint)
            return false;
    ...
    private static boolean polylineContainsPolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        polyline_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineDisjointPoint_(Polyline polyline_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, point_b, tolerance, false) == Relation.disjoint)
            return true;
    ...
    private static boolean polylineTouchesPoint_(Polyline polyline_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, point_b, tolerance, false) == Relation.disjoint)
            return false;
    ...
    private static boolean polylineContainsPoint_(Polyline polyline_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, point_b, tolerance, false) == Relation.disjoint)
            return false;
    ...
    private static boolean polylineDisjointMultiPoint_(Polyline polyline_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, multipoint_b, tolerance, false) == Relation.disjoint)
            return true;
    ...
    private static boolean polylineTouchesMultiPoint_(Polyline polyline_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, multipoint_b, tolerance, false) == Relation.disjoint) {
            return false;
        ...
        if (accel != null) {
            ...
            if (quadTreeA == null) {
                qtA = InternalUtils.buildQuadTree((MultiPathImpl) polyline_a._getImpl(), envInter);
                quadTreeA = qtA;
            }
        } else {
            qtA = InternalUtils.buildQuadTree((MultiPathImpl) polyline_a._getImpl(), envInter);
    ...
    private static boolean polylineCrossesMultiPoint_(Polyline polyline_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        // Quick rasterize test to see whether the the geometries are disjoint.
        if (tryRasterizedContainsOrDisjoint_(polyline_a, multipoint_b, tolerance, false) == Relation.disjoint) {
            return false;
    ...
    private static boolean polylineContainsMultiPoint_(Polyline polyline_a, MultiPoint multipoint_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        multipoint_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineEqualsEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineDisjointEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineTouchesEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineOverlapsEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineWithinEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean polylineContainsEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        envelope_b.queryEnvelope2D(env_b);
        polyline_a.queryEnvelope2D(env_a);
    ...
    private static boolean polylineCrossesEnvelope_(Polyline polyline_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        polyline_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean multiPointEqualsMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b,
        ...
        return multiPointCoverageMultiPoint_(multipoint_a, multipoint_b, tolerance, false, true, false,
                progress_tracker);
    }

    // Returns true if multipoint_a is disjoint from multipoint_b.
    private static boolean multiPointDisjointMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b,
            double tolerance, ProgressTracker progress_tracker) {
    ...
    private static boolean multiPointOverlapsMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b,
            double tolerance, ProgressTracker progress_tracker) {
        return multiPointCoverageMultiPoint_(multipoint_a, multipoint_b, tolerance, false, false, true,
                progress_tracker);
    }

    // Returns true if multipoint_a contains multipoint_b.
    private static boolean multiPointContainsMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b,
            double tolerance, ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
        multipoint_b.queryEnvelope2D(env_b);
        ...
        return multiPointCoverageMultiPoint_(multipoint_b, multipoint_a, tolerance, true, false, false,
                progress_tracker);
    }

    private static boolean multiPointContainsMultiPointBrute_(MultiPoint multipoint_a, MultiPoint multipoint_b,
            double tolerance) {
    ...
    static boolean multiPointEqualsPoint_(MultiPoint multipoint_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
    ...
    private static boolean multiPointDisjointPoint_(MultiPoint multipoint_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        Point2D pt_b = point_b.getXY();
    ...
    private static boolean multiPointWithinPoint_(MultiPoint multipoint_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        return multiPointEqualsPoint_(multipoint_a, point_b, tolerance, progress_tracker);
    }

    // Returns true if multipoint_a contains point_b.
    private static boolean multiPointContainsPoint_(MultiPoint multipoint_a, Point point_b, double tolerance,
            ProgressTracker progress_tracker) {
        return !multiPointDisjointPoint_(multipoint_a, point_b, tolerance, progress_tracker);
    }

    // Returns true if multipoint_a equals envelope_b.
    private static boolean multiPointEqualsEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean multiPointDisjointEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean multiPointTouchesEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    private static boolean multiPointWithinEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
        ...
        if (env_b.getHeight() <= tolerance && env_b.getWidth() <= tolerance)
            return envelopeEqualsEnvelope_(env_a, env_b, tolerance, progress_tracker); // treat as point
    ...
    private static boolean multiPointContainsEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    static boolean multiPointCrossesEnvelope_(MultiPoint multipoint_a, Envelope envelope_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D();
        multipoint_a.queryEnvelope2D(env_a);
        envelope_b.queryEnvelope2D(env_b);
    ...
    private static boolean pointEqualsPoint_(Point2D pt_a, Point2D pt_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    private static boolean pointDisjointPoint_(Point2D pt_a, Point2D pt_b, double tolerance,
            ProgressTracker progress_tracker) {
        if (Point2D.sqrDistance(pt_a, pt_b) > tolerance * tolerance)
            return true;
    ...
    private static boolean pointContainsPoint_(Point2D pt_a, Point2D pt_b, double tolerance,
            ProgressTracker progress_tracker) {
        return pointEqualsPoint_(pt_a, pt_b, tolerance, progress_tracker);
    }

    // Returns true if pt_a equals enve_b.
    private static boolean pointEqualsEnvelope_(Point2D pt_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_a = new Envelope2D();
    ...
    static boolean pointDisjointEnvelope_(Point2D pt_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_b_inflated = new Envelope2D();
        env_b_inflated.setCoords(env_b);
        env_b_inflated.inflate(tolerance, tolerance);
    ...
    private static boolean pointTouchesEnvelope_(Point2D pt_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        if (env_b.getHeight() <= tolerance && env_b.getWidth() <= tolerance)
    ...
    private static boolean pointWithinEnvelope_(Point2D pt_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        if (env_b.getHeight() <= tolerance && env_b.getWidth() <= tolerance) {
    ...
    private static boolean pointContainsEnvelope_(Point2D pt_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        return pointEqualsEnvelope_(pt_a, env_b, tolerance, progress_tracker);
    }

    // Returns true if env_a equals env_b.
    private static boolean envelopeEqualsEnvelope_(Envelope2D env_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    static boolean envelopeDisjointEnvelope_(Envelope2D env_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        Envelope2D env_b_inflated = new Envelope2D();
        env_b_inflated.setCoords(env_b);
        env_b_inflated.inflate(tolerance, tolerance);
    ...
    private static boolean envelopeTouchesEnvelope_(Envelope2D env_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        ...
        if (_env_a.getHeight() <= tolerance || _env_a.getWidth() <= tolerance) {// treat
            ...
            if (_env_b.getHeight() <= tolerance || _env_b.getWidth() <= tolerance) {// treat env_b as line
            ...
            assert (!envelopeDisjointEnvelope_(_env_a, _env_b, tolerance, progress_tracker));
    ...
    private static boolean envelopeOverlapsEnvelope_(Envelope2D env_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        if (envelopeInfContainsEnvelope_(env_a, env_b, tolerance)
                || envelopeInfContainsEnvelope_(env_b, env_a, tolerance))
            return false;
        ...
        assert (!envelopeInfContainsEnvelope_(env_inter, env_a, tolerance)
                && !envelopeInfContainsEnvelope_(env_inter, env_b, tolerance));

        return true;
    }

    // Returns true if env_a contains env_b.
    private static boolean envelopeContainsEnvelope_(Envelope2D env_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        if (!envelopeInfContainsEnvelope_(env_a, env_b, tolerance))
            return false;
    ...
    private static boolean envelopeCrossesEnvelope_(Envelope2D env_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
        if (envelopeInfContainsEnvelope_(env_a, env_b, tolerance)
                || envelopeInfContainsEnvelope_(env_b, env_a, tolerance))
            return false;
    ...
    private static boolean polygonDisjointMultiPath_(Polygon polygon_a, MultiPath multipath_b, double tolerance,
            ProgressTracker progress_tracker) {
        Point2D pt_a, pt_b;
        ...
        PairwiseIntersectorImpl intersector = new PairwiseIntersectorImpl(multi_path_impl_a, multi_path_impl_b,
                tolerance, true);
        ...
        do {
            ...
            if (!b_checked_polygon_a_quad_tree) {
                if (PointInPolygonHelper.quadTreeWillHelp(polygon_a, multipath_b.getPathCount() - 1)
                        && (multi_path_impl_a._getAccelerators() == null
                                || multi_path_impl_a._getAccelerators().getQuadTree() == null)) {
                    pa = new Polygon();
                    polygon_a.copyTo(pa);
                    ((MultiPathImpl) pa._getImpl())
                            ._buildQuadTreeAccelerator(Geometry.GeometryAccelerationDegree.enumMedium);
            ...
            if (multipath_b.getType().value() == Geometry.GeometryType.Polygon) {
                if (!b_checked_polygon_b_quad_tree) {
                    ...
                    if (PointInPolygonHelper.quadTreeWillHelp(polygon_b, polygon_a.getPathCount() - 1)
                            && (multi_path_impl_b._getAccelerators() == null
                                    || multi_path_impl_b._getAccelerators().getQuadTree() == null)) {
                        pb = new Polygon();
                        polygon_b.copyTo(pb);
                        ((MultiPathImpl) pb._getImpl())
                                ._buildQuadTreeAccelerator(Geometry.GeometryAccelerationDegree.enumMedium);
                        p_polygon_b = pb;
                    } else {
                        p_polygon_b = (Polygon) multipath_b;
    ...
    private static boolean multiPathExactlyEqualsMultiPath_(MultiPath multipathA, MultiPath multipathB,
            double tolerance, ProgressTracker progress_tracker) {
    ...
    private static boolean multiPointExactlyEqualsMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b,
            double tolerance, ProgressTracker progress_tracker) {
    ...
    private static boolean multiPointCoverageMultiPoint_(MultiPoint _multipointA, MultiPoint _multipointB,
            double tolerance, boolean bPerformWithin, boolean bPerformEquals, boolean bPerformOverlaps,
            ProgressTracker progress_tracker) {
    ...
    private static boolean multiPointIntersectsMultiPoint_(MultiPoint _multipointA, MultiPoint _multipointB,
            double tolerance, ProgressTracker progress_tracker) {
        MultiPoint multipoint_a;
        MultiPoint multipoint_b;
    ...
    private static boolean linearPathEqualsLinearPath_(MultiPath multipathA, MultiPath multipathB, double tolerance,
            boolean bEnforceOrientation) {
        return linearPathWithinLinearPath_(multipathA, multipathB, tolerance, bEnforceOrientation)
                && linearPathWithinLinearPath_(multipathB, multipathA, tolerance, bEnforceOrientation);
    }

    // Returns true if the segments of multipathA are within the segments of
    // multipathB.
    private static boolean linearPathWithinLinearPath_(MultiPath multipathA, MultiPath multipathB, double tolerance,
            boolean bEnforceOrientation) {
        ...
        if (accel != null) {
            ...
            if (quadTreeB == null) {
                qtB = InternalUtils.buildQuadTree((MultiPathImpl) multipathB._getImpl(), envInter);
                quadTreeB = qtB;
            }
        } else {
            qtB = InternalUtils.buildQuadTree((MultiPathImpl) multipathB._getImpl(), envInter);
                ...
                for (int elementHandleB = qtIterB.next(); elementHandleB != -1; elementHandleB = qtIterB.next()) {
                    ...
                    if (result == 2 && (!bEnforceOrientation || scalarsB[0] <= scalarsB[1])) {
                        ...
                        if (scalar_a_0 * lengthA <= tolerance && (1.0 - scalar_a_1) * lengthA <= tolerance) {
                            ...
                            overlapEvent = OverlapEvent.construct(ivertex_a, ipath_a, scalar_a_0, scalar_a_1, ivertex_b,
    ...
    private static boolean linearPathOverlapsLinearPath_(MultiPath multipathA, MultiPath multipathB, double tolerance) {
        ...
        return !linearPathWithinLinearPath_(multipathA, multipathB, tolerance, false)
                && !linearPathWithinLinearPath_(multipathB, multipathA, tolerance, false);
    }

    // Returns true the dimension of intersection of _multipathA and
    // _multipathB.
    static int linearPathIntersectsLinearPathMaxDim_(MultiPath _multipathA, MultiPath _multipathB, double tolerance,
            ...
            while (segIterA.hasNextSegment()) {
                ...
                for (int elementHandleB = qtIterB.next(); elementHandleB != -1; elementHandleB = qtIterB.next()) {
                    ...
                    if (result > 0) {
                        double scalar_a_0 = scalarsA[0];
                        double scalar_b_0 = scalarsB[0];
                        double scalar_a_1 = (result == 2 ? scalarsA[1] : NumberUtils.TheNaN);
                        double scalar_b_1 = (result == 2 ? scalarsB[1] : NumberUtils.TheNaN);
                ...
                if (ievent < relOps.m_overlap_events.size()) {
                    ...
                    double lastScalar = 0.0;
                    int lastPath = relOps.m_overlap_events.get(eventIndices.get(ievent)).m_ipath_a;
                    ...
                    for (int i = ievent; i < relOps.m_overlap_events.size(); i++) {
                        ...
                        if (lengthA * (overlapEvent.m_scalar_a_0 - lastScalar) > tolerance) {
                            overlapLength = lengthA * (overlapEvent.m_scalar_a_1 - overlapEvent.m_scalar_a_0); // reset
                            lastScalar = overlapEvent.m_scalar_a_1;
                            ...
                            if (overlapEvent.m_ipath_a != lastPath) {
                                ...
                                overlapLength += lengthA * (overlapEvent.m_scalar_a_1 - overlapEvent.m_scalar_a_0); // accumulate
                            }
    ...
    private static boolean linearPathIntersectsLinearPath_(MultiPath multipathA, MultiPath multipathB,
            double tolerance) {
    ...
    // Quad_tree_impl.
    private static boolean linearPathIntersectsMultiPoint_(MultiPath multipathA, MultiPoint multipoint_b,
            double tolerance, boolean b_intersects_all) {
        SegmentIteratorImpl segIterA = ((MultiPathImpl) multipathA._getImpl()).querySegmentIterator();
        ...
        if (accel != null) {
            ...
            quadTreePathsA = accel.getQuadTreeForPaths();
            if (quadTreeA == null) {
                qtA = InternalUtils.buildQuadTree((MultiPathImpl) multipathA._getImpl(), envInter);
                quadTreeA = qtA;
            }
        } else {
            qtA = InternalUtils.buildQuadTree((MultiPathImpl) multipathA._getImpl(), envInter);
    ...
    private static boolean linearPathIntersectsEnvelope_(MultiPath multipath_a, Envelope2D env_b, double tolerance,
            ProgressTracker progress_tracker) {
    ...
    // "intersects".
    static int tryRasterizedContainsOrDisjoint_(Geometry geom_a, Geometry geom_b, double tolerance,
            boolean bExtraTestForIntersects) {
        int gtA = geom_a.getType().value();
        int gtB = geom_b.getType().value();
        do {
            if (Geometry.isMultiVertex(gtA)) {
                MultiVertexGeometryImpl impl = (MultiVertexGeometryImpl) geom_a._getImpl();
                ...
                if (accel != null) {
                    ...
                    if (rgeom != null) {
                        if (gtB == Geometry.GeometryType.Point) {
                            ...
                            RasterizedGeometry2D.HitType hit = rgeom.queryPointInGeometry(ptB.x, ptB.y);
                        ...
                        RasterizedGeometry2D.HitType hit = rgeom.queryEnvelopeInGeometry(env_b);
                        if (hit == RasterizedGeometry2D.HitType.Inside) {
                            return Relation.contains;
                        } else if (hit == RasterizedGeometry2D.HitType.Outside) {
                            return Relation.disjoint;
                        } else if (bExtraTestForIntersects && Geometry.isMultiVertex(gtB)) {
                            if (checkVerticesForIntersection_((MultiVertexGeometryImpl) geom_b._getImpl(), rgeom)) {
        ...
        do {
            if (Geometry.isMultiVertex(gtB)) {
                MultiVertexGeometryImpl impl = (MultiVertexGeometryImpl) geom_b._getImpl();
                GeometryAccelerators accel = impl._getAccelerators();
                if (accel != null) {
                    RasterizedGeometry2D rgeom = accel.getRasterizedGeometry();
                    if (rgeom != null) {
                        if (gtA == Geometry.GeometryType.Point) {
                            ...
                            RasterizedGeometry2D.HitType hit = rgeom.queryPointInGeometry(ptA.x, ptA.y);
                            if (hit == RasterizedGeometry2D.HitType.Inside) {
                        ...
                        RasterizedGeometry2D.HitType hit = rgeom.queryEnvelopeInGeometry(env_a);
                        if (hit == RasterizedGeometry2D.HitType.Inside) {
                            return Relation.within;
                        } else if (hit == RasterizedGeometry2D.HitType.Outside) {
                            return Relation.disjoint;
                        } else if (bExtraTestForIntersects && Geometry.isMultiVertex(gtA)) {
                            if (checkVerticesForIntersection_((MultiVertexGeometryImpl) geom_a._getImpl(), rgeom)) {
                                return Relation.intersects;
                            }
                        }
    ...
    private static boolean polygonTouchesPolygonImpl_(Polygon polygon_a, Polygon polygon_b, double tolerance,
        ...
        PairwiseIntersectorImpl intersector = new PairwiseIntersectorImpl(polygon_impl_a, polygon_impl_b, tolerance,
                false);
        ...
        String scl = "F********";
        boolean bRelation = RelationalOperationsMatrix.polygonRelatePolygon_(_polygonA, _polygonB, tolerance, scl,
                progressTracker);

        return bRelation;
    }

    private static boolean polygonOverlapsPolygonImpl_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progressTracker) {
        MultiPathImpl polygon_impl_a = (MultiPathImpl) polygon_a._getImpl();
        MultiPathImpl polygon_impl_b = (MultiPathImpl) polygon_b._getImpl();
        ...
        boolean bRelation = RelationalOperationsMatrix.polygonRelatePolygon_(_polygonA, _polygonB, tolerance,
                scl.toString(), progressTracker);
        return bRelation;
    }

    private static boolean polygonContainsPolygonImpl_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progressTracker) {
        ...
        boolean bContains = RelationalOperationsMatrix.polygonContainsPolygon_(_polygonA, polygon_b, tolerance,
                progressTracker);
        return bContains;
    }

    private static boolean polygonContainsMultiPath_(Polygon polygon_a, MultiPath multi_path_b, double tolerance,
            boolean[] b_result_known, ProgressTracker progress_tracker) {
        b_result_known[0] = false;
        ...
        PairwiseIntersectorImpl intersector = new PairwiseIntersectorImpl(polygon_impl_a, multi_path_impl_b, tolerance,
                false);
        boolean b_boundaries_intersect = false;
        ...
        if (!b_boundaries_intersect) {
            ...
            for (int ipath = 0, npath = multi_path_b.getPathCount(); ipath < npath; ipath++) {
                if (multi_path_b.getPathSize(ipath) > 0) {
                    ...
                    if (!b_checked_polygon_a_quad_tree) {
                        if (PointInPolygonHelper.quadTreeWillHelp(polygon_a, multi_path_b.getPathCount() - 1)
                                && (polygon_impl_a._getAccelerators() == null
                                        || polygon_impl_a._getAccelerators().getQuadTree() == null)) {
                            pa = new Polygon();
                            polygon_a.copyTo(pa);
                            ((MultiPathImpl) pa._getImpl())
                                    ._buildQuadTreeAccelerator(Geometry.GeometryAccelerationDegree.enumMedium);
    ...
    private static boolean polygonTouchesPolylineImpl_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progressTracker) {
        MultiPathImpl polygon_impl_a = (MultiPathImpl) polygon_a._getImpl();
        ...
        PairwiseIntersectorImpl intersector = new PairwiseIntersectorImpl(polygon_impl_a, polyline_impl_b, tolerance,
                false);
        ...
        String scl = "F********";
        boolean bRelation = RelationalOperationsMatrix.polygonRelatePolyline_(_polygonA, _polylineB, tolerance, scl,
                progressTracker);

        return bRelation;
    }

    private static boolean polygonCrossesPolylineImpl_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progressTracker) {
        MultiPathImpl polygon_impl_a = (MultiPathImpl) polygon_a._getImpl();
        MultiPathImpl polyline_impl_b = (MultiPathImpl) polyline_b._getImpl();
        ...
        Envelope2D env_a = new Envelope2D(), env_b = new Envelope2D(), envAInflated = new Envelope2D(),
                envBInflated = new Envelope2D(), envInter = new Envelope2D();
        polygon_a.queryEnvelope2D(env_a);
        polyline_b.queryEnvelope2D(env_b);
        ...
        String scl = "T*****T**";
        boolean bRelation = RelationalOperationsMatrix.polygonRelatePolyline_(polygon_a, polyline_b, tolerance, scl,
                progressTracker);

        return bRelation;
    }

    private static boolean polygonContainsPolylineImpl_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        boolean[] b_result_known = new boolean[1];
        b_result_known[0] = false;
        ...
        boolean bContains = RelationalOperationsMatrix.polygonContainsPolyline_(_polygonA, polyline_b, tolerance,
                progress_tracker);
        return bContains;
    }

    private static boolean polygonContainsPointImpl_(Polygon polygon_a, Point2D pt_b, double tolerance,
            ProgressTracker progressTracker) {
    ...
    private static boolean polygonTouchesPointImpl_(Polygon polygon_a, Point2D pt_b, double tolerance,
            ProgressTracker progressTracker) {
        PolygonUtils.PiPResult result = PolygonUtils.isPointInPolygon2D(polygon_a, pt_b, tolerance);
    ...
    static boolean multiPointDisjointPointImpl_(MultiPoint multipoint_a, Point2D pt_b, double tolerance,
            ProgressTracker progressTracker) {
        Point2D pt_a = new Point2D();
        double tolerance_sq = tolerance * tolerance;
    ...
    private static final class OverlapEvent {
        ...
        static OverlapEvent construct(int ivertex_a, int ipath_a, double scalar_a_0, double scalar_a_1, int ivertex_b,
    ...
    private static class OverlapComparer extends AttributeStreamOfInt32.IntComparator {
    ...
    static final class Accelerate_helper {
        static boolean accelerate_geometry(Geometry geometry, SpatialReference sr,
            ...
            if (GeometryAccelerators.canUseRasterizedGeometry(geometry))
                bAccelerated |= ((MultiVertexGeometryImpl) geometry._getImpl())._buildRasterizedGeometryAccelerator(tol,
                        accel_degree);
