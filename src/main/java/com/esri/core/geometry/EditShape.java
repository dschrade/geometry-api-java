...
final class EditShape {
    ...
    int newGeometry_(int gt) {
        ...
        m_geometry_index_list.setField(index, 7, m_geometry_index_list.elementToIndex(index));// geometry index
    ...
    void appendMultiPath_(int dstGeom, MultiPath multi_path) {
        ...
        m_xy_stream = (AttributeStreamOfDbl) m_vertices.getAttributeStreamRef(VertexDescription.Semantics.POSITION);
        boolean b_some_segments = m_segments != null && mp_impl.getSegmentFlagsStreamRef() != null;
        ...
        for (int ipath = 0, npath = mp_impl.getPathCount(); ipath < npath; ipath++) {
            ...
            for (int ivertex = mp_impl.getPathStart(ipath),
                    iend = mp_impl.getPathEnd(ipath); ivertex < iend; ivertex++) {
    ...
    int addPathFromMultiPath(MultiPath multi_path, int ipath, boolean as_polygon) {
        int newgeom = createGeometry(as_polygon ? Geometry.Type.Polygon : Geometry.Type.Polyline,
    ...
    Geometry getGeometry(int geometry) {
        ...
        Geometry geom = InternalUtils.createGeometry(gt, m_vertices_mp.getDescription());
        ...
        if (Geometry.isMultiPath(gt)) {
            ...
            for (int iattrib = 0, nattrib = description.getAttributeCount(); iattrib < nattrib; iattrib++) {
                ...
                AttributeStreamBase dst_stream = AttributeStreamBase.createAttributeStreamWithSemantics(semantics,
                        point_count);
                AttributeStreamBase src_stream = m_vertices.getAttributeStreamRef(semantics);
                int dst_index = 0;
                int ipath = 0;
                int nvert = 0;
                for (int path = getFirstPath(geometry); path != -1; path = getNextPath(path)) {
                    ...
                    if (semantics == VertexDescription.Semantics.POSITION) {
                        ...
                        Point2D pt = new Point2D();
                        for (int vertex = getFirstVertex(path); dst_index < nvert; vertex = getNextVertex(
                                vertex), dst_index++) {
            ...
            for (int iattrib = 0, nattrib = description.getAttributeCount(); iattrib < nattrib; iattrib++) {
                ...
                int path_size = getPathSize(path);
                for (int vertex = getFirstVertex(path); dst_index < path_size; vertex = getNextVertex(
                        vertex), dst_index++) {
                    int src_index = getVertexIndex(vertex);
                    for (int icomp = 0; icomp < ncomps; icomp++) {
                        double d = src_stream.readAsDbl(src_index * ncomps + icomp);
    ...
    int createGeometry(Geometry.Type geometry_type) {
        return createGeometry(geometry_type, VertexDescriptionDesignerImpl.getDefaultDescriptor2D());
    }
    ...
    void setGeometryUserIndex(int geom, int index, int value) {
        ...
        if (pindex >= stream.size())
            stream.resize(StrictMath.max((int) (pindex * 1.25), (int) 16), -1);
    ...
    int createGeometryUserIndex() {
        ...
        for (int i = 0; i < m_geometry_indices.size(); i++) {
            if (m_geometry_indices.get(i) == null) {
                m_geometry_indices.set(i, (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(0));
                return i;
            }
        }

        m_geometry_indices.add((AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(0));
    ...
    int filterClosePoints(double tolerance, boolean b_remove_last_vertices, boolean only_polygons) {
        ...
        for (int geometry = getFirstGeometry(); geometry != -1; geometry = getNextGeometry(geometry)) {
            ...
            for (int path = getFirstPath(geometry); path != -1;) {
                ...
                for (int vertex = isClosedPath(path) ? first_vertex : getLastVertex(path); getPathSize(path) > 0;) {
    ...
    int splitSegment(int origin_vertex, double[] split_scalars, int split_count) {
        ...
        Segment seg = getSegmentFromIndex_(vindex);
        double seg_length = seg == null ? m_vertices._getShortestDistance(vindex, vindex_next)
                : seg.calculateLength2D();
        double told = 0.0;
        for (int i = 0; i < split_count; i++) {
            ...
            if (told < t && t < 1.0) {
                ...
                m_vertices._interpolateTwoVertices(vindex, vindex_next, f, getHelperPoint_());// use this call mainly to
                                                                                              // interpolate the
                                                                                              // attributes. XYs
                                                                                              // are interpolated
                                                                                              // incorrectly for
    ...
    void setWeight(int vertex, double weight) {
        ...
        if (m_weights == null) {
            ...
            m_weights = (AttributeStreamOfDbl) (AttributeStreamBase.createDoubleStream(m_vertices.getPointCount(),
                    1.0));
        }
    ...
    int createUserIndex() {
        ...
        for (int i = 0; i < m_indices.size(); i++) {
            if (m_indices.get(i) == null) {
                m_indices.set(i, (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(0, -1));
                return i;
            }
        }

        m_indices.add((AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(0, -1));
    ...
    int insertClosedPath_(int geometry, int before_path, int first_vertex, int checked_vertex,
    ...
    void setClosedPath(int path, boolean b_yes_no) {
        ...
        int flags = (oldflags | (int) PathFlags_.closedPath) - (int) PathFlags_.closedPath;// clear the bit;
    ...
    void setExterior(int path, boolean b_yes_no) {
        int oldflags = getPathFlags_(path);
        int flags = (oldflags | (int) PathFlags_.exteriorPath) - (int) PathFlags_.exteriorPath;// clear the bit;
        setPathFlags_(path, flags | (b_yes_no ? (int) PathFlags_.exteriorPath : 0));
    }
    ...
    int createPathUserIndex() {
        ...
        for (int i = 0; i < m_pathindices.size(); i++) {
            if (m_pathindices.get(i) == null) {
                m_pathindices.set(i, (AttributeStreamOfInt32) (AttributeStreamBase.createIndexStream(0)));
                return i;
            }
        }

        m_pathindices.add((AttributeStreamOfInt32) (AttributeStreamBase.createIndexStream(0)));
    ...
    int getPrevVertex(int currentVertex, int dir) {
        return dir > 0 ? m_vertex_index_list.getField(currentVertex, 1)
                : m_vertex_index_list.getField(currentVertex, 2);
    }

    int getNextVertex(int currentVertex, int dir) {
        return dir > 0 ? m_vertex_index_list.getField(currentVertex, 2)
                : m_vertex_index_list.getField(currentVertex, 1);
    }
    ...
    static class VertexIterator {
        ...
        private VertexIterator(EditShape parent, int geometry, int path, int vertex, int first_vertex, int index,
        ...
        int moveToNextHelper_() {
            ...
            while (m_geometry != -1) {
                ...
                if (m_b_skip_mulit_points && !Geometry.isMultiPath(m_parent.getGeometryType(m_geometry))) {
                    continue;
                }
        ...
        public static VertexIterator create_(EditShape parent, int geometry, int path, int vertex, int first_vertex,
                int index, boolean b_skip_mulit_points) {
            return new VertexIterator(parent, geometry, path, vertex, first_vertex, index, b_skip_mulit_points);
        }
    };
    ...
    VertexIterator queryVertexIterator(boolean b_skip_multi_points) {
        ...
        for (geometry = getFirstGeometry(); geometry != -1; geometry = getNextGeometry(geometry)) {
            if (b_skip_multi_points && !Geometry.isMultiPath(getGeometryType(geometry)))
                continue;
        ...
        return VertexIterator.create_(this, geometry, path, vertex, first_vertex, index, b_skip_multi_points);
    }
    ...
    void interpolateAttributesForClosedPath_(int semantics, int path, int from_vertex, int to_vertex, double sub_length,
            int ordinate) {
    ...
    void setRingAreaValid_(int path, boolean b_yes_no) {
        int oldflags = getPathFlags_(path);
        int flags = (oldflags | (int) PathFlags_.ringAreaValid) - (int) PathFlags_.ringAreaValid;// clear the bit;
        setPathFlags_(path, flags | (b_yes_no ? (int) PathFlags_.ringAreaValid : 0));
    }
