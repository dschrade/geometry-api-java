...
public abstract class OperatorUnion extends Operator implements CombineOperator {
     ...
     * 
     * @param geom1
     *            and geom2 are the geometry instances to be unioned.
     *
     */
    public abstract Geometry execute(Geometry geom1, Geometry geom2, SpatialReference sr,
            ProgressTracker progressTracker);

    public static OperatorUnion local() {
        return (OperatorUnion) OperatorFactoryLocal.getInstance().getOperator(Type.Union);
    }
