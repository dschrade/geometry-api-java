...
class OperatorImportFromWkbLocal extends OperatorImportFromWkb {
    ...
    @Override
    public Geometry execute(int importFlags, Geometry.Type type, ByteBuffer wkbBuffer,
        ...
        try {
            ...
            while (!stack.isEmpty()) {
                ...
                if (ogcType == 7) {
                    ...
                    geometry = importFromWkb(importFlags, Geometry.Type.Unknown, wkbHelper);
    ...
    private static Geometry importFromWkbPolygon(boolean bMultiPolygon, int importFlags, boolean bZs, boolean bMs,
            WkbHelper wkbHelper) {
        int offset;
        int polygonCount;
        ...
        for (int ipolygon = 0; ipolygon < polygonCount; ipolygon++) {
            ...
            for (int ipart = 0; ipart < ipartcount; ipart++) {
                ...
                if ((startx == endx || (NumberUtils.isNaN(startx) && NumberUtils.isNaN(endx)))
                        && (starty == endy || (NumberUtils.isNaN(starty) && NumberUtils.isNaN(endy)))
                        && (!bZs || startz == endz || (NumberUtils.isNaN(startz) && NumberUtils.isNaN(endz)))
                        && (!bMs || startm == endm || (NumberUtils.isNaN(startm) && NumberUtils.isNaN(endm)))) {
                    point_count += ipointcount - 1;
                } else {
                    point_count += ipointcount;
                }
        ...
        if (point_count > 0) {
            ...
            pathFlags = (AttributeStreamOfInt8) (AttributeStreamBase.createByteStream(parts.size(),
                    (byte) PathFlags.enumClosed));
            position = (AttributeStreamOfDbl) (AttributeStreamBase
                    .createAttributeStreamWithSemantics(VertexDescription.Semantics.POSITION, point_count));

            if (bZs)
                zs = (AttributeStreamOfDbl) (AttributeStreamBase
                        .createAttributeStreamWithSemantics(VertexDescription.Semantics.Z, point_count));

            if (bMs)
                ms = (AttributeStreamOfDbl) (AttributeStreamBase
                        .createAttributeStreamWithSemantics(VertexDescription.Semantics.M, point_count));
        }
        ...
        for (int ipolygon = 0; ipolygon < polygonCount; ipolygon++) {
            ...
            for (int ipart = ipolygonstart; ipart < ipolygonend; ipart++) {
                ...
                if (ipart == ipolygonstart)
                    pathFlags.setBits(ipart, (byte) PathFlags.enumOGCStartPolygon);
                ...
                for (int i = ipartstart; i < ipartend; i++) {
                    ...
                    if (bZs) {
                        ...
                        zs.write(i, z);
                        if (!VertexDescription.isDefaultValue(VertexDescription.Semantics.Z, z))
                            bCreateZs = true;
                    }
                    ...
                    if (bMs) {
                        ...
                        ms.write(i, m);
                        if (!VertexDescription.isDefaultValue(VertexDescription.Semantics.M, m))
                            bCreateMs = true;
                    }
                }
        ...
        if (point_count > 0) {
            ...
            polygon.setAttributeStreamRef(VertexDescription.Semantics.POSITION, position);
        ...
        if ((importFlags & (int) WkbImportFlags.wkbImportNonTrusted) == 0)
            polygon.setIsSimple(MultiVertexGeometryImpl.GeometryXSimple.Weak, 0.0, false);
    ...
    private static Geometry importFromWkbPolyline(boolean bMultiPolyline, int importFlags, boolean bZs, boolean bMs,
            WkbHelper wkbHelper) {
        int offset;
        ...
        if (point_count > 0) {
            ...
            polyline.setAttributeStreamRef(VertexDescription.Semantics.POSITION, position);

            if (bZs) {
                if (!bCreateZs)
                    zs = null;

                polyline.setAttributeStreamRef(VertexDescription.Semantics.Z, zs);
            }
            ...
            if (bMs) {
                ...
                polyline.setAttributeStreamRef(VertexDescription.Semantics.M, ms);
            }
    ...
    private static Geometry importFromWkbMultiPoint(int importFlags, boolean bZs, boolean bMs, WkbHelper wkbHelper) {
        ...
        if (point_count > 0) {
            ...
            multipoint.setAttributeStreamRef(VertexDescription.Semantics.POSITION, position);

            if (bZs) {
                if (!bCreateZs)
                    zs = null;

                multipoint.setAttributeStreamRef(VertexDescription.Semantics.Z, zs);
            }
            ...
            if (bMs) {
                ...
                multipoint.setAttributeStreamRef(VertexDescription.Semantics.M, ms);
            }
