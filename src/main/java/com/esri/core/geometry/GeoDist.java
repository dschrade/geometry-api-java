...
final class GeoDist {
    ...
    static private double lam_delta(double lam) {
        double d = StrictMath.IEEEremainder(lam, PE_2PI);
    ...
    static private double q90(double a, double e2) {
         ...
         * Developments ... // pp. 122-127. Terms extended past n4 by David Burrows,
         * ESRI
         */
        ...
        /*
         * q90 = a * PE_PI2 * (1 + 1/4 n2 + 1/64 n4 + 1/256 n6 + 25/16384 n8 + 49/65536
         * n10 + ...)/(1.0 + n)
         */

        double t = StrictMath.sqrt(1.0 - e2);
    ...
    static public void geodesic_distance_ngs(double a, double e2, double lam1, double phi1, double lam2, double phi2,
            PeDouble p_dist, PeDouble p_az12, PeDouble p_az21) {
        /* Highly edited version (plus lots of additions) of NGS FORTRAN code */

        /*
         * inverse for long-line and antipodal cases.* latitudes may be 90 degrees
         * exactly.* latitude positive north, longitude positive east, radians.* azimuth
         * clockwise from north, radians.* original programmed by thaddeus vincenty,
         * 1975, 1976* removed back side solution option, debugged, revised -- 2011may01
         * -- dgm* this version of code is interim -- antipodal boundary needs work
         * 
         * * output (besides az12, az21, and dist):* These have been removed from this
         * esri version of the ngs code* it, iteration count* sigma, spherical distance
         * on auxiliary sphere* lam_sph, longitude difference on auxiliary sphere* kind,
        ...
        if (PE_ZERO(e2)) /* Sphere */
        {
            ...
            cos_phi1 = StrictMath.cos(phi1);
            cos_phi2 = StrictMath.cos(phi2);
            sin_phi1 = StrictMath.sin(phi1);
            sin_phi2 = StrictMath.sin(phi2);

            if (p_dist != null) {
                tem1 = StrictMath.sin((phi2 - phi1) / 2.0);
                tem2 = StrictMath.sin(dlam / 2.0);
                sigma = 2.0 * StrictMath.asin(StrictMath.sqrt(tem1 * tem1 + cos_phi1 * cos_phi2 * tem2 * tem2));
                p_dist.val = sigma * a;
            }
            ...
            if (p_az12 != null) {
                if (PE_EQ(PE_ABS(phi1), PE_PI2)) /* Origin at N or S Pole */
                {
                    ...
                    p_az12.val = StrictMath.atan2(cos_phi2 * StrictMath.sin(dlam),
                            cos_phi1 * sin_phi2 - sin_phi1 * cos_phi2 * StrictMath.cos(dlam));
                }
            }
            ...
            if (p_az21 != null) {
                if (PE_EQ(PE_ABS(phi2), PE_PI2)) /* Destination at N or S Pole */
                {
                    ...
                    p_az21.val = StrictMath.atan2(cos_phi1 * StrictMath.sin(dlam),
                            sin_phi2 * cos_phi1 * StrictMath.cos(dlam) - cos_phi2 * sin_phi1);
        ...
        f = 1.0 - StrictMath.sqrt(1.0 - e2);
        boa = 1.0 - f;

        eta1 = StrictMath.atan(boa * StrictMath.tan(phi1)); /* better reduced latitude */
        sin_eta1 = StrictMath.sin(eta1);
        cos_eta1 = StrictMath.cos(eta1);

        eta2 = StrictMath.atan(boa * StrictMath.tan(phi2)); /* better reduced latitude */
        sin_eta2 = StrictMath.sin(eta2);
        cos_eta2 = StrictMath.cos(eta2);
        ...
        while (q_continue_looping && it < 100) {
            ...
            if (kind == 1) {
                sin_lam_sph = StrictMath.sin(lam_sph);

                /*
                 * if ( PE_ABS(PE_PI - PE_ABS(dlam)) < 2.0e-11 ) sin_lam_sph = 0.0
                 * no--troublesome
                 */

                cos_lam_sph = StrictMath.cos(lam_sph);
                tem1 = cos_eta2 * sin_lam_sph;
                temp = cos_eta1 * sin_eta2 - sin_eta1 * cos_eta2 * cos_lam_sph;
                sin_sigma = StrictMath.sqrt(tem1 * tem1 + temp * temp); /*
                                                                         * v14 (Rapp 1.87)
                                                                         */
                cos_sigma = sin_eta1 * sin_eta2 + cos_eta1 * cos_eta2 * cos_lam_sph; /* v15 (Rapp 1.88) */
                sigma = StrictMath.atan2(sin_sigma, cos_sigma); /* (Rapp 1.89) */
            ...
            if (kind == 1) {
                ...
                if (PE_ABS(lam_sph) > PE_PI) {
                    ...
                    sigma = PE_PI - PE_ABS(StrictMath.atan(sin_eta1 / cos_eta1) + StrictMath.atan(sin_eta2 / cos_eta2));
                    sin_sigma = StrictMath.sin(sigma);
                    cos_sigma = StrictMath.cos(sigma);
                    c = ((-3.0 * cos2_azeq + 4.0) * f + 4.0) * cos2_azeq * f / 16.0; /* v10 (Rapp 1.83) */
            ...
            } else /* kind == 2 */
            {
                ...
                cos_lam_sph = -Math.sqrt(PE_ABS(1.0 - sin_lam_sph * sin_lam_sph));
                lam_sph = StrictMath.atan2(sin_lam_sph, cos_lam_sph);
                tem1 = cos_eta2 * sin_lam_sph;
                temp = cos_eta1 * sin_eta2 - sin_eta1 * cos_eta2 * cos_lam_sph;
                sin_sigma = StrictMath.sqrt(tem1 * tem1 + temp * temp);
                cos_sigma = sin_eta1 * sin_eta2 + cos_eta1 * cos_eta2 * cos_lam_sph;
                sigma = StrictMath.atan2(sin_sigma, cos_sigma);
                c = ((-3.0 * cos2_azeq + 4.0) * f + 4.0) * cos2_azeq * f / 16.0; /*
                                                                                  * v10 ( Rapp 1.83 )
                                                                                  */
                if (PE_ABS(sin_azeq - prev) < tol) {
        ...
        if (p_dist != null) {
            ...
            ep2 = 1.0 / (boa * boa) - 1.0;
            bige = StrictMath.sqrt(1.0 + ep2 * cos2_azeq); /* 15 */
            ...
            dsigma = bigb * sin_sigma * (costm + bigb / 4.0 * (cos_sigma * (-1.0 + 2.0 * costm2) - z)); /* 19 */
        ...
        if (p_az12 != null || p_az21 != null) {
            if (kind == 2) /* antipodal */
            {
                ...
                az21 = StrictMath.sqrt(1.0 - az12 * az12);
                if (temp < 0.0) {
                    az21 = -az21;
                }
                az12 = StrictMath.atan2(az12, az21);
                tem1 = -sin_azeq;
                tem2 = sin_eta1 * sin_sigma - cos_eta1 * cos_sigma * az21;
                az21 = StrictMath.atan2(tem1, tem2);
            } else /* long-line */
            {
                tem1 = cos_eta2 * sin_lam_sph;
                tem2 = cos_eta1 * sin_eta2 - sin_eta1 * cos_eta2 * cos_lam_sph;
                az12 = StrictMath.atan2(tem1, tem2);
