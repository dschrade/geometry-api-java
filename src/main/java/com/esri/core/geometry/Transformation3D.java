...
final public class Transformation3D {
     ...
     * The a, b, and result could point to same objects. <br>
     ...
     *            GeometryException("math singularity") exception if the Inverse can
     *            not be calculated.
     */
    public static void inverse(Transformation3D src, Transformation3D result) {
        double det = src.xx * (src.yy * src.zz - src.zy * src.yz) - src.yx * (src.xy * src.zz - src.zy * src.xz)
                + src.zx * (src.xy * src.yz - src.yy * src.xz);
        if (det != 0) {
            double xx, yx, zx;
            double xy, yy, zy;
            double xz, yz, zz;
