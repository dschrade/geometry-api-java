...
final class MathUtils {
    ...
    static final class KahanSummator {
        ...
        /**
         * initialize to the given start value. \param startValue_ The value to be added
        ...
        /**
         * Resets the accumulated sum to zero. The getResult() returns startValue_ after
         * this call.
         */
        void reset() {
            sum = 0;
    ...
    static double copySign(double x, double y) {
        return y >= 0.0 ? StrictMath.abs(x) : -StrictMath.abs(x);
    }
     ...
     * The interpolation formula is (end - start) * t + start. However, the
     * computation ensures that t = 0 produces exactly start, and t = 1, produces
     * exactly end. It also guarantees that for 0 <= t <= 1, the interpolated value
     * v is between start and end.
     */
    static double lerp(double start_, double end_, double t) {
        ...
        assert (t < 0 || t > 1.0 || (v >= start_ && v <= end_) || (v <= start_ && v >= end_)
                || NumberUtils.isNaN(start_) || NumberUtils.isNaN(end_));
        return v;
    }
    ...
    static void lerp(double start_x, double start_y, double end_x, double end_y, double t, Point2D result) {
        ...
        assert (t < 0 || t > 1.0 || (result.x >= start_x && result.x <= end_x)
                || (result.x <= start_x && result.x >= end_x));
        assert (t < 0 || t > 1.0 || (result.y >= start_y && result.y <= end_y)
                || (result.y <= start_y && result.y >= end_y));
    }
