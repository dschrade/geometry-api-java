...
class OperatorBufferCursor extends GeometryCursor {
    ...
    OperatorBufferCursor(GeometryCursor inputGeoms, SpatialReference sr, double[] distances, double max_deviation,
            int max_vertices, boolean b_union, ProgressTracker progress_tracker) {
        m_index = -1;
    ...
    Geometry buffer(Geometry geom, double distance) {
        return m_bufferer.buffer(geom, distance, m_Spatial_reference, m_max_deviation, m_max_vertices_in_full_circle,
                m_progress_tracker);
    }
}
