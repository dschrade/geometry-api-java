...
class OperatorImportFromWktLocal extends OperatorImportFromWkt {
    ...
    public OGCStructure executeOGC(int import_flags, String wkt_string, ProgressTracker progress_tracker) {
        ...
        while (wkt_parser.nextToken() != WktParser.WktToken.not_available) {
            ...
            if (current_token == WktParser.WktToken.geometrycollection) {
                ...
                if (current_token == WktParser.WktToken.attribute_z || current_token == WktParser.WktToken.attribute_m
            ...
            Geometry geometry = importFromWkt(import_flags, Geometry.Type.Unknown, wkt_parser);
    ...
    static Geometry polygonTaggedText(boolean b_multi_polygon, int import_flags, WktParser wkt_parser) {
        ...
        if (current_token == WktParser.WktToken.attribute_z) {
            zs = (AttributeStreamOfDbl) AttributeStreamBase.createDoubleStream(0, NumberUtils.TheNaN);
            multi_path_impl.addAttribute(VertexDescription.Semantics.Z);
            wkt_parser.nextToken();
        } else if (current_token == WktParser.WktToken.attribute_m) {
            ms = (AttributeStreamOfDbl) AttributeStreamBase.createDoubleStream(0, NumberUtils.TheNaN);
        ...
        if (point_count != 0) {
            ...
            multi_path_impl.setAttributeStreamRef(VertexDescription.Semantics.POSITION, position);
            multi_path_impl.setPathStreamRef(paths);
            multi_path_impl.setPathFlagsStreamRef(path_flags);

            if (zs != null)
                multi_path_impl.setAttributeStreamRef(VertexDescription.Semantics.Z, zs);

            if (ms != null)
                multi_path_impl.setAttributeStreamRef(VertexDescription.Semantics.M, ms);
        ...
        if ((import_flags & (int) WktImportFlags.wktImportNonTrusted) == 0)
            multi_path_impl.setIsSimple(MultiPathImpl.GeometryXSimple.Weak, 0.0, false);
    ...
    static Geometry multiPointTaggedText(int import_flags, WktParser wkt_parser) {
        ...
        if (point_count != 0) {
            ...
            multi_point_impl.setAttributeStreamRef(VertexDescription.Semantics.POSITION, position);

            if (zs != null)
                multi_point_impl.setAttributeStreamRef(VertexDescription.Semantics.Z, zs);

            if (ms != null)
                multi_point_impl.setAttributeStreamRef(VertexDescription.Semantics.M, ms);
    ...
    static int multiPolygonText(AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position,
            AttributeStreamOfInt32 paths, AttributeStreamOfInt8 path_flags, WktParser wkt_parser) {
    ...
    static int multiLineStringText(AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position,
            AttributeStreamOfInt32 paths, AttributeStreamOfInt8 path_flags, WktParser wkt_parser) {
    ...
    static int multiPointText(AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position,
        ...
        while (current_token != WktParser.WktToken.right_paren) {
            ...
            if (current_token == WktParser.WktToken.left_paren || current_token == WktParser.WktToken.empty)
    ...
    static int polygonText(AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position,
            AttributeStreamOfInt32 paths, AttributeStreamOfInt8 path_flags, int total_point_count,
            WktParser wkt_parser) {
        // At start of PolygonText
    ...
    static int lineStringText(boolean b_ring, AttributeStreamOfDbl zs, AttributeStreamOfDbl ms,
        ...
        while (current_token != WktParser.WktToken.right_paren) {
            ...
            if (b_ring && point_count >= 2 && current_token == WktParser.WktToken.right_paren) {
                ...
                if ((startx == x || (NumberUtils.isNaN(startx) && NumberUtils.isNaN(x)))
                        ...
                        && (!wkt_parser.hasZs() || startz == z || (NumberUtils.isNaN(startz) && NumberUtils.isNaN(z)))
                        && (!wkt_parser.hasMs() || startm == m || (NumberUtils.isNaN(startm) && NumberUtils.isNaN(m))))
                    b_add_point = false;
            }
    ...
    static int pointText(AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position,
            WktParser wkt_parser) {
        // At start of PointText
    ...
    static void addToStreams(AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position, double x,
