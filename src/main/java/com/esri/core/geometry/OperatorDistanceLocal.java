...
class OperatorDistanceLocal extends OperatorDistance {
    ...
    class DistanceCalculator {
        ...
        private double executeBruteForce_(/* const */Geometry geometryA, /* const */
            ...
            boolean geometriesAreDisjoint = !m_env2DgeometryA.isIntersecting(m_env2DgeometryB);
            if (Geometry.isMultiPath(geometryA.getType().value())
                ...
                if (((MultiPath) geometryA).getPointCount() > ((MultiPath) geometryB).getPointCount())
                    return bruteForceMultiPathMultiPath_((MultiPath) geometryA, (MultiPath) geometryB,
                            geometriesAreDisjoint);
                swapEnvelopes_();
                double answer = bruteForceMultiPathMultiPath_((MultiPath) geometryB, (MultiPath) geometryA,
                        geometriesAreDisjoint);
                swapEnvelopes_();
                return answer;
                    ...
                    && Geometry.isMultiPath(geometryB.getType().value())) { // MultiPoint
                                                                            ...
                                                                            // MultiPath
                swapEnvelopes_();
                double answer = bruteForceMultiPathMultiPoint_((MultiPath) geometryB, (MultiPoint) geometryA,
                        geometriesAreDisjoint);
                swapEnvelopes_();
                return answer;
                    ...
                    && Geometry.isMultiPath(geometryA.getType().value())) { // MultiPath
                                                                            // vs.
                                                                            // MultiPoint
                return bruteForceMultiPathMultiPoint_((MultiPath) geometryA, (MultiPoint) geometryB,
                        geometriesAreDisjoint);
            } else if (geometryA.getType() == Geometry.Type.MultiPoint
                ...
                if (((MultiPoint) geometryA).getPointCount() > ((MultiPoint) geometryB).getPointCount())
                    return bruteForceMultiPointMultiPoint_((MultiPoint) geometryA, (MultiPoint) geometryB,
                            geometriesAreDisjoint);
                swapEnvelopes_();
                double answer = bruteForceMultiPointMultiPoint_((MultiPoint) geometryB, (MultiPoint) geometryA,
                        geometriesAreDisjoint);
                swapEnvelopes_();
                return answer;
            }
        ...
        private double bruteForceMultiPathMultiPath_(/* const */MultiPath geometryA, /* const */MultiPath geometryB,
            ...
            if (!geometriesAreDisjoint) {
                ...
                if (weakIntersectionTest_(geometryA, geometryB, segIterA, segIterB))
                    return 0.0;
            }
            ...
            return StrictMath.sqrt(minSqrDistance);
        }

        private double bruteForceMultiPathMultiPoint_(/* const */MultiPath geometryA, /* const */
            ...
            /* const */MultiPointImpl multiPointImplB = (MultiPointImpl) geometryB._getImpl();
            int pointCountB = multiPointImplB.getPointCount();
            boolean bDoPiPTest = !geometriesAreDisjoint && (geometryA.getType() == Geometry.Type.Polygon);
            ...
            while (segIterA.nextPath()) {
                while (segIterA.hasNextSegment()) {
                    ...
                    if (pointCountB > 1 && env2DSegmentA.sqrDistance(m_env2DgeometryB) > minSqrDistance)
                        continue;
                    ...
                    for (int i = 0; i < pointCountB; i++) {
                        ...
                        if (bDoPiPTest) {
                            ...
                            if (PolygonUtils.isPointInPolygon2D((Polygon) geometryA, inputPoint,
                                    0) != PolygonUtils.PiPResult.PiPOutside)
                                return 0.0;
                        }
        ...
        private double bruteForceMultiPointMultiPoint_(/* const */MultiPoint geometryA, /* const */
                MultiPoint geometryB, boolean geometriesAreDisjoint) {
            double minSqrDistance = NumberUtils.doubleMax();
            ...
            double sqrDistance = minSqrDistance;
            /* const */MultiPointImpl multiPointImplA = (/* const */MultiPointImpl) geometryA._getImpl();
            /* const */MultiPointImpl multiPointImplB = (/* const */MultiPointImpl) geometryB._getImpl();
            int pointCountA = multiPointImplA.getPointCount();
            int pointCountB = multiPointImplB.getPointCount();
            for (int i = 0; i < pointCountA; i++) {
                multiPointImplA.getXY(i, pointA);

                if (pointCountB > 1 && m_env2DgeometryB.sqrDistance(pointA) > minSqrDistance)
                    continue;
            ...
            if (geometryA.getType() == Geometry.Type.Polygon) {
                ...
                while (segIterB.nextPath()) {
                    if (segIterB.hasNextSegment()) {
                        /* const */Segment segmentB = segIterB.nextSegment();
                        if (PolygonUtils.isPointInPolygon2D((Polygon) geometryA, segmentB.getEndXY(),
                                0) != PolygonUtils.PiPResult.PiPOutside)
                            return true;
                    }
                }
            ...
            if (geometryB.getType() == Geometry.Type.Polygon) {
                ...
                while (segIterA.nextPath()) {
                    if (segIterA.hasNextSegment()) {
                        /* const */Segment segmentA = segIterA.nextSegment();
                        if (PolygonUtils.isPointInPolygon2D((Polygon) geometryB, segmentA.getEndXY(),
                                0) != PolygonUtils.PiPResult.PiPOutside)
                            return true;
                    }
                }
