...
final class IntervalTreeImpl {
    ...
    /*
     * Resets the Interval_tree_impl to an empty state, but maintains a handle on
     ...
     * Interval_tree_impl but with a new query, use the reset_iterator function on
     * the Interval_tree_iterator_impl. \param query The Envelope_1D interval used
     * for the query. \param tolerance The tolerance used for the intersection
     * tests.
     */
     ...
     * stabbing query. To reuse the existing iterator on the same Interval_tree_impl
     * but with a new query, use the reset_iterator function on the
     * Interval_tree_iterator_impl. \param query The double used for the stabbing
     * query. \param tolerance The tolerance used for the intersection tests.
     */
    ...
    private StridedIndexTypeCollection m_tertiary_nodes; // 5 elements for offline dynamic case, 4 elements for static
                                                         // case
    ...
    /*
     * m_tertiary_nodes 0: m_discriminant_index_1 1: m_secondary 2: m_lptr 3: m_rptr
     * 4: m_pptr
     */
    ...
    private void insertIntervalsStatic_() {
        ...
        m_interval_nodes.setCapacity(size); // one for each interval being inserted. each element contains a tertiary
                                            // node, a left secondary node, and a right secondary node.
        m_secondary_lists.reserveNodes(2 * size); // one for each end point of the original interval set (not the unique
                                                  // set)
    ...
    private int insertIntervalEnd_(int end_index, int root) {
        ...
        while (bSearching) {
            ...
            if (ptr == -1 || discriminant_index_1 != getDiscriminantIndex1_(ptr)) {
                ...
                assert (getLPTR_(tertiary_handle) == -1 && getRPTR_(tertiary_handle) == -1
                        && (!m_b_offline_dynamic || getPPTR_(tertiary_handle) == -1));
    ...
    static final class IntervalTreeIteratorImpl {
        ...
        private boolean right_() {
            ...
            if (m_current_end_handle != -1 && IntervalTreeImpl.isRight_(getCurrentEndIndex_())
