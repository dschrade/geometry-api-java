...
public final class Point2D implements Serializable {
    ...
    public boolean isEqual(Point2D other, double tol) {
        return (StrictMath.abs(x - other.x) <= tol) && (StrictMath.abs(y - other.y) <= tol);
    }
    ...
    public static double distance(Point2D pt1, Point2D pt2) {
        return StrictMath.sqrt(sqrDistance(pt1, pt2));
    }
    ...
    double _dotProductAbs(Point2D other) {
        return StrictMath.abs(x * other.x) + StrictMath.abs(y * other.y);
    }
    ...
    /**
     * 270 degree rotation, anticlockwise. Equivalent to RotateDirect(-cos(pi/2),
     * sin(-pi/2)).
     */
     ...
     * between vectors (1,0) and (0, 1), second between (0, 1) and (-1, 0), etc. The
     * quarters are numbered counterclockwise. Angle intervals corresponding to
     * quarters: 1 : [0 : 90); 2 : [90 : 180); 3 : [180 : 270); 4 : [270 : 360)
     */
    ...
    /**
     * Assume vector v1 and v2 have same origin. The function compares the vectors
     * by angle in the counter clockwise direction from the axis X.
     * 
     * For example, V1 makes 30 degree angle counterclockwise from horizontal x axis
     * V2, makes 270, V3 makes 90, then compareVectors(V1, V2) == -1.
     * compareVectors(V1, V3) == -1. compareVectors(V2, V3) == 1.
     * 
    ...
    double _norm(int metric) {
        ...
        switch (metric) {
        case 0: // L-infinite
            return StrictMath.abs(x) >= StrictMath.abs(y) ? StrictMath.abs(x) : StrictMath.abs(y);

        case 1: // L1 or Manhattan metric
            return StrictMath.abs(x) + StrictMath.abs(y);

        case 2: // L2 or Euclidean metric
            return StrictMath.sqrt(x * x + y * y);

        default:
            return Math.pow(StrictMath.pow(x, (double) metric) + StrictMath.pow(y, (double) metric),
                    1.0 / (double) metric);
        }
    }
     ...
     * right-hand side of the line, negative otherwise. If the two input points are
     * equal, the (positive) distance of this point to p_1 is returned.
     */
    ...
    /**
     * Calculates the orientation of the triangle formed by p, q, r. Returns 1 for
     * counter-clockwise, -1 for clockwise, and 0 for collinear. May use high
     * precision arithmetics for some special degenerate cases.
     */
    ...
    /**
     * Calculates if the point s is inside of the circumcircle inscribed by the
     * clockwise oriented triangle p-q-r. Returns 1 for outside, -1 for inside, and
     * 0 for cocircular. Note that the convention used here differs from what is
     * commonly found in literature, which can define the relation in terms of a
     * counter-clockwise oriented circle, and this flips the sign (think of the
     * signed volume of the tetrahedron). May use high precision arithmetics for
     * some special cases.
     */
    ...
    private static Point2D calculateCenterFromThreePointsHelper_(Point2D from, Point2D mid_point, Point2D to) {
        ...
        double base = r1 + StrictMath.abs(from.x) + StrictMath.abs(mid_point.x) + StrictMath.abs(to.x)
                + StrictMath.abs(from.y) + StrictMath.abs(mid_point.y) + StrictMath.abs(to.y);

        double tol = 1e-15;
        if ((StrictMath.abs(r1 - r2) <= base * tol && StrictMath.abs(r1 - r3) <= base * tol))
            return center;// returns center value for MP_value type or when calculated radius value for
                          // from - center, mid - center, and to - center are very close.
