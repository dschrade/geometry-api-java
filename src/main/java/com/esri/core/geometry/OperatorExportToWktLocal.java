...
class OperatorExportToWktLocal extends OperatorExportToWkt {
    ...
    static void exportToWkt(int export_flags, Geometry geometry, StringBuilder string) {
        ...
        case Geometry.GeometryType.Polygon:
            if ((export_flags & WktExportFlags.wktExportLineString) != 0
                ...
                throw new IllegalArgumentException(
                        "Cannot export a Polygon as (Multi)LineString/(Multi)Point : " + export_flags);
        ...
        case Geometry.GeometryType.Polyline:
            if ((export_flags & WktExportFlags.wktExportPolygon) != 0
                    ...
                    || (export_flags & WktExportFlags.wktExportMultiPoint) != 0)
                throw new IllegalArgumentException(
                        "Cannot export a Polyline as (Multi)Polygon/(Multi)Point : " + export_flags);
        ...
        case Geometry.GeometryType.MultiPoint:
            if ((export_flags & WktExportFlags.wktExportLineString) != 0
                    ...
                    || (export_flags & WktExportFlags.wktExportMultiPolygon) != 0)
                throw new IllegalArgumentException(
                        "Cannot export a MultiPoint as (Multi)LineString/(Multi)Polygon: " + export_flags);
        ...
        case Geometry.GeometryType.Point:
            if ((export_flags & WktExportFlags.wktExportLineString) != 0
                    ...
                    || (export_flags & WktExportFlags.wktExportMultiPolygon) != 0)
                throw new IllegalArgumentException(
                        "Cannot export a Point as (Multi)LineString/(Multi)Polygon: " + export_flags);
        ...
        case Geometry.GeometryType.Envelope:
            if ((export_flags & WktExportFlags.wktExportLineString) != 0
                    ...
                    || (export_flags & WktExportFlags.wktExportMultiPoint) != 0)
                throw new IllegalArgumentException(
                        "Cannot export an Envelope as (Multi)LineString/(Multi)Point: " + export_flags);
    ...
    static void exportPolygonToWkt(int export_flags, Polygon polygon, StringBuilder string) {
        ...
        if (point_count > 0) {
            ...
            if (b_export_zs) {
                if (polygon_impl._attributeStreamIsAllocated(VertexDescription.Semantics.Z))
                    zs = (AttributeStreamOfDbl) polygon_impl.getAttributeStreamRef(VertexDescription.Semantics.Z);
            }

            if (b_export_ms) {
                if (polygon_impl._attributeStreamIsAllocated(VertexDescription.Semantics.M))
                    ms = (AttributeStreamOfDbl) polygon_impl.getAttributeStreamRef(VertexDescription.Semantics.M);
            }
        }
        ...
        if ((export_flags & WktExportFlags.wktExportPolygon) != 0) {
            if (polygon_count > 1)
                throw new IllegalArgumentException(
                        "Cannot export a Polygon with specified export flags: " + export_flags);

            polygonTaggedText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths, path_count,
                    string);
        } else {
            multiPolygonTaggedText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths,
                    polygon_count, path_count, string);
        }
    }
    ...
    static void exportPolylineToWkt(int export_flags, Polyline polyline, StringBuilder string) {
        ...
        if (point_count > 0) {
            position = (AttributeStreamOfDbl) polyline_impl.getAttributeStreamRef(VertexDescription.Semantics.POSITION);
            path_flags = polyline_impl.getPathFlagsStreamRef();
            paths = polyline_impl.getPathStreamRef();

            if (b_export_zs) {
                if (polyline_impl._attributeStreamIsAllocated(VertexDescription.Semantics.Z))
                    zs = (AttributeStreamOfDbl) (polyline_impl.getAttributeStreamRef(VertexDescription.Semantics.Z));
            }

            if (b_export_ms) {
                if (polyline_impl._attributeStreamIsAllocated(VertexDescription.Semantics.M))
                    ms = (AttributeStreamOfDbl) (polyline_impl.getAttributeStreamRef(VertexDescription.Semantics.M));
            }
        }
        ...
        if ((export_flags & WktExportFlags.wktExportLineString) != 0) {
            if (path_count > 1)
                throw new IllegalArgumentException(
                        "Cannot export a LineString with specified export flags: " + export_flags);

            lineStringTaggedText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths, string);
        } else {
            multiLineStringTaggedText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths,
                    path_count, string);
        }
    }
    ...
    static void exportMultiPointToWkt(int export_flags, MultiPoint multipoint, StringBuilder string) {
        ...
        if (point_count > 0) {
            ...
            if (b_export_zs) {
                if (multipoint_impl._attributeStreamIsAllocated(VertexDescription.Semantics.Z))
                    zs = (AttributeStreamOfDbl) (multipoint_impl.getAttributeStreamRef(VertexDescription.Semantics.Z));
            }

            if (b_export_ms) {
                if (multipoint_impl._attributeStreamIsAllocated(VertexDescription.Semantics.M))
                    ms = (AttributeStreamOfDbl) (multipoint_impl.getAttributeStreamRef(VertexDescription.Semantics.M));
            }
        }
        ...
        if ((export_flags & WktExportFlags.wktExportPoint) != 0) {
            if (point_count > 1)
                throw new IllegalArgumentException(
                        "Cannot export a Point with specified export flags: " + export_flags);
    ...
    static void exportEnvelopeToWkt(int export_flags, Envelope envelope, StringBuilder string) {
        ...
        if ((export_flags & WktExportFlags.wktExportMultiPolygon) != 0) {
            multiPolygonTaggedTextFromEnvelope_(precision, b_export_zs, b_export_ms, xmin, ymin, xmax, ymax, zmin, zmax,
                    mmin, mmax, string);
        } else {
            polygonTaggedTextFromEnvelope_(precision, b_export_zs, b_export_ms, xmin, ymin, xmax, ymax, zmin, zmax,
                    mmin, mmax, string);
        }
    }

    static void multiPolygonTaggedText_(int precision, boolean b_export_zs, boolean b_export_ms,
        ...
        multiPolygonText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths, polygon_count,
                path_count, string);

        string.append(')');
    }

    static void multiPolygonTaggedTextFromEnvelope_(int precision, boolean b_export_zs, boolean b_export_ms,
        ...
        writeEnvelopeAsWktPolygon_(precision, b_export_zs, b_export_ms, xmin, ymin, xmax, ymax, zmin, zmax, mmin, mmax,
                string);

        string.append(')');
    }

    static void multiLineStringTaggedText_(int precision, boolean b_export_zs, boolean b_export_ms,
            AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position,
            AttributeStreamOfInt8 path_flags, AttributeStreamOfInt32 paths, int path_count, StringBuilder string) {
        ...
        multiLineStringText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths, path_count,
                string);

        string.append(')');
    }

    static void multiPointTaggedText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, int point_count, StringBuilder string) {
    ...
    static void multiPointTaggedTextFromPoint_(int precision, boolean b_export_zs, boolean b_export_ms, double x,
            double y, double z, double m, StringBuilder string) {
        string.append("MULTIPOINT ");
    ...
    static void polygonTaggedText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, AttributeStreamOfInt8 path_flags,
            AttributeStreamOfInt32 paths, int path_count, StringBuilder string) {
    ...
    static void polygonTaggedTextFromEnvelope_(int precision, boolean b_export_zs, boolean b_export_ms, double xmin,
            double ymin, double xmax, double ymax, double zmin, double zmax, double mmin, double mmax,
            StringBuilder string) {
        string.append("POLYGON ");
    ...
    static void lineStringTaggedText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, AttributeStreamOfInt8 path_flags,
            AttributeStreamOfInt32 paths, StringBuilder string) {
    ...
    static void pointTaggedTextFromMultiPoint_(int precision, boolean b_export_zs, boolean b_export_ms,
            AttributeStreamOfDbl zs, AttributeStreamOfDbl ms, AttributeStreamOfDbl position, StringBuilder string) {
        string.append("POINT ");
    ...
    static void multiPolygonText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, AttributeStreamOfInt8 path_flags,
            AttributeStreamOfInt32 paths, int polygon_count, int path_count, StringBuilder string) {
        int polygon_start = 0;
        int polygon_end = 1;

        while (polygon_end < path_count && (path_flags.read(polygon_end) & PathFlags.enumOGCStartPolygon) == 0)
            polygon_end++;

        polygonText_(precision, b_export_zs, b_export_ms, zs, ms, position, path_flags, paths, polygon_start,
                polygon_end, string);
    ...
    static void multiLineStringText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, AttributeStreamOfInt8 path_flags,
            AttributeStreamOfInt32 paths, int path_count, StringBuilder string) {
        boolean b_closed = ((path_flags.read(0) & PathFlags.enumClosed) != 0);
        ...
        for (int path = 1; path < path_count; path++) {
            ...
            lineStringText_(false, b_closed, precision, b_export_zs, b_export_ms, zs, ms, position, paths, path,
                    string);
        }
    }

    static void multiPointText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, int point_count, StringBuilder string) {
        pointText_(precision, b_export_zs, b_export_ms, zs, ms, position, 0, string);
    ...
    static void polygonText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, AttributeStreamOfInt8 path_flags,
            AttributeStreamOfInt32 paths, int polygon_start, int polygon_end, StringBuilder string) {
        string.append('(');

        lineStringText_(true, true, precision, b_export_zs, b_export_ms, zs, ms, position, paths, polygon_start,
                string);
    ...
    static void lineStringText_(boolean bRing, boolean b_closed, int precision, boolean b_export_zs,
    ...
    static int pointText_(int precision, boolean b_export_zs, boolean b_export_ms, double x, double y, double z,
            double m, StringBuilder string) {
        string.append('(');
    ...
    static void pointText_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, int point, StringBuilder string) {
        ...
        if (b_export_zs)
            z = (zs != null ? zs.read(point) : VertexDescription.getDefaultValue(VertexDescription.Semantics.Z));

        if (b_export_ms)
            m = (ms != null ? ms.read(point) : VertexDescription.getDefaultValue(VertexDescription.Semantics.M));

        pointText_(precision, b_export_zs, b_export_ms, x, y, z, m, string);
    }

    static void point_(int precision, boolean b_export_zs, boolean b_export_ms, double x, double y, double z, double m,
            StringBuilder string) {
    ...
    static void point_(int precision, boolean b_export_zs, boolean b_export_ms, AttributeStreamOfDbl zs,
            AttributeStreamOfDbl ms, AttributeStreamOfDbl position, int point, StringBuilder string) {
        double x = position.read(2 * point);
        double y = position.read(2 * point + 1);
        double z = NumberUtils.TheNaN;
    ...
    static void writeEnvelopeAsWktPolygon_(int precision, boolean b_export_zs, boolean b_export_ms, double xmin,
            double ymin, double xmax, double ymax, double zmin, double zmax, double mmin, double mmax,
            StringBuilder string) {
        string.append("((");
