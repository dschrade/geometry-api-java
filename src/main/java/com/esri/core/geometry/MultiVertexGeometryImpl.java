...
abstract class MultiVertexGeometryImpl extends MultiVertexGeometry {
    ...
    /**
     * \internal Called inside of the VerifyAllStreams to get a child class a chance
     * to do additional verify.
     */
    ...
    public interface DirtyFlags {
        ...
        /**
         * when DirtyIsKnownSimple is 0, this flag indicates whether the geometry is
         * weak simple or not
         */
        int IsWeakSimple = 2;
        ...
        int DirtyIntervals = DirtyExactIntervals | DirtyLooseIntervals;
        ...
        int DirtyCoordinates = DirtyIsKnownSimple | DirtyIntervals | DirtyIsEnvelope | DirtyLength2D | DirtyRingAreas2D
                | DirtyOGCFlags;
     ...
     * The stream can have more elements, than the Geometry point count, but only
     * necessary part will be saved when exporting to a ESRI shape or other
     ...
     * reference. If one changes the stream later through the reference, one has to
     * call NotifyStreamChanged. \exception Throws invalid_argument exception if the
     * input stream type does not match that of the semantics persistence.
     */
    public void setAttributeStreamRef(int semantics, AttributeStreamBase stream) {
        ...
        if ((stream != null) && VertexDescription.getPersistence(semantics) != stream.getPersistence())// input stream
                                                                                                       // has wrong
                                                                                                       // persistence
            throw new IllegalArgumentException();
        ...
        if (m_vertexAttributes == null)
            m_vertexAttributes = new AttributeStreamBase[m_description.getAttributeCount()];
    ...
    // Checked vs. Jan 11, 2011
    /**
     * \internal Calculates loose envelope. Returns True if the calculation renders
     * exact envelope.
     */
    ...
    public int hashCode() {
        ...
        if (!isEmptyImpl()) {
            ...
            for (int i = 0, n = m_description.getAttributeCount(); i < n; i++) {
                int components = VertexDescription.getComponentCount(m_description._getSemanticsImpl(i));
    ...
    public boolean equals(Object other) {
        ...
        for (int i = 0; i < m_description.getAttributeCount(); i++) {
            ...
            AttributeStreamBase streamOther = otherMulti.getAttributeStreamRef(semantics);
    ...
    // Checked vs. Jan 11, 2011
    /**
     * Sets the envelope of the Geometry. The Envelope description must match that
     * of the Geometry.
     */
    ...
    // Checked vs. Jan 11, 2011
    /**
     * Notifies the Geometry of changes made to the vertices so that it could reset
     * cached structures.
     */
    ...
    /**
     * @param bExact
     *            True, when the exact envelope need to be calculated and false for
     *            the loose one.
     */
            ...
            for (int attributeIndex = 0; attributeIndex < m_description.getAttributeCount(); attributeIndex++) {
                ...
                if (m_vertexAttributes[attributeIndex] != null) {
                    ...
                    if (size < m_pointCount) {
                        size = (m_reservedPointCount > m_pointCount + 5) ? (m_pointCount * 5 + 3) / 4 : m_pointCount;// reserve
                                                                                                                     // 25%
                                                                                                                     // more
                                                                                                                     // than
                                                                                                                     // user
                    ...
                    m_vertexAttributes[attributeIndex] = AttributeStreamBase
                            .createAttributeStreamWithSemantics(semantics, m_pointCount);
    ...
    int queryCoordinates(Point2D[] dst, int dstSize, int beginIndex, int endIndex) {
        ...
        endIndexC = StrictMath.min(endIndexC, beginIndex + dstSize);
    ...
    int QueryCoordinates(Point3D[] dst, int dstSize, int beginIndex, int endIndex) {
        ...
        double v = VertexDescription.getDefaultValue(VertexDescription.Semantics.Z);
    ...
    public abstract boolean _buildRasterizedGeometryAccelerator(double toleranceXY,
            GeometryAccelerationDegree accelDegree);

    public abstract boolean _buildQuadTreeAccelerator(GeometryAccelerationDegree d);
