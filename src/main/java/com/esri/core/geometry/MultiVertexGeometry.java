...
public abstract class MultiVertexGeometry extends Geometry implements Serializable {
     ...
     *            The attributes, that are present in the pointSrc and missing in
     *            this Geometry, will be added to the Geometry. <br>
     *            The vertex attributes missing in the pointSrc but present in the
     *            Geometry will be set to the default values (see
    ...
    /**
     * Sets XY coordinates of the given vertex of the Geometry. All other attributes
     * are unchanged.
     */
    public abstract void setXY(int index, Point2D pt);

    /**
     * Returns XYZ coordinates of the given vertex of the Geometry. If the Geometry
     * has no Z's, the default value for Z is returned (0).
     */
    abstract Point3D getXYZ(int index);

    /**
     * Sets XYZ coordinates of the given vertex of the Geometry. If Z attribute is
     * not present in this Geometry, it is added. All other attributes are
     * unchanged.
     */
     ...
     * @param ordinate
     *            is the ordinate of a vertex attribute (for example, y has ordinate
     *            of 1, because it is second ordinate of POSITION)
     * 
     *            If attribute is not present, the default value is returned. See
     *            VertexDescription::GetDefaultValue() method.
     */
     ...
     * 
     *            If attribute is not present, the default value is returned. See
     *            VertexDescription::GetDefaultValue() method. Avoid using this
     *            method on non-integer atributes.
     */
     ...
     * @param ordinate
     *            is the ordinate of a vertex attribute (for example, y has ordinate
     *            of 1, because it is seond ordinate of POSITION)
     * @param value
     *            is the value to set. as well as the number of components of the
     *            attribute.
     * 
    ...
    /**
     * Same as above, but works with ints. Avoid using this method on non-integer
     * atributes because some double attributes may have NaN default values (e.g.
     * Ms)
     */
