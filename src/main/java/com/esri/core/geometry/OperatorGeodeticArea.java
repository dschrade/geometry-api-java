...
abstract class OperatorGeodeticArea extends Operator {
     ...
     * 
     * @param geoms
     *            The geometry cursor to be iterated over to perform the Geodetic
     *            Area calculation.
     * @param sr
     ...
     */
    public abstract double[] execute(GeometryCursor geoms, SpatialReference sr, int geodeticCurveType,
     ...
     */
    public abstract double execute(Geometry geom, SpatialReference sr, int geodeticCurveType,
            ProgressTracker progressTracker);

    public static OperatorGeodeticArea local() {
        return (OperatorGeodeticArea) OperatorFactoryLocal.getInstance().getOperator(Type.GeodeticArea);
    }
