...
class RelationalOperationsMatrix {
    ...
    // string.
    static boolean relate(Geometry geometry_a, Geometry geometry_b, SpatialReference sr, String scl,
        ...
        case Geometry.GeometryType.Polygon:
            switch (typeB) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelatePolygon_((Polygon) (_geometryA), (Polygon) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polygonRelatePolyline_((Polygon) (_geometryA), (Polyline) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = polygonRelatePoint_((Polygon) (_geometryA), (Point) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = polygonRelateMultiPoint_((Polygon) (_geometryA), (MultiPoint) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;
        ...
        case Geometry.GeometryType.Polyline:
            switch (typeB) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelatePolyline_((Polygon) (_geometryB), (Polyline) (_geometryA), tolerance,
                        getTransposeMatrix_(scl), progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelatePolyline_((Polyline) (_geometryA), (Polyline) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = polylineRelatePoint_((Polyline) (_geometryA), (Point) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = polylineRelateMultiPoint_((Polyline) (_geometryA), (MultiPoint) (_geometryB), tolerance,
        ...
        case Geometry.GeometryType.Point:
            switch (typeB) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelatePoint_((Polygon) (_geometryB), (Point) (_geometryA), tolerance,
                        getTransposeMatrix_(scl), progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelatePoint_((Polyline) (_geometryB), (Point) (_geometryA), tolerance,
                        getTransposeMatrix_(scl), progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = pointRelatePoint_((Point) (_geometryA), (Point) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = multiPointRelatePoint_((MultiPoint) (_geometryB), (Point) (_geometryA), tolerance,
                        getTransposeMatrix_(scl), progress_tracker);
                break;
        ...
        case Geometry.GeometryType.MultiPoint:
            switch (typeB) {
            case Geometry.GeometryType.Polygon:
                bRelation = polygonRelateMultiPoint_((Polygon) (_geometryB), (MultiPoint) (_geometryA), tolerance,
                        getTransposeMatrix_(scl), progress_tracker);
                break;

            case Geometry.GeometryType.Polyline:
                bRelation = polylineRelateMultiPoint_((Polyline) (_geometryB), (MultiPoint) (_geometryA), tolerance,
                        getTransposeMatrix_(scl), progress_tracker);
                break;

            case Geometry.GeometryType.MultiPoint:
                bRelation = multiPointRelateMultiPoint_((MultiPoint) (_geometryA), (MultiPoint) (_geometryB), tolerance,
                        scl, progress_tracker);
                break;

            case Geometry.GeometryType.Point:
                bRelation = multiPointRelatePoint_((MultiPoint) (_geometryA), (Point) (_geometryB), tolerance, scl,
                        progress_tracker);
                break;
    ...
    static boolean polygonRelatePolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        ...
        if (!bRelationKnown) {
            ...
            int relation = RelationalOperations.tryRasterizedContainsOrDisjoint_(polygon_a, polygon_b, tolerance,
                    false);
    ...
    // The relation is based on the simplified-Polygon A containing Polygon B, which
    // may be non-simple.
    static boolean polygonContainsPolygon_(Polygon polygon_a, Polygon polygon_b, double tolerance,
            ProgressTracker progress_tracker) {
        ...
        if (!bRelationKnown) {
            // Quick rasterize test to see whether the the geometries are disjoint, or if
        ...
        // If the simplified Polygon A does not have interior, then it cannot contain
        // anything.
    ...
    // Returns true if the relation holds.
    static boolean polygonRelatePolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
        ...
        if (!bRelationKnown) {
            // Quick rasterize test to see whether the the geometries are
            // disjoint, or if one is contained in the other.
            int relation = RelationalOperations.tryRasterizedContainsOrDisjoint_(polygon_a, polyline_b, tolerance,
                    false);
    ...
    static boolean polygonContainsPolyline_(Polygon polygon_a, Polyline polyline_b, double tolerance,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_("T*****F**");
        ...
        if (!bRelationKnown) {
            ...
            if (relation == RelationalOperations.Relation.disjoint) {
                relOps.areaLineDisjointPredicates_(polygon_a, polyline_b); // passing polyline to get boundary
                                                                           // information
                bRelationKnown = true;
            } else if (relation == RelationalOperations.Relation.contains) {
                relOps.areaLineContainsPredicates_(polygon_a, polyline_b); // passing polyline to get boundary
                                                                           // information
                bRelationKnown = true;
            }
        }
    ...
    // Returns true if the relation holds
    static boolean polygonRelateMultiPoint_(Polygon polygon_a, MultiPoint multipoint_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
        ...
        if (!bRelationKnown) {
            // Quick rasterize test to see whether the the geometries are
            // disjoint, or if one is contained in the other.
            int relation = RelationalOperations.tryRasterizedContainsOrDisjoint_(polygon_a, multipoint_b, tolerance,
                    false);
    ...
    // Returns true if the relation holds.
    static boolean polylineRelatePolyline_(Polyline polyline_a, Polyline polyline_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
        ...
        if (!bRelationKnown) {
            // Quick rasterize test to see whether the the geometries are
            // disjoint, or if one is contained in the other.
            int relation = RelationalOperations.tryRasterizedContainsOrDisjoint_(polyline_a, polyline_b, tolerance,
                    false);
    ...
    // Returns true if the relation holds.
    static boolean polylineRelateMultiPoint_(Polyline polyline_a, MultiPoint multipoint_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
        ...
        if (!bRelationKnown) {
            // Quick rasterize test to see whether the the geometries are
            // disjoint, or if one is contained in the other.
            int relation = RelationalOperations.tryRasterizedContainsOrDisjoint_(polyline_a, multipoint_b, tolerance,
                    false);
    ...
    // Returns true if the relation holds.
    static boolean multiPointRelateMultiPoint_(MultiPoint multipoint_a, MultiPoint multipoint_b, double tolerance,
            String scl, ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
    ...
    // Returns true if the relation holds.
    static boolean polygonRelatePoint_(Polygon polygon_a, Point point_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
        ...
        if (!bRelationKnown) {
            PolygonUtils.PiPResult res = PolygonUtils.isPointInPolygon2D(polygon_a, pt_b, tolerance); // uses
                                                                                                      // accelerator
            ...
            if (res == PolygonUtils.PiPResult.PiPInside) {// polygon must have area
                ...
                if (area != 0) {
                    ...
                    relOps.m_matrix[MatrixPredicate.InteriorExterior] = (env.getHeight() == 0.0 && env.getWidth() == 0.0
                            ? -1
                            : 1);
                }
            } else {
    ...
    // Returns true if the relation holds.
    static boolean polylineRelatePoint_(Polyline polyline_a, Point point_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
        ...
        if (!bRelationKnown) {
            ...
            if (relOps.m_perform_predicates[MatrixPredicate.InteriorInterior]
                ...
                if (b_intersects) {
                    if (relOps.m_perform_predicates[MatrixPredicate.InteriorInterior]) {
                        ...
                        b_boundary_contains_point = !RelationalOperations.multiPointDisjointPointImpl_(boundary_a, pt_b,
            ...
            if (relOps.m_perform_predicates[MatrixPredicate.BoundaryExterior]) {
                if (boundary_a != null && boundary_a.isEmpty()) {
                    ...
                    if (b_boundary_contains_point_known && !b_boundary_contains_point) {
                        ...
                        boolean b_boundary_equals_point = RelationalOperations.multiPointEqualsPoint_(boundary_a,
                                point_b, tolerance, progress_tracker);
            ...
            if (relOps.m_perform_predicates[MatrixPredicate.InteriorExterior]) {
                ...
                if (b_has_length) {
                    ...
                    boolean b_interior_equals_point = RelationalOperations.multiPointEqualsPoint_(interior_a, point_b,
                            tolerance, progress_tracker);
    ...
    // Returns true if the relation holds.
    static boolean multiPointRelatePoint_(MultiPoint multipoint_a, Point point_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        RelationalOperationsMatrix relOps = new RelationalOperationsMatrix();
        relOps.resetMatrix_();
        relOps.setPredicates_(scl);
    ...
    // Returns true if the relation holds.
    static boolean pointRelatePoint_(Point point_a, Point point_b, double tolerance, String scl,
            ProgressTracker progress_tracker) {
        ...
        if (Geometry.isMultiPath(type)) {
            if (type == Geometry.GeometryType.Polygon) {
                ...
                if (area != 0) {
                    ...
                    matrix[MatrixPredicate.InteriorExterior] = (env.getHeight() == 0.0 && env.getWidth() == 0.0 ? 0
                            : 1);
                }
            } else {
                boolean b_has_length = ((Polyline) g_a).calculateLength2D() != 0;
                matrix[MatrixPredicate.InteriorExterior] = (b_has_length ? 1 : 0);
                matrix[MatrixPredicate.BoundaryExterior] = (Boundary.hasNonEmptyBoundary((Polyline) g_a, null) ? 0
                        : -1);
            }
        } else {
            matrix[MatrixPredicate.InteriorExterior] = 0;
    ...
    private static boolean equals_(String scl) {
        // Valid for all
        if (scl.charAt(0) == 'T' && scl.charAt(1) == '*' && scl.charAt(2) == 'F' && scl.charAt(3) == '*'
                && scl.charAt(4) == '*' && scl.charAt(5) == 'F' && scl.charAt(6) == 'F' && scl.charAt(7) == 'F'
                && scl.charAt(8) == '*')
            return true;
    ...
    private static boolean disjoint_(String scl) {
        if (scl.charAt(0) == 'F' && scl.charAt(1) == 'F' && scl.charAt(2) == '*' && scl.charAt(3) == 'F'
                && scl.charAt(4) == 'F' && scl.charAt(5) == '*' && scl.charAt(6) == '*' && scl.charAt(7) == '*'
                && scl.charAt(8) == '*')
            return true;
    ...
    private static boolean touches_(String scl, int dim_a, int dim_b) {
        ...
        if (!(dim_a == 2 && dim_b == 2)) {
            ...
            if (scl.charAt(0) == 'F' && scl.charAt(1) == '*' && scl.charAt(2) == '*' && scl.charAt(3) == 'T'
                    && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == '*' && scl.charAt(7) == '*'
                    && scl.charAt(8) == '*')
                return true;

            if (dim_a == 1 && dim_b == 1) {
                // Valid for Line-Line
                if (scl.charAt(0) == 'F' && scl.charAt(1) == 'T' && scl.charAt(2) == '*' && scl.charAt(3) == '*'
                        && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == '*' && scl.charAt(7) == '*'
                        && scl.charAt(8) == '*')
                    return true;
            }
        ...
        if (dim_b != 0) {
            if (scl.charAt(0) == 'F' && scl.charAt(1) == '*' && scl.charAt(2) == '*' && scl.charAt(3) == '*'
                    && scl.charAt(4) == 'T' && scl.charAt(5) == '*' && scl.charAt(6) == '*' && scl.charAt(7) == '*'
                    && scl.charAt(8) == '*')
                return true;
        }
    ...
    private static boolean crosses_(String scl, int dim_a, int dim_b) {
        if (dim_a > dim_b) {
            ...
            if (scl.charAt(0) == 'T' && scl.charAt(1) == '*' && scl.charAt(2) == '*' && scl.charAt(3) == '*'
                    && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == 'T' && scl.charAt(7) == '*'
                    && scl.charAt(8) == '*')
                return true;
        ...
        if (dim_a == 1 && dim_b == 1) {
            // Valid for Line-Line
            if (scl.charAt(0) == '0' && scl.charAt(1) == '*' && scl.charAt(2) == '*' && scl.charAt(3) == '*'
                    && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == '*' && scl.charAt(7) == '*'
                    && scl.charAt(8) == '*')
                return true;
        }
    ...
    private static boolean contains_(String scl) {
        // Valid for all
        if (scl.charAt(0) == 'T' && scl.charAt(1) == '*' && scl.charAt(2) == '*' && scl.charAt(3) == '*'
                && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == 'F' && scl.charAt(7) == 'F'
                && scl.charAt(8) == '*')
            return true;
    ...
    private static boolean overlaps_(String scl, int dim_a, int dim_b) {
        if (dim_a == dim_b) {
            if (dim_a != 1) {
                ...
                if (scl.charAt(0) == 'T' && scl.charAt(1) == '*' && scl.charAt(2) == 'T' && scl.charAt(3) == '*'
                        && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == 'T' && scl.charAt(7) == '*'
                        && scl.charAt(8) == '*')
                    return true;
            ...
            // Valid for Line-Line
            if (scl.charAt(0) == '1' && scl.charAt(1) == '*' && scl.charAt(2) == 'T' && scl.charAt(3) == '*'
                    && scl.charAt(4) == '*' && scl.charAt(5) == '*' && scl.charAt(6) == 'T' && scl.charAt(7) == '*'
                    && scl.charAt(8) == '*')
                return true;
        }
    ...
    private boolean areaAreaPredicates_(int half_edge, int id_a, int id_b) {
        ...
        if (m_perform_predicates[MatrixPredicate.InteriorExterior]) {
            interiorAreaExteriorArea_(half_edge, id_a, id_b, MatrixPredicate.InteriorExterior);
            bRelationKnown &= isPredicateKnown_(MatrixPredicate.InteriorExterior);
        }

        if (m_perform_predicates[MatrixPredicate.BoundaryInterior]) {
            interiorAreaBoundaryArea_(half_edge, id_b, MatrixPredicate.BoundaryInterior);
        ...
        if (m_perform_predicates[MatrixPredicate.BoundaryExterior]) {
            boundaryAreaExteriorArea_(half_edge, id_a, id_b, MatrixPredicate.BoundaryExterior);
        ...
        if (m_perform_predicates[MatrixPredicate.ExteriorBoundary]) {
            boundaryAreaExteriorArea_(half_edge, id_b, id_a, MatrixPredicate.ExteriorBoundary);
    ...
    private void areaAreaDisjointPredicates_(Polygon polygon_a, Polygon polygon_b) {
        ...
        areaGeomContainsOrDisjointPredicates_(polygon_a,
                m_perform_predicates[MatrixPredicate.InteriorExterior] ? MatrixPredicate.InteriorExterior : -1,
                m_scl.charAt(MatrixPredicate.InteriorExterior),
                m_perform_predicates[MatrixPredicate.BoundaryExterior] ? MatrixPredicate.BoundaryExterior : -1,
                m_scl.charAt(MatrixPredicate.BoundaryExterior));
        areaGeomContainsOrDisjointPredicates_(polygon_b,
                m_perform_predicates[MatrixPredicate.ExteriorInterior] ? MatrixPredicate.ExteriorInterior : -1,
                m_scl.charAt(MatrixPredicate.ExteriorInterior),
                m_perform_predicates[MatrixPredicate.ExteriorBoundary] ? MatrixPredicate.ExteriorBoundary : -1,
                m_scl.charAt(MatrixPredicate.ExteriorBoundary));
    }

    private void areaGeomContainsOrDisjointPredicates_(Polygon polygon, int matrix_interior, char c1,
            int matrix_boundary, char c2) {
        if (matrix_interior != -1 || matrix_boundary != -1) {
            boolean has_area = ((c1 != 'T' && c1 != 'F' && matrix_interior != -1)
                    || (c2 != 'T' && c2 != 'F' && matrix_boundary != -1) ? polygon.calculateArea2D() != 0 : true);
    ...
    private void areaAreaContainsPredicates_(Polygon polygon_b) {
        ...
        areaGeomContainsOrDisjointPredicates_(polygon_b,
                m_perform_predicates[MatrixPredicate.InteriorInterior] ? MatrixPredicate.InteriorInterior : -1,
                m_scl.charAt(MatrixPredicate.InteriorInterior),
                m_perform_predicates[MatrixPredicate.InteriorBoundary] ? MatrixPredicate.InteriorBoundary : -1,
                m_scl.charAt(MatrixPredicate.InteriorBoundary));
    ...
    private void areaLineDisjointPredicates_(Polygon polygon, Polyline polyline) {
        ...
        areaGeomContainsOrDisjointPredicates_(polygon,
                m_perform_predicates[MatrixPredicate.InteriorExterior] ? MatrixPredicate.InteriorExterior : -1,
                m_scl.charAt(MatrixPredicate.InteriorExterior),
                m_perform_predicates[MatrixPredicate.BoundaryExterior] ? MatrixPredicate.BoundaryExterior : -1,
                m_scl.charAt(MatrixPredicate.BoundaryExterior));
    ...
    private boolean lineLinePredicates_(int half_edge, int id_a, int id_b) {
        ...
        if (m_perform_predicates[MatrixPredicate.InteriorBoundary]) {
            interiorLineBoundaryLine_(half_edge, id_a, id_b, m_cluster_index_a, m_cluster_index_b,
        ...
        if (m_perform_predicates[MatrixPredicate.BoundaryInterior]) {
            interiorLineBoundaryLine_(half_edge, id_b, id_a, m_cluster_index_b, m_cluster_index_a,
    ...
    private void boundaryAreaBoundaryArea_(int half_edge, int id_a, int id_b) {
        ...
        if (m_matrix[MatrixPredicate.BoundaryBoundary] != 0) {
            if (m_topo_graph.getHalfEdgeUserIndex(m_topo_graph.getHalfEdgePrev(m_topo_graph.getHalfEdgeTwin(half_edge)),
                    m_visited_index) != 1) {
    ...
    private void exteriorAreaBoundaryLine_(int half_edge, int id_a, int id_b, int cluster_index_b) {
        ...
        if (m_topo_graph.getHalfEdgeUserIndex(m_topo_graph.getHalfEdgePrev(m_topo_graph.getHalfEdgeTwin(half_edge)),
            ...
            if ((clusterParentage & id_a) == 0) {
                ...
                if ((faceParentage & id_a) == 0) {
                    assert ((m_topo_graph.getHalfEdgeParentage(m_topo_graph.getHalfEdgeTwin(half_edge)) & id_a) == 0);
    ...
    // with the interior of Line B.
    private void interiorLineInteriorLine_(int half_edge, int id_a, int id_b, int cluster_index_a,
            int cluster_index_b) {
        if (m_matrix[MatrixPredicate.InteriorInterior] == 1)
            return;
                ...
                if ((clusterParentage & id_a) != 0 && (clusterParentage & id_b) != 0) {
                    ...
                    if ((index_a % 2 == 0) && (index_b % 2 == 0)) {
                        assert ((m_topo_graph.getClusterParentage(cluster) & id_a) != 0
                                && (m_topo_graph.getClusterParentage(cluster) & id_b) != 0);
                        m_matrix[MatrixPredicate.InteriorInterior] = 0;
                    }
                }
            }
    ...
    // with the boundary of Line B.
    private void interiorLineBoundaryLine_(int half_edge, int id_a, int id_b, int cluster_index_a, int cluster_index_b,
            int predicate) {
    ...
    // with the boundary of Line B.
    private void boundaryLineBoundaryLine_(int half_edge, int id_a, int id_b, int cluster_index_a,
            int cluster_index_b) {
        if (m_matrix[MatrixPredicate.BoundaryBoundary] == 0)
            return;
