...
final class AttributeStreamOfDbl extends AttributeStreamBase {
    ...
    public void resize(int newSize) {
        ...
        if (newSize <= m_size) {
            ...
            if (newSize > m_buffer.length) {
                int sz = (newSize < 64) ? StrictMath.max(newSize * 2, 4) : (newSize * 5) / 4;
     ...
     * interleaved x and y. The envelope will be set to empty if the pointCount is
     * zero.
     */
        ...
        if (bForward) {
            ...
            for (int i = 0; i < count; i += stride) {
                ...
                for (int s = 0; s < stride; s++) {
                    m_buffer[oldSize + i + s] = ((AttributeStreamOfDbl) src).m_buffer[start + n + s];
                }
            }
        }
    }
    ...
    @Override
    public void insertRange(int start, AttributeStreamBase src, int srcStart, int count, boolean bForward, int stride,
            int validSize) {
        if (m_bReadonly)
            throw new GeometryException("invalid_call");
        ...
        System.arraycopy(m_buffer, start, m_buffer, start + count, validSize - start);
        ...
        if (bForward) {
            System.arraycopy(((AttributeStreamOfDbl) src).m_buffer, srcStart, m_buffer, start, count);
        } else {
            int n = count;
            ...
            for (int i = 0; i < count; i += stride) {
                ...
                for (int s = 0; s < stride; s++) {
                    m_buffer[start + i + s] = ((AttributeStreamOfDbl) src).m_buffer[srcStart + n + s];
                }
            }
        }
    }
    ...
    public void insert(int index, Point2D point, int validSize) {
        ...
        System.arraycopy(m_buffer, index, m_buffer, index + 2, validSize - index);
    ...
    public void eraseRange(int index, int count, int validSize) {
        ...
        if (validSize - (index + count) > 0) {
            System.arraycopy(m_buffer, index + count, m_buffer, index, validSize - (index + count));
        }
        m_size -= count;
    }
    ...
    @Override
    public void writeRange(int startElement, int count, AttributeStreamBase _src, int srcStart, boolean bForward,
            int stride) {
