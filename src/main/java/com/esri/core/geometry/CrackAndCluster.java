...
final class CrackAndCluster {
    ...
    static boolean non_empty_points_need_to_cluster(double tolerance, Point pt1, Point pt2) {
        ...
        return Clusterer.isClusterCandidate_(pt1.getX(), pt1.getY(), pt2.getX(), pt2.getY(),
                MathUtils.sqr(tolerance_for_clustering));
    }
    ...
    private boolean _do() {
        ...
        // double toleranceCluster = m_tolerance * StrictMath.sqrt(2.0) * 1.00001;
        boolean bChanged = false;
        int max_iter = m_shape.getTotalPointCount() + 10 > 30 ? 1000
                : (m_shape.getTotalPointCount() + 10) * (m_shape.getTotalPointCount() + 10);
        int iter = 0;
        boolean has_point_features = m_shape.hasPointFeatures();
        for (;; iter++) {
            if (iter > max_iter)
                throw new GeometryException("Internal Error: max number of iterations exceeded");// too
                                                                                                 // many
                                                                                                 // iterations
            ...
            if (m_filter_degenerate_segments) {
                boolean bFiltered = (m_shape.filterClosePoints(tolerance_for_clustering, true, false) != 0); // remove
                                                                                                             // all
                // degenerate
                // segments.
                bChanged |= bFiltered;
            ...
            if (iter == 0 || has_point_features
                    || Cracker.needsCracking(true, m_shape, tolerance_for_needs_cracking, null, m_progressTracker)) {
