...
class OperatorIntersectionCursor extends GeometryCursor {
    ...
    OperatorIntersectionCursor(GeometryCursor inputGeoms, GeometryCursor geomIntersector, SpatialReference sr,
    ...
    Geometry intersect(Geometry input_geom) {
        ...
        Envelope2D commonExtent = InternalUtils.getMergedExtent(m_geomIntersector, input_geom);
        ...
        double tol = 0;
        Geometry clippedIntersector = Clipper.clip(m_geomIntersector, env, tol, 0.0);
        Geometry clippedInputGeom = Clipper.clip(input_geom, env, tol, 0.0);
        // perform the clip
        return TopologicalOperations.intersection(clippedInputGeom, clippedIntersector, m_spatial_reference,
                m_progress_tracker);
    }
    ...
    GeometryCursor intersectEx(Geometry input_geom) {
        ...
        // perform the clip
        Geometry[] res_vec;
        res_vec = TopologicalOperations.intersectionEx(clippedInputGeom, clippedIntersector, m_spatial_reference,
                m_progress_tracker);
        return prepareVector_(input_geom.getDescription(), m_dimensionMask, res_vec);
    }
    ...
    Geometry tryNativeImplementation_(Geometry input_geom) {
        ...
        Envelope2D mergedExtent = InternalUtils.getMergedExtent(input_geom, m_geomIntersector);
        double tolerance = InternalUtils.calculateToleranceFromGeometry(m_spatial_reference, mergedExtent, false);
        ...
        if (!bResultIsEmpty) {// try accelerated test
            int res = OperatorInternalRelationUtils.quickTest2D_Accelerated_DisjointOrContains(m_geomIntersector,
                    input_geom, tolerance);
        ...
        if ((m_dimensionMask == -1 || m_dimensionMask == (1 << 2)) && gtInput == Geometry.GeometryType.Envelope
        ...
        if ((gtInput == Geometry.GeometryType.Envelope && Geometry.getDimensionFromType(m_geomIntersectorType) == 0)
                || (m_geomIntersectorType == Geometry.GeometryType.Envelope
                        && Geometry.getDimensionFromType(gtInput) == 0)) {
            Envelope env = gtInput == Geometry.GeometryType.Envelope ? (Envelope) input_geom
                    : (Envelope) m_geomIntersector;
            Geometry other = gtInput == Geometry.GeometryType.Envelope ? m_geomIntersector : input_geom;
            Envelope2D env_2D = new Envelope2D();
            env.queryEnvelope2D(env_2D);
            return Clipper.clip(other, env_2D, tolerance, 0);
        }

        if ((Geometry.getDimensionFromType(gtInput) == 0 && Geometry.getDimensionFromType(m_geomIntersectorType) > 0)
                || (Geometry.getDimensionFromType(gtInput) > 0
                        && Geometry.getDimensionFromType(m_geomIntersectorType) == 0)) {// multipoint
                                                                                        // intersection
            double tolerance1 = InternalUtils.calculateToleranceFromGeometry(m_spatial_reference, input_geom, false);
            if (gtInput == Geometry.GeometryType.MultiPoint)
                return TopologicalOperations.intersection((MultiPoint) input_geom, m_geomIntersector, tolerance1);
            if (gtInput == Geometry.GeometryType.Point)
                return TopologicalOperations.intersection((Point) input_geom, m_geomIntersector, tolerance1);
            if (m_geomIntersectorType == Geometry.GeometryType.MultiPoint)
                return TopologicalOperations.intersection((MultiPoint) m_geomIntersector, input_geom, tolerance1);
            if (m_geomIntersectorType == Geometry.GeometryType.Point)
                return TopologicalOperations.intersection((Point) m_geomIntersector, input_geom, tolerance1);
            throw GeometryException.GeometryInternalError();
        }

        // Try Polyline vs Polygon
        if ((m_dimensionMask == -1 || m_dimensionMask == (1 << 1)) && (gtInput == Geometry.GeometryType.Polyline)
                && (m_geomIntersectorType == Geometry.GeometryType.Polygon)) {
            return tryFastIntersectPolylinePolygon_((Polyline) (input_geom), (Polygon) (m_geomIntersector));
        }

        // Try Polygon vs Polyline
        if ((m_dimensionMask == -1 || m_dimensionMask == (1 << 1)) && (gtInput == Geometry.GeometryType.Polygon)
                && (m_geomIntersectorType == Geometry.GeometryType.Polyline)) {
            return tryFastIntersectPolylinePolygon_((Polyline) (m_geomIntersector), (Polygon) (input_geom));
        }
    ...
    Geometry tryFastIntersectPolylinePolygon_(Polyline polyline, Polygon polygon) {
        ...
        double tolerance = InternalUtils.calculateToleranceFromGeometry(m_spatial_reference, polygon, false);
        ...
        if (polygon.getPointCount() > 5) {
            double tol = 0;
            Geometry clippedPolygon = Clipper.clip(polygon, clipEnvelope, tol, 0.0);
        ...
        double thisAlgorithmComplexity = ((double) unresolvedSegments * polygonImpl.getPointCount());// assume the worst
                                                                                                     // case.
        double planesweepComplexity = StrictMath.log(totalPoints) * totalPoints;
        ...
        while (polylineIter.nextPath()) {
                ...
                if (clipStatus < 0) {
                    ...
                    if (polygonQuadTree != null) {
                        ...
                        for (int ind = qIter.next(); ind != -1; ind = qIter.next()) {
                            polygonIter.resetToVertex(polygonQuadTree.getElement(ind)); // path_index
                    ...
                    if (intersections.size() > 0) {// intersections detected.
                        ...
                        for (int i = 0, n = intersections.size(); i < n; i++) {
                            ...
                            if (state >= stateManySegments) {
                                resultPolylineImpl.addSegmentsFromPath(polylineImpl, polylinePathIndex, start_index,
                                        inCount, state == stateManySegmentsNewPath);
                                    ...
                                    if (!bWholeSegment) {
                                        ...
                                        if (state < stateManySegments) {
                                            start_index = polylineIter.getStartPointIndex()
                                                    - polylineImpl.getPathStart(polylinePathIndex);
                                            inCount = 1;
                        ...
                        clipStatus = analyseClipSegment_(polygon, polylineSeg.getStartXY(), tolerance);// simple
                                                                                                       // case
                                                                                                       // no
                                                                                                       // intersection.
                                                                                                       // Both
                        ...
                        assert (analyseClipSegment_(polygon, polylineSeg.getEndXY(), tolerance) == clipStatus);
                    ...
                    if (clipStatus == 0) {// outside
                        assert (analyseClipSegment_(polygon, polylineSeg, tolerance) == 0);
                    ...
                    if (clipStatus == 1) {
                        assert (analyseClipSegment_(polygon, polylineSeg, tolerance) == 1);
                        if (state == stateNewPath) {
                            state = stateManySegmentsNewPath;
                            start_index = polylineIter.getStartPointIndex()
                                    - polylineImpl.getPathStart(polylinePathIndex);
            ...
            if (state >= stateManySegments) {
                resultPolylineImpl.addSegmentsFromPath(polylineImpl, polylinePathIndex, start_index, inCount,
                        state == stateManySegmentsNewPath);
                start_index = -1;
            }
        }
    ...
    Geometry normalizeIntersectionOutput(Geometry geom, int GT_1, int GT_2) {
        if (GT_1 == Geometry.GeometryType.Point || GT_2 == Geometry.GeometryType.Point) {
