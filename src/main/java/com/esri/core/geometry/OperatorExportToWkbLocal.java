...
class OperatorExportToWkbLocal extends OperatorExportToWkb {
    ...
    private static int exportToWKB(int exportFlags, Geometry geometry, ByteBuffer wkbBuffer) {
        ...
        case Geometry.GeometryType.Polygon:
            ...
            return exportPolygonToWKB(exportFlags, (Polygon) geometry, wkbBuffer);
        case Geometry.GeometryType.Polyline:
            if ((exportFlags & WkbExportFlags.wkbExportPolygon) != 0
                    ...
                    || (exportFlags & WkbExportFlags.wkbExportMultiPoint) != 0)
                throw new GeometryException("invalid argument");
            return exportPolylineToWKB(exportFlags, (Polyline) geometry, wkbBuffer);
        ...
        case Geometry.GeometryType.MultiPoint:
            if ((exportFlags & WkbExportFlags.wkbExportLineString) != 0
                    ...
                    || (exportFlags & WkbExportFlags.wkbExportMultiPolygon) != 0)
                throw new GeometryException("invalid argument");
            return exportMultiPointToWKB(exportFlags, (MultiPoint) geometry, wkbBuffer);
        ...
        case Geometry.GeometryType.Envelope:
            if ((exportFlags & WkbExportFlags.wkbExportLineString) != 0
                    ...
                    || (exportFlags & WkbExportFlags.wkbExportMultiPoint) != 0)
                throw new GeometryException("invalid argument");
            return exportEnvelopeToWKB(exportFlags, (Envelope) geometry, wkbBuffer);
    ...
    private static int exportPolygonToWKB(int exportFlags, Polygon _polygon, ByteBuffer wkbBuffer) {
        ...
        if ((exportFlags & (int) WkbExportFlags.wkbExportPolygon) != 0 && polygonCount > 1)
        ...
        // get size for buffer
        int size = 0;
        if ((exportFlags & (int) WkbExportFlags.wkbExportPolygon) == 0 || polygonCount == 0)
            size += 1 /* byte order */ + 4 /* wkbType */ + 4 /* numPolygons */;

        size += polygonCount * (1 /* byte order */ + 4 /* wkbType */ + 4/* numRings */)
        ...
        if (bExportZs) {
            if (polygon._attributeStreamIsAllocated(VertexDescription.Semantics.Z))
                zs = (AttributeStreamOfDbl) (polygon.getAttributeStreamRef(VertexDescription.Semantics.Z));
        }

        AttributeStreamOfDbl ms = null;
        if (bExportMs) {
            if (polygon._attributeStreamIsAllocated(VertexDescription.Semantics.M))
                ms = (AttributeStreamOfDbl) (polygon.getAttributeStreamRef(VertexDescription.Semantics.M));
        }
        ...
        for (int ipolygon = 0; ipolygon < (int) polygonCount; ipolygon++) {
            ...
            while (ipolygonend < partCount && (pathFlags.read(ipolygonend) & PathFlags.enumOGCStartPolygon) == 0)
                ipolygonend++;
            ...
            for (int ipart = ipolygonstart; ipart < ipolygonend; ipart++) {
                ...
                if (bExportZs) {
                    ...
                    if (zs != null)
                        ...
                        z = VertexDescription.getDefaultValue(VertexDescription.Semantics.Z);
                ...
                if (bExportMs) {
                    ...
                    if (ms != null)
                        ...
                        m = VertexDescription.getDefaultValue(VertexDescription.Semantics.M);
    ...
    private static int exportPolylineToWKB(int exportFlags, Polyline _polyline, ByteBuffer wkbBuffer) {
        ...
        boolean bExportZs = polyline.hasAttribute(VertexDescription.Semantics.Z)
                && (exportFlags & WkbExportFlags.wkbExportStripZs) == 0;
        boolean bExportMs = polyline.hasAttribute(VertexDescription.Semantics.M)
                && (exportFlags & WkbExportFlags.wkbExportStripMs) == 0;

        int partCount = polyline.getPathCount();
        if ((exportFlags & WkbExportFlags.wkbExportLineString) != 0 && partCount > 1)
            throw new IllegalArgumentException();
        ...
        // get size for buffer
        int size = 0;
        if ((exportFlags & WkbExportFlags.wkbExportLineString) == 0 || partCount == 0)
            size += 1 /* byte order */ + 4 /* wkbType */ + 4 /* numLineStrings */;

        size += partCount * (1 /* byte order */ + 4 /* wkbType */ + 4/* num_points */)
        ...
        AttributeStreamOfDbl zs = null;
        if (bExportZs) {
            if (polyline._attributeStreamIsAllocated(VertexDescription.Semantics.Z))
                zs = (AttributeStreamOfDbl) (polyline.getAttributeStreamRef(VertexDescription.Semantics.Z));
        }

        AttributeStreamOfDbl ms = null;
        if (bExportMs) {
            if (polyline._attributeStreamIsAllocated(VertexDescription.Semantics.M))
                ms = (AttributeStreamOfDbl) (polyline.getAttributeStreamRef(VertexDescription.Semantics.M));
        }
    ...
    private static int exportMultiPointToWKB(int exportFlags, MultiPoint _multipoint, ByteBuffer wkbBuffer) {
        ...
        boolean bExportZs = multipoint.hasAttribute(VertexDescription.Semantics.Z)
                && (exportFlags & WkbExportFlags.wkbExportStripZs) == 0;
        boolean bExportMs = multipoint.hasAttribute(VertexDescription.Semantics.M)
                && (exportFlags & WkbExportFlags.wkbExportStripMs) == 0;

        int point_count = multipoint.getPointCount();
        if ((exportFlags & WkbExportFlags.wkbExportPoint) != 0 && point_count > 1)
            throw new IllegalArgumentException();
        ...
        if ((exportFlags & WkbExportFlags.wkbExportPoint) == 0) {
            size = 1 /* byte order */ + 4 /* wkbType */ + 4 /* num_points */
                    + point_count * (1 /* byte order */ + 4 /* wkbType */ + 2 * 8 /*
                                                                                   * xy coordinates
                                                                                   */);
        ...
        AttributeStreamOfDbl zs = null;
        if (bExportZs) {
            if (multipoint._attributeStreamIsAllocated(VertexDescription.Semantics.Z))
                zs = (AttributeStreamOfDbl) (multipoint.getAttributeStreamRef(VertexDescription.Semantics.Z));
        }

        AttributeStreamOfDbl ms = null;
        if (bExportMs) {
            if (multipoint._attributeStreamIsAllocated(VertexDescription.Semantics.M))
                ms = (AttributeStreamOfDbl) (multipoint.getAttributeStreamRef(VertexDescription.Semantics.M));
        }
    ...
    private static int exportEnvelopeToWKB(int exportFlags, Envelope envelope, ByteBuffer wkbBuffer) {
        ...
        // get size for buffer
        int size = 0;
        if ((exportFlags & WkbExportFlags.wkbExportMultiPolygon) != 0 || partCount == 0)
            size += 1 /* byte order */ + 4 /* wkbType */ + 4 /* numPolygons */;

        size += partCount * (1 /* byte order */ + 4 /* wkbType */ + 4/* numRings */) + partCount * (4 /* num_points */)
                + point_count * (2 * 8 /*
                                        * xy coordinates
                                        */);
        ...
        if (bExportZs)
            z_interval = envelope.queryInterval(VertexDescription.Semantics.Z, 0);

        Envelope1D mInterval = null;
        if (bExportMs)
            mInterval = envelope.queryInterval(VertexDescription.Semantics.M, 0);
