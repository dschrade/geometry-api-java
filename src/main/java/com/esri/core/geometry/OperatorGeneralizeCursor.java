...
final class OperatorGeneralizeCursor extends GeometryCursor {
    ...
    public OperatorGeneralizeCursor(GeometryCursor geoms, double maxDeviation, boolean bRemoveDegenerateParts,
    ...
    private Geometry Generalize(Geometry geom) {
        ...
        for (int ipath = 0, npath = mp.getPathCount(); ipath < npath; ipath++) {
            GeneralizePath((MultiPathImpl) mp._getImpl(), ipath, (MultiPathImpl) dstmp._getImpl(), line);
        }
    ...
    private int FindGreatestDistance(Line line, Point2D ptHelper, AttributeStreamOfDbl xy, int start, int end,
            int pathEnd) {
        int to = end - 1;
