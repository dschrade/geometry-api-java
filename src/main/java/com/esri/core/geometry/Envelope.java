...
public class Envelope extends Geometry implements Serializable {
    ...
    /**
     * Constructs an envelope that covers the given point. The coordinates of the
     * point are used to set the extent of the envelope.
     * 
     * @param point
    ...
    /**
     * Sets the envelope from the array of points. The result envelope is a bounding
     * box of all the points in the array. If the array has zero length, the
     * envelope will be empty.
     * 
     * @param points
     *            The point array.
     */
     ...
     * 
     * @return A point whose x and y-coordinates are that of the center of the
     *         envelope.
     */
    ...
    public void getCenter(Point point_out) {
        ...
        for (int i = 1; i < nattrib; i++) {
            ...
            for (int iord = 0; iord < ncomp; iord++) {
                double v = 0.5
                        * (getAttributeAsDblImpl_(0, semantics, iord) + getAttributeAsDblImpl_(1, semantics, iord));
    ...
    /**
     * Merges this envelope with the extent of the given envelope. If this envelope
     * is empty, the coordinates of the given envelope are assigned. If the given
     * envelope is empty, this envelope is unchanged.
     * 
     * @param other
     ...
     * increased to include the point. If the envelope is empty, the coordinates of
     * the point to merge are assigned. If the point is empty, the original envelope
     * is unchanged.
     * 
     * @param point
    ...
    public void copyTo(Geometry dst) {
        ...
        if (m_attributes != null) {
            ...
            System.arraycopy(m_attributes, 0, envDst.m_attributes, 0, (m_description.getTotalComponentCount() - 2) * 2);
        }
    }
    ...
    void queryEnvelope3D(Envelope3D env) {
        ...
        env.xmax = m_envelope.xmax;
        env.ymax = m_envelope.ymax;
        env.setCoords(m_envelope.xmin, m_envelope.ymin, _getAttributeAsDbl(0, Semantics.Z, 0), m_envelope.xmax,
    ...
    /**
     * Sets the point's coordinates to the coordinates of the envelope at the given
     * corner.
     * 
     * @param index
    ...
    double getAttributeAsDblImpl_(int end_point, int semantics, int ordinate) {
        ...
        if (attribute_index >= 0) {
            return m_attributes[getEndPointOffset(m_description, end_point)
                    + m_description.getPointAttributeOffset_(attribute_index) - 2 + ordinate];
        }
    ...
    void setAttributeAsDblImpl_(int end_point, int semantics, int ordinate, double value) {
        ...
        m_attributes[getEndPointOffset(m_description, end_point)
                + m_description.getPointAttributeOffset_(attribute_index) - 2 + ordinate] = value;
    }
    ...
    /**
     * Sets the envelope's corners to be centered around the specified point, using
    ...
    /**
     * Normalizes envelopes if the minimum dimension is larger than the maximum
     * dimension.
     */
    ...
    public String toString() {
        ...
        String s = "Envelope: [" + m_envelope.xmin + ", " + m_envelope.ymin + ", " + m_envelope.xmax + ", "
                + m_envelope.ymax + "]";
        return s;
    }
