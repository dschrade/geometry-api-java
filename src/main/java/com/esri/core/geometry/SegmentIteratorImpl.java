...
final class SegmentIteratorImpl {
    ...
    /**
     * Resets the iterator so that the call to PreviousSegment will return the last
     * segment of the current path.
     */
    ...
    public void resetToVertex(int vertexIndex, int _pathIndex) {
        if (m_currentPathIndex >= 0 && m_currentPathIndex < m_parent.getPathCount()) {// check if we
                                                                                      // are in
                                                                                      // the
                                                                                      // current
                                                                                      // path
        ...
        int path_index;
        if (_pathIndex >= 0 && _pathIndex < m_parent.getPathCount() && vertexIndex >= m_parent.getPathStart(_pathIndex)
    ...
    /**
     * Resets the iterator such that the subsequent call to the NextPath will set
     * the iterator to the first segment of the first path.
     */
    ...
    /**
     * Resets the iterator such that the subsequent call to the PreviousPath will
     * set the iterator to the last segment of the last path.
     */
    ...
    /**
     * Resets the iterator such that the subsequent call to the NextPath will set
     * the iterator to the first segment of the given path. The call to PreviousPath
     * will reset the iterator to the last segment of path pathIndex - 1.
     */
    ...
    public void _updateSegment() {
        ...
        AttributeStreamOfInt8 segFlagStream = m_parent.getSegmentFlagsStreamRef();
