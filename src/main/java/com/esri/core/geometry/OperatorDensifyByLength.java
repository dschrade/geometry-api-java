...
public abstract class OperatorDensifyByLength extends Operator {
     ...
     *         dim &lt; 1, but simply passes them along).
     */
    public abstract GeometryCursor execute(GeometryCursor inputGeometries, double maxLength,
     ...
     *            After that the curves are replaced with straight segments.
     * @param progressTracker
     * @return Returns the densified geometry. (It does nothing to geometries with
     *         dim &lt; 1, but simply passes them along).
     */
    ...
    public static OperatorDensifyByLength local() {
        return (OperatorDensifyByLength) OperatorFactoryLocal.getInstance().getOperator(Type.DensifyByLength);
    }
