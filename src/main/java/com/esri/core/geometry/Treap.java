...
final class Treap {
    ...
    public int addElementAtPosition(int prevNode, int nextNode, int element, boolean bUnique, boolean bCallCompare,
            int treap) {
        int treap_ = treap;
        if (treap_ == -1) {
            if (m_defaultTreap == nullNode())
    ...
    private void bubbleDown_(int treap_node_index) {
        ...
        while (left != nullNode() || right != nullNode()) {
            int lcprior = left != nullNode() ? getPriority_(left) : NumberUtils.intMax();
            int rcprior = right != nullNode() ? getPriority_(right) : NumberUtils.intMax();
            int minprior = StrictMath.min(lcprior, rcprior);
    ...
    private int getMaxDepthHelper_(int node) {
        ...
        return 1 + StrictMath.max(getMaxDepthHelper_(getLeft(node)), getMaxDepthHelper_(getRight(node)));
    }
