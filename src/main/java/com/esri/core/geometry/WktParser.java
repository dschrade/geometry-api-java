...
final class WktParser {
    ...
    double currentNumericLiteral() {
        ...
        double value = Double.parseDouble(m_wkt_string.substring(m_start_token, m_end_token));
        return value;
    }
    ...
    private interface Number {
        static final int signed_numeric_literal = WktToken.x_literal | WktToken.y_literal | WktToken.z_literal
                | WktToken.m_literal;
    }
    ...
    private void geometry_() {
        ...
        if (m_start_token + 5 <= m_wkt_string.length()
                && m_wkt_string.regionMatches(true, m_start_token, "point", 0, 5)) {
            ...
            m_function_stack.add(State.pointStart);
                ...
                && m_wkt_string.regionMatches(true, m_start_token, "linestring", 0, 10)) {
            m_end_token = m_start_token + 10;
            m_current_token_type = WktToken.linestring;
            m_function_stack.add(State.lineStringStart);
        } else if (m_start_token + 10 <= m_wkt_string.length()
                && m_wkt_string.regionMatches(true, m_start_token, "multipoint", 0, 10)) {
            m_end_token = m_start_token + 10;
            ...
            m_function_stack.add(State.multiPointStart);
                ...
                && m_wkt_string.regionMatches(true, m_start_token, "polygon", 0, 7)) {
            ...
            m_function_stack.add(State.polygonStart);
                ...
                && m_wkt_string.regionMatches(true, m_start_token, "multilinestring", 0, 15)) {
            ...
            m_function_stack.add(State.multiLineStringStart);
                ...
                && m_wkt_string.regionMatches(true, m_start_token, "multipolygon", 0, 12)) {
            ...
            m_function_stack.add(State.multiPolygonStart);
                ...
                && m_wkt_string.regionMatches(true, m_start_token, "geometrycollection", 0, 18)) {
            ...
            throw new IllegalArgumentException("Could not parse Well-Known Text around position: " + m_end_token);
        }
    ...
    private void attributes_() {
        ...
        if (m_wkt_string.charAt(m_end_token) == 'z' || m_wkt_string.charAt(m_end_token) == 'Z') {
            b_has_zs = true;
        ...
        if (m_wkt_string.charAt(m_end_token) == 'm' || m_wkt_string.charAt(m_end_token) == 'M') {
            b_has_ms = true;
    ...
    private void sign_() {
        // Optional - or + sign
        if (m_wkt_string.charAt(m_end_token) == '-' || m_wkt_string.charAt(m_end_token) == '+') {
            if (++m_end_token >= m_wkt_string.length())
                throw new IllegalArgumentException();
        }
    }
    ...
    private void exp_() {
        ...
        if (m_wkt_string.charAt(m_end_token) == 'e' || m_wkt_string.charAt(m_end_token) == 'E') {
            if (++m_end_token >= m_wkt_string.length())
                throw new IllegalArgumentException();
