...
public abstract class MultiPath extends MultiVertexGeometry implements Serializable {
     ...
     *            The index of first segment in the path to start adding from. The
     *            value has to be between 0 and src.getSegmentCount(srcPathIndex) -
     *            1.
     * @param srcSegmentCount
     *            The number of segments to add. If 0, the function does nothing.
     * @param bStartNewPath
     ...
     *            segment is not added. This is done to ensure proper connection to
     *            existing segments. When the source path is closed, and the closing
     *            segment is among those to be added, it is added also as a closing
     *            segment, not as a real segment. Use add_segment instead if you do
     *            not like that behavior.
     *
     *            This MultiPath obtains all missing attributes from the src
     *            MultiPath.
     */
    public void addSegmentsFromPath(MultiPath src, int srcPathIndex, int srcSegmentFrom, int srcSegmentCount,
            boolean bStartNewPath) {
        m_impl.addSegmentsFromPath((MultiPathImpl) src._getImpl(), srcPathIndex, srcSegmentFrom, srcSegmentCount,
                bStartNewPath);
    }
     ...
     * @param src
     *            The multipath to insert into this multipath. Can be the same as
     *            the multipath being modified.
     * @param srcPathIndex
     ...
     *            The point index before all other vertices to insert in the given
     *            path of this multipath. This value must be between 0 and
     *            GetPathSize(pathIndex), or -1 to insert points at the end of the
     *            given path.
     * @param src
     ...
     *            When FALSE, the points are inserted in reverse order.
     */
    public void insertPoints(int pathIndex, int beforePointIndex, MultiPath src, int srcPathIndex,
            int srcPointIndexFrom, int srcPointCount, boolean bForward) {
        m_impl.insertPoints(pathIndex, beforePointIndex, (MultiPathImpl) src._getImpl(), srcPathIndex,
     ...
     * 
     * @param pathIndex
     *            The path index in this class to insert points to. Must correspond
     *            to an existing path.
     * @param beforePointIndex
     *            The point index in the given path of this MultiPath before which
     *            the vertices need to be inserted. This value must be between 0 and
     *            GetPathSize(pathIndex), or -1 to insert points at the end of the
     *            given path.
     * @param src
     *            The source array
     ...
     *            When FALSE, the points are inserted in reverse order.
     */
    void insertPoints(int pathIndex, int beforePointIndex, Point2D[] src, int srcPointIndexFrom, int srcPointCount,
            boolean bForward) {
     ...
     *            correspond to an existing path.
     * @param beforePointIndex
     *            The point index in the given path of this multipath. This value
     *            must be between 0 and GetPathSize(pathIndex), or -1 to insert the
     *            point at the end of the given path.
     * @param pt
     ...
     * 
     * @param endPoint
     *            The end point to which the newly added line segment should point.
     */
     ...
     * segment is a segment that connects the last and the first points of the path.
     * This is a virtual segment. The first point is not duplicated to close the
     * path.
     * 
     * Call this method only for polylines. For polygons this method is implicitly
     * called for the Polygon class.
     */
    ...
    /**
     * Closes all open paths by adding an implicit line segment from the end point
     * to the start point. Call this method only for polylines.For polygons this
     * method is implicitly called for the Polygon class.
     */
    ...
    /**
     * Indicates if the given path is closed (represents a ring). A closed path has
     * a virtual segment that connects the last and the first points of the path.
     * The first point is not duplicated to close the path. Polygons always have all
     * paths closed.
     * 
     * @param pathIndex
     ...
     * multipath. Calling nextPath() will set the iterator to the first path of this
     * multipath.
     * 
     ...
     * Calling PreviousSegment () will return the segment that starts at the
     * previous vertex.
     * 
    ...
    public SegmentIterator querySegmentIteratorAtVertex(int startVertexIndex) {
        return new SegmentIterator(m_impl.querySegmentIteratorAtVertex(startVertexIndex));
    }
