...
class Envelope2DIntersectorImpl {
    ...
    void endRedConstruction() {
        ...
        if (m_envelopes_red != null && m_envelopes_red.size() > 0 && m_envelopes_blue != null
                && m_envelopes_blue.size() > 0) {
            if (m_function == -1)
    ...
    /*
     * Moves the iterator to the next intersecting pair of envelopes.Returns true if
     * an intersecting pair is found. You can call get_handle_a() and get_handle_b()
     * to get the index of each envelope in the current intersection. Otherwise if
     * false is returned, then are no more intersections (if at all).
     */
    boolean next() {
        if (m_b_done)
            return false;
    ...
    /*
     * Returns the index of the first envelope in the intersection. In the red/blue
     * case, this will be an index to the red envelopes.
     */
    int getHandleA() {
    ...
    /*
     * Returns the index of the second envelope in the intersection. In the red/blue
     * case, this will be an index to the blue envelopes.
     */
    int getHandleB() {
    ...
    /*
     * Returns a reference to the envelope at the given handle. Use this for the
     * red/red intersection case.
     */
    ...
    /*
     * Returns the user element associated with handle. Use this for the red/red
     * intersection case.
     */
    ...
    private boolean sweepBruteForce_() {// this isn't really a sweep, it just walks along the array of red envelopes
                                        // backward.
    ...
    private boolean sweepRedBlueBruteForce_() {// this isn't really a sweep, it just walks along the array of red
                                               // envelopes backward.
        if (--m_sweep_index_red == -1) {
            m_envelope_handle_a = -1;
            m_envelope_handle_b = -1;
    ...
    private boolean sweepRedBlue_() {// controls whether we want to sweep the red envelopes or sweep the blue
                                     // envelopes
    ...
    private boolean sweepRed_() {
        ...
        if (m_interval_tree_blue.size() > 0) {
            m_iterator_blue.resetIterator(m_envelopes_red.get(envelope_handle_red).xmin,
            ...
            m_queued_indices_red.set(envelope_handle_red,
                    m_queued_envelopes.addElement(m_queued_list_red, envelope_handle_red));
    ...
    private boolean sweepBlue_() {
        ...
        if (m_interval_tree_red.size() > 0) {
            m_iterator_red.resetIterator(m_envelopes_blue.get(envelope_handle_blue).xmin,
                    m_envelopes_blue.get(envelope_handle_blue).xmax, m_tolerance);
            ...
            m_queued_indices_blue.set(envelope_handle_blue,
                    m_queued_envelopes.addElement(m_queued_list_blue, envelope_handle_blue));
            m_function = State.sweepRedBlue;
        }
