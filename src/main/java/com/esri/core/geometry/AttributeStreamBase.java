...
abstract class AttributeStreamBase {
    ...
    /**
     * Resizes the AttributeStream to the new size. Does not change the capacity of
     * the stream.
     */
    ...
    /**
     * Adds a range of elements from the source stream. The streams must be of the
     * same type.
     * 
     * @param src
    ...
    /**
     * Inserts a range of elements from the source stream. The streams must be of
     * the same type.
     * 
     * @param start
     ...
     */
    public abstract void insertRange(int start, AttributeStreamBase src, int srcStart, int count, boolean bForward,
    ...
    /**
     * Adds a range of elements from the source byte buffer. This stream is resized
     * automatically to accomodate required number of elements.
     * 
     * @param startElement
     *            the index of the element in this stream to start setting elements
     *            from.
     * @param count
     ...
     * @param stride
     *            Used for reversed writing only to indicate the unit of writing.
     *            elements inside a stride are not reversed. Only the strides are
     *            reversed.
     */
    public abstract void writeRange(int startElement, int count, AttributeStreamBase src, int sourceStart,
            boolean bForward, int stride);

    /**
     * Adds a range of elements from the source byte buffer. The stream is resized
     * automatically to accomodate required number of elements.
     * 
     * @param startElement
     *            the index of the element in this stream to start setting elements
     ...
     * @param dst
     *            The destination ByteBuffer. The buffer must be large enough or it
     *            will throw.
     * @param dstOffsetBytes
     *            The offset in the destination ByteBuffer to start write elements
     *            from.
     */
    ...
    public static AttributeStreamBase createByteStream(int size, byte defaultValue) {
        AttributeStreamBase newStream = new AttributeStreamOfInt8(size, defaultValue);
        return newStream;
    ...
    public static AttributeStreamBase createDoubleStream(int size, double defaultValue) {
        AttributeStreamBase newStream = new AttributeStreamOfDbl(size, defaultValue);
        return newStream;
    }
    ...
    /**
     * Makes the stream to be readonly. Any operation that changes the content or
     * size of the stream will throw.
     */
     ...
     * @param size
     *            The number of elements (floats, doubles, or 32 bit integers) of
     *            the given type in the stream.
     */
    ...
    public static AttributeStreamBase createAttributeStreamWithSemantics(int semantics, int vertexCount) {
        ...
        return createAttributeStreamWithPersistence(persistence, vertexCount * ncomps,
