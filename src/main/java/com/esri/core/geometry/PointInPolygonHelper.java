...
final class PointInPolygonHelper {
    ...
    private void doOne(Segment seg) {
        if (!m_bTestBorder) {
            ...
            if (m_bAlternate && m_inputPoint.isEqual(seg.getStartXY()) || m_inputPoint.isEqual(seg.getEndXY())) {// test
                                                                                                                 // if
                                                                                                                 // the
                                                                                                                 // point
                                                                                                                 // coincides
        ...
        if (seg.getStartY() == m_inputPoint.y && seg.getStartY() == seg.getEndY()) {// skip horizontal
                                                                                    // segments. test if the
                                                                                    // point lies on a
                                                                                    // horizontal segment
            if (m_bAlternate && !m_bTestBorder) {
                double minx = StrictMath.min(seg.getStartX(), seg.getEndX());
                double maxx = StrictMath.max(seg.getStartX(), seg.getEndX());
        ...
        if (m_inputPoint.x > maxx) {
            ...
            if (m_inputPoint.x >= StrictMath.min(seg.getStartX(), seg.getEndX())) {
    ...
    private boolean processSegment(Segment segment) {
        Envelope1D yrange = segment.queryInterval((int) VertexDescription.Semantics.POSITION, 1);
    ...
    private static int _isPointInPolygonInternalWithQuadTree(Polygon inputPolygon, QuadTreeImpl quadTree,
    ...
    static int isPointInPolygon(Polygon inputPolygon, double inputPointXVal, double inputPointYVal, double tolerance) {
        ...
        return _isPointInPolygonInternal(inputPolygon, new Point2D(inputPointXVal, inputPointYVal), tolerance);
    }

    public static int isPointInRing(MultiPathImpl inputPolygonImpl, int iRing, Point2D inputPoint, double tolerance,
            QuadTree quadTree) {
        Envelope2D env = new Envelope2D();
    ...
    static boolean quadTreeWillHelp(Polygon polygon, int c_queries) {
        ...
        double c_quad_tree = c_build_quad_tree * n
                + c_query_quad_tree * (StrictMath.log((double) n) / StrictMath.log(2.0)) * c_queries;
