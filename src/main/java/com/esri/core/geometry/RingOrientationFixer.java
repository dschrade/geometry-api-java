...
class RingOrientationFixer {
        ...
        int compare(Treap treap, int left, int node) {
            ...
            if (m_left_elm == left)
                ...
                if (m_seg_1 == null) {
                    ...
                    m_seg_1 = m_line_1;
                    x_1 = m_line_1.intersectionOfYMonotonicWithAxisX(m_helper.m_y_scanline, 0);
                } else
                    x_1 = m_seg_1.intersectionOfYMonotonicWithAxisX(m_helper.m_y_scanline, 0);
            ...
            if (seg_2 == null) {
                ...
                seg_2 = m_line_2;
                x2 = m_line_2.intersectionOfYMonotonicWithAxisX(m_helper.m_y_scanline, 0);
            } else
                x2 = seg_2.intersectionOfYMonotonicWithAxisX(m_helper.m_y_scanline, 0);
            ...
            if (x_1 == x2) {
                ...
                double y1 = !bStartLower1 ? m_seg_1.getStartY() : m_seg_1.getEndY();
    ...
    boolean fixRingOrientation_() {
        ...
        for (int ivertex = m_sorted_vertices.getFirst(
                m_sorted_vertices.getFirstList()); ivertex != -1; ivertex = m_sorted_vertices.getNext(ivertex)) {
        ...
        for (int path = m_shape.getFirstPath(m_geometry); path != -1;) {
            if (m_shape.getPathUserIndex(path, m_path_orientation_index) == 3) {// exterior
                m_shape.setExterior(path, true);
                int afterPath = path;
                for (int nextHole = m_shape.getPathUserIndex(path, m_path_parentage_index); nextHole != -1;) {
    ...
    boolean processBunchForRingOrientationTestOddEven_(AttributeStreamOfInt32 bunch) {
        ...
        for (int i = 0; i < m_unknown_nodes.size() && m_unknown_ring_orientation_count > 0; i++) {
            ...
            for (int vertex = m_shape.getNextVertex(first_vertex); vertex != first_vertex; vertex = m_shape
                ...
                if (cluster_to != -1) {
                    ...
                    if (cluster_stack.getLast() == cluster_to) {
                        ...
                        int new_path = m_shape.insertClosedPath_(m_geometry, -1, from_next,
                                m_shape.getFirstVertex(path), first_vertex_correction_requied);
