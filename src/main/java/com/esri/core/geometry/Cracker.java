...
final class Cracker {
    ...
    private boolean crackBruteForce_() {
        ...
        for (int vertex_1 = iter_1.next(); vertex_1 != -1; vertex_1 = iter_1.next()) {
            ...
            EditShape.VertexIterator iter_2 = m_shape.queryVertexIterator(iter_1);
                        ...
                        if (seg_1_env.contains(pt)) {
                            ...
                            if (split_count_1 > 0) {
                                ...
                                if (seg_2_zero) {
                                    ...
                                    int v_to = -1;
                                    for (int v = m_shape.getNextVertex(vertex_2); v != -1
                                            && v != vertex_2; v = m_shape.getNextVertex(v)) {
                        ...
                        if (seg_2_env.contains(pt)) {
                            ...
                            if (split_count_2 > 0) {
                                ...
                                if (seg_1_zero) {
                                    ...
                                    // segment at vertex_2 is dzero length, change all attached zero length segments
                                    int v_to = -1;
                                    for (int v = m_shape.getNextVertex(vertex_1); v != -1
                                            && v != vertex_1; v = m_shape.getNextVertex(v)) {
    ...
    boolean needsCrackingImpl_() {
        ...
        for (int vertex = event_q.get(event_q_index++); vertex != -1;) {
            ...
            boolean b_continuing_segment_chain_optimization = swept_edges_to_delete.size() == 1
                    && edges_to_insert.size() == 2;
    ...
    // Used for IsSimple.
    static boolean needsCracking(boolean allowCoincident, EditShape shape, double tolerance, NonSimpleResult result,
