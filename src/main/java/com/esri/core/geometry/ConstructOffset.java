...
class ConstructOffset {
    ...
    static class GraphicPoint {
        ...
        GraphicPoint(GraphicPoint srcPt, double d, double angle) {
            x = srcPt.x + d * StrictMath.cos(angle);
            y = srcPt.y + d * StrictMath.sin(angle);
            type = srcPt.type;
            m = srcPt.m;
        }
    ...
    // static
    static Geometry execute(Geometry inputGeometry, double distance, OperatorOffset.JoinType joins, double miterLimit,
            double tolerance, ProgressTracker progressTracker) {
    ...
    private final double pi = StrictMath.PI;// GEOMETRYX_PI;
    private final double two_pi = StrictMath.PI * 2;// GEOMETRYX_2PI;
    private final double half_pi = StrictMath.PI / 2;// GEOMETRYX_HalfPI;
    ...
    void addPoint(GraphicPoint offPt, int i_src) {
        ...
        if (s < 0) {
            ...
            if (scal(pt1, pt, pt, m_offsetPts.get(m_offsetPtCount - 1)) > 0) {
                ...
                double a = StrictMath.atan2(pt1.y - pt0.y, pt1.x - pt0.x);
                p = new GraphicPoint(pt1, m_distance, a - half_pi);
                m_offsetPts.set(m_offsetPtCount - 1, p);

                if (m_joins == OperatorOffset.JoinType.Bevel || m_joins == OperatorOffset.JoinType.Miter) {
    ...
    boolean buildOffset() {
        ...
        for (i = 0; i < n; i++) {
            ...
            a1 = StrictMath.atan2(dy1, dx1);
            a2 = StrictMath.atan2(dy2, dx2);
            m_a1 = a1;
            m_a2 = a2;
            if (i == 0) {
                a1_0 = a1;
            ...
            if (cross_product * m_distance > 0.0) // inner angle
            {
                ...
                if (m_joins == OperatorOffset.JoinType.Bevel || m_joins == OperatorOffset.JoinType.Miter) {
                    ...
                    double r = (a2 - a1) * 0.5;
                    double d = m_distance / StrictMath.abs(StrictMath.sin(r));
            ...
            if ((pt.type & IS_END) != 0) {
                ...
                double r = 1.0 - flattenTolerance / StrictMath.abs(m_distance);
                long na = 1;
                double da = (m_distance < 0) ? -pi : pi; // da is negative when
                                                         // m_offset is
                                                         // negative (???)
                if (r > -1.0 && r < 1.0) {
                    double a = StrictMath.acos(r) * 2; // angle where "arrow?" is less
                ...
                double d = m_distance / StrictMath.cos(da / 2);
                a += da / 2;
            ...
            else if (m_joins == OperatorOffset.JoinType.Bevel) // bevel
            {
                ...
                double da = (a2 - half_pi) - (a1 + half_pi); // da is negative
                                                             ...
                                                             // negative
                if (r > -1.0 && r < 1.0) {
                    double a = StrictMath.acos(r) * 2.0; // angle where "arrow?" is
                    // less than
                    // flattenTolerance
                    // do not consider an angle smaller than a degree
                    if (a < oneDegree)
                        a = oneDegree;
                    na = (long) (StrictMath.abs(da) / a + 1.5);
                    if (na > 1)
                        da /= na;
                }
                double d = m_distance / StrictMath.cos(da * 0.5);
                ...
                dx2 = pt2.x - pt.x;
                dy2 = pt2.y - pt.y;
                double d1 = StrictMath.sqrt(dx1 * dx1 + dy1 * dy1);
                double d2 = StrictMath.sqrt(dx2 * dx2 + dy2 * dy2);
                ...
                // continue;
                // }
                double distanceFromCorner = StrictMath.abs(m_distance / StrictMath.sin(StrictMath.acos(cosa) * 0.5));
                double bevelDistance = StrictMath.abs(m_miterLimit * m_distance);
                if (distanceFromCorner > bevelDistance) {
                    ...
                    double sideLength = StrictMath
                            .sqrt(distanceFromCorner * distanceFromCorner - m_distance * m_distance),
                            halfWidth = (distanceFromCorner - bevelDistance) * StrictMath.abs(m_distance) / sideLength;
                ...
                // miter join
                double r = (a2 - a1) * 0.5;
                double d = m_distance / StrictMath.abs(StrictMath.sin(r)); // r should not
                // be null
                // (trapped by
                // the bevel
                // case)
            ...
            if (endVertex - startVertex >= 2) {
                ...
                if (buildOffset())
                    ...
                    if (m_offsetPts.size() >= 2) {
                        ...
                        int iStart = -1;
                        int iEnd = -1;
                        boolean prevClosed = (m_offsetPts.get(m_offsetPtCount - 1).type & CLOSING_SEG) != 0;
                        if (!prevClosed)
                            iStart = 0;
    ...
    int deleteClosedSeg(int seg) {
        ...
        for (int i = 1; i <= n - 2; i++) {
            ...
            for (int j = 1; j <= i; j++) {
                ...
                if ((m_offsetPts.get(im).type & BAD_SEG) == 0 && (m_offsetPts.get(ip).type & BAD_SEG) == 0) {
    ...
    int handleClosedIntersection(int im, int ip) {
        ...
        if (findIntersection(pt1, pt2, pt3, pt4, ii) && !ii.atExistingPt)
            if (StrictMath.signum((pt2.x - pt1.x) * (pt4.y - pt3.y) - (pt2.y - pt1.y) * (pt4.x - pt3.x)) != StrictMath
                    .signum(m_distance)) {
    ...
    boolean sectGraphicRect(GraphicPoint pt1, GraphicPoint pt2, GraphicPoint pt3, GraphicPoint pt4) {
        return (StrictMath.max(pt1.x, pt2.x) >= StrictMath.min(pt3.x, pt4.x)
                && StrictMath.max(pt3.x, pt4.x) >= StrictMath.min(pt1.x, pt2.x)
                && StrictMath.max(pt1.y, pt2.y) >= StrictMath.min(pt3.y, pt4.y)
                && Math.max(pt3.y, pt4.y) >= StrictMath.min(pt1.y, pt2.y));
    }

    boolean findIntersection(GraphicPoint bp1, GraphicPoint bp2, GraphicPoint bp3, GraphicPoint bp4,
