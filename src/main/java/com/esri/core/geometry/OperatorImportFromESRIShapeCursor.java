...
class OperatorImportFromESRIShapeCursor extends GeometryCursor {
    ...
    private Geometry importFromESRIShape(ByteBuffer shapeBuffer) {
        ...
        try {
            ...
            case ShapeType.ShapePolylineZM:
                generaltype = ShapeType.ShapeGeneralPolyline;
                modifiers = ShapeModifiers.ShapeHasZs | (int) ShapeModifiers.ShapeHasMs;
                break;
            ...
            case ShapeType.ShapeMultiPointZM:
                generaltype = ShapeType.ShapeGeneralMultiPoint;
                modifiers = (int) ShapeModifiers.ShapeHasZs | (int) ShapeModifiers.ShapeHasMs;
                break;
            ...
            switch (generaltype) {
            case ShapeType.ShapeGeneralPolygon:
                if (m_type != Geometry.GeometryType.Polygon && m_type != Geometry.GeometryType.Unknown
                        && m_type != Geometry.GeometryType.Envelope)
                    throw new GeometryException("invalid shape type");
                return importFromESRIShapeMultiPath(true, modifiers, shapeBuffer);

            case ShapeType.ShapeGeneralPolyline:
                if (m_type != Geometry.GeometryType.Polyline && m_type != Geometry.GeometryType.Unknown
                        && m_type != Geometry.GeometryType.Envelope)
                    throw new GeometryException("invalid shape type");
                return importFromESRIShapeMultiPath(false, modifiers, shapeBuffer);

            case ShapeType.ShapeGeneralMultiPoint:
                if (m_type != Geometry.GeometryType.MultiPoint && m_type != Geometry.GeometryType.Unknown
                        && m_type != Geometry.GeometryType.Envelope)
                    throw new GeometryException("invalid shape type");
                return importFromESRIShapeMultiPoint(modifiers, shapeBuffer);

            case ShapeType.ShapeGeneralPoint:
                if (m_type != Geometry.GeometryType.Point && m_type != Geometry.GeometryType.MultiPoint
                        && m_type != Geometry.GeometryType.Unknown && m_type != Geometry.GeometryType.Envelope)
                    throw new GeometryException("invalid shape type");
    ...
    private Geometry importFromESRIShapeMultiPath(boolean bPolygon, int modifiers, ByteBuffer shapeBuffer) {
        ...
        if (m_type == Geometry.GeometryType.Polygon || m_type == Geometry.GeometryType.Polyline
            ...
            if (pointCount > 0) {
                ...
                position = (AttributeStreamOfDbl) AttributeStreamBase
                        .createAttributeStreamWithSemantics(Semantics.POSITION, pointCount);
                ...
                for (int ipart = 0; ipart < partCount; ipart++) {
                    ...
                    if (endpart - startpart < 2) {// a part with only one point
                        multipathImpl.setIsSimple(GeometryXSimple.Unknown, 0.0, false);
                    }
        ...
        if (bZs) {
            if (pointCount > 0) {
                ...
                offset += 8;
                double zmax = Interop.translateFromAVNaN(shapeBuffer.getDouble(offset));
                offset += 8;
                ...
                if (m_type == Geometry.GeometryType.Polygon || m_type == Geometry.GeometryType.Polyline
                        || m_type == Geometry.GeometryType.Unknown) {
                    zs = (AttributeStreamOfDbl) AttributeStreamBase.createAttributeStreamWithSemantics(Semantics.Z,
                            pointCount);
                    ...
                    for (int ipart = 0; ipart < partCount; ipart++) {
                        int endpartActual = parts.read(ipart + 1);
                        int endpart = StrictMath.abs(endpartActual);

                        double startz = Interop.translateFromAVNaN(shapeBuffer.getDouble(offset));
                        offset += 8;
                        zs.write(startpart, startz);
                        if (!VertexDescription.isDefaultValue(Semantics.Z, startz))
                            bCreate = true;
                        ...
                        for (int i = startpart + 1; i < endpart; i++) {
                            ...
                            offset += 8;
                            zs.write(i, z);
                            if (!VertexDescription.isDefaultValue(Semantics.Z, z))
                                bCreate = true;
                        }
        ...
        if (bMs) {
            if (pointCount > 0) {
                ...
                offset += 8;
                double mmax = Interop.translateFromAVNaN(shapeBuffer.getDouble(offset));
                offset += 8;
                ...
                if (m_type == Geometry.GeometryType.Polygon || m_type == Geometry.GeometryType.Polyline
                        || m_type == Geometry.GeometryType.Unknown) {
                    ms = (AttributeStreamOfDbl) AttributeStreamBase.createAttributeStreamWithSemantics(Semantics.M,
                            pointCount);
                    ...
                    for (int ipart = 0; ipart < partCount; ipart++) {
                        ...
                        if (!VertexDescription.isDefaultValue(Semantics.M, startm))
                            bCreate = true;
                        ...
                        for (int i = startpart + 1; i < endpart; i++) {
                            ...
                            offset += 8;
                            ms.write(i, m);
                            if (!VertexDescription.isDefaultValue(Semantics.M, m))
                                bCreate = true;
                        }
        ...
        if (bIDs) {
            if (pointCount > 0) {
                ...
                if (m_type == Geometry.GeometryType.Polygon || m_type == Geometry.GeometryType.Polyline
                        || m_type == Geometry.GeometryType.Unknown) {
                    ids = (AttributeStreamOfInt32) AttributeStreamBase.createAttributeStreamWithSemantics(Semantics.ID,
                            pointCount);
                    ...
                    for (int ipart = 0; ipart < partCount; ipart++) {
                        ...
                        if (!VertexDescription.isDefaultValue(Semantics.ID, startid))
                            bCreate = true;
                        ...
                        for (int i = startpart + 1; i < endpart; i++) {
                            ...
                            offset += 4;
                            ids.write(i, id);
                            if (!bCreate && !VertexDescription.isDefaultValue(Semantics.ID, id))
                                bCreate = true;
    ...
    private Geometry importFromESRIShapeMultiPoint(int modifiers, ByteBuffer shapeBuffer) {
        ...
        if (m_type == Geometry.GeometryType.MultiPoint || m_type == Geometry.GeometryType.Unknown) {
            ...
            if (cPoints > 0) {
                ...
                position = (AttributeStreamOfDbl) AttributeStreamBase
                        .createAttributeStreamWithSemantics(Semantics.POSITION, cPoints);
        ...
        if (bZs) {
            if (cPoints > 0) {
                ...
                if (m_type == Geometry.GeometryType.MultiPoint || m_type == Geometry.GeometryType.Unknown) {
                    ...
                    boolean bCreate = false;
                    for (int i = 0; i < cPoints; i++) {
                        double value = Interop.translateFromAVNaN(shapeBuffer.getDouble(offset));
                        offset += 8;
                        zs.write(i, value);
                        if (!VertexDescription.isDefaultValue(Semantics.Z, value))
                            bCreate = true;
                    }
            ...
            if (cPoints > 0) {
                ...
                if (m_type == Geometry.GeometryType.MultiPoint || m_type == Geometry.GeometryType.Unknown) {
                    ...
                    for (int i = 0; i < cPoints; i++) {
                        ...
                        offset += 8;
                        ms.write(i, value);
                        if (!VertexDescription.isDefaultValue(Semantics.M, value))
                            bCreate = true;
                    }
            ...
            if (cPoints > 0) {
                ...
                if (m_type == Geometry.GeometryType.MultiPoint || m_type == Geometry.GeometryType.Unknown) {
                    ...
                    for (int i = 0; i < cPoints; i++) {
                        ...
                        offset += 4;
                        ids.write(i, value);
                        if (!VertexDescription.isDefaultValue(Semantics.ID, value))
                            bCreate = true;
        ...
        if (m_type == Geometry.GeometryType.MultiPoint) {
            ...
            if (!bEmpty) {
                AttributeStreamBase newPositionStream = AttributeStreamBase
                        .createAttributeStreamWithSemantics(Semantics.POSITION, 1);
                AttributeStreamOfDbl position = (AttributeStreamOfDbl) newPositionStream;
                position.write(0, x);
                position.write(1, y);

                multipointImpl.setAttributeStreamRef(Semantics.POSITION, newPositionStream);
                multipointImpl.resize(1);
            }

            if (bZs) {
                multipointImpl.addAttribute(Semantics.Z);
                if (!bEmpty && !VertexDescription.isDefaultValue(Semantics.Z, z)) {
                    AttributeStreamBase newZStream = AttributeStreamBase.createAttributeStreamWithSemantics(Semantics.Z,
                            1);
                    newZStream.writeAsDbl(0, z);
                    multipointImpl.setAttributeStreamRef(Semantics.Z, newZStream);
                }
            }

            if (bMs) {
                multipointImpl.addAttribute(Semantics.M);
                if (!bEmpty && !VertexDescription.isDefaultValue(Semantics.M, m)) {
                    AttributeStreamBase newMStream = AttributeStreamBase.createAttributeStreamWithSemantics(Semantics.M,
                            1);
                    newMStream.writeAsDbl(0, m);
                    multipointImpl.setAttributeStreamRef(Semantics.M, newMStream);
                }
            }

            if (bIDs) {
                multipointImpl.addAttribute(Semantics.ID);
                if (!bEmpty && !VertexDescription.isDefaultValue(Semantics.ID, id)) {
                    ...
                    multipointImpl.setAttributeStreamRef(Semantics.ID, newIDStream);
                }
            }
