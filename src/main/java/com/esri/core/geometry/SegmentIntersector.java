...
class SegmentIntersector {
    ...
    private IntersectionPart newIntersectionPart_(Segment _seg) {
        if (m_recycled_intersection_parts.isEmpty()) {
            ...
            IntersectionPart part = m_recycled_intersection_parts.get(m_recycled_intersection_parts.size() - 1);
            part.seg = _seg;
            m_recycled_intersection_parts.remove(m_recycled_intersection_parts.size() - 1);
            return part;
        }
    }
    ...
    public void setRankAndWeight(int input_segment_index, double start_weight, int start_rank, double end_weight,
            int end_rank, int interior_rank) {
        ...
        if (b_intersecting || (part1.seg._isIntersecting(part2.seg, tolerance, true) & 5) != 0) {
            if (part1.seg.getType().value() == Geometry.GeometryType.Line) {
                ...
                if (part2.seg.getType().value() == Geometry.GeometryType.Line) {
                    ...
                    int count = Line._intersectLineLine(line_1, line_2, null, m_param_1, m_param_2, tolerance);
                    if (count == 0) {
                        assert (count > 0);
                        throw GeometryException.GeometryInternalError();
                    }
    ...
    public void intersect(double tolerance, Point pt_intersector_point, int point_rank, double point_weight,
