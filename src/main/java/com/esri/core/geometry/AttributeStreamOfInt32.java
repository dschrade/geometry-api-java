...
final class AttributeStreamOfInt32 extends AttributeStreamBase implements Serializable {
    ...
    public void addRange(AttributeStreamBase src, int start, int count, boolean bForward, int stride) {
        ...
        if (bForward) {
            System.arraycopy(((AttributeStreamOfInt32) src).m_buffer, start, m_buffer, oldSize, count);
        } else {
            int n = count;
    ...
    public void insertRange(int start, AttributeStreamBase src, int srcStart, int count, boolean bForward, int stride,
        ...
        if (bForward) {
            System.arraycopy(((AttributeStreamOfInt32) src).m_buffer, srcStart, m_buffer, start, count);
        } else {
            int n = count;
            ...
            for (int i = 0; i < count; i += stride) {
                ...
                for (int s = 0; s < stride; s++) {
                    m_buffer[start + i + s] = ((AttributeStreamOfInt32) src).m_buffer[srcStart + n + s];
                }
            }
        }
    }
    ...
    private void writeObject(java.io.ObjectOutputStream stream) throws IOException {
        ...
        for (int i = 0; i < m_size;) {
            int n = StrictMath.min(32, m_size - i);
            if (bytes == null) {
