...
final class PlaneSweepCrackerHelper {
    ...
    static final class SimplifySweepMonikerComparator extends SweepMonkierComparator {
        PlaneSweepCrackerHelper m_parent;
    ...
    void applyIntersectorToEditShape_(int edgeOrigins, SegmentIntersector intersector, int intersector_index) {
        ...
        for (vertexHandle = m_edge_vertices.getNext(vertexHandle); vertexHandle != -1; vertexHandle = m_edge_vertices
            ...
            assert ((b_forward && getClusterFromVertex(m_shape.getNextVertex(vertex)) == cluster2)
                    || (getClusterFromVertex(vertex) == cluster2
                            && getClusterFromVertex(m_shape.getNextVertex(vertex)) == cluster_1));
        ...
        pt_1 = intersector.getResultSegment(intersector_index, intersector.getResultSegmentCount(intersector_index) - 1)
                .getEndXY();
    ...
    void createEdgesAndClustersFromSplitEdge_(int edge1, SegmentIntersector intersector, int intersector_index) {
        ...
        for (int i = 1, n = intersector.getResultSegmentCount(intersector_index); i < n; i++) {// each
                                                                                               // iteration
                                                                                               // adds
                                                                                               // new
                                                                                               // Cluster
        ...
        for (int vertexHandle = m_edge_vertices
                .getFirst(edgeOrigins1); vertexHandle != -1; vertexHandle = m_edge_vertices.getNext(vertexHandle)) {
            int vertex = m_edge_vertices.getElement(vertexHandle);
    ...
    void processSplitHelper1_(int index, int edge, SegmentIntersector intersector) {
        ...
        if (!m_complications && count > 1) {
            ...
            if (ptClusterStart.compare(midPoint) != dir || midPoint.compare(ptClusterEnd) != dir) {// split segment
                                                                                                   // midpoint is
                                                                                                   // above the
                                                                                                   // sweep line.
                                                                                                   // Therefore the
    ...
    void fixIntersection_(int left, int right) {
        ...
        // DEBUGPRINTF(L"Intersecting %d (%0.4f, %0.4f - %0.4f, %0.4f) and %d (%0.4f,
        // %0.4f - %0.4f, %0.4f)\n",
    ...
    void insertNewEdges_() {
        ...
        while (m_edges_to_insert_in_sweep_structure.size() != 0) {
            if (m_edges_to_insert_in_sweep_structure.size() > StrictMath.max((int) 100, m_shape.getTotalPointCount())) {
                assert (false);
            ...
            assert (getEdgeSweepNode(edge) == StridedIndexTypeCollection.impossibleIndex3());
    ...
    boolean insertNewEdgeToSweepStructure_(int edge, int terminatingCluster) {
        ...
        if (m_b_continuing_segment_chain_optimization) {
            newEdgeNode = m_sweep_structure.addElementAtPosition(m_prev_neighbour, m_next_neighbour, edge, true, true,
                    -1);
            m_b_continuing_segment_chain_optimization = false;
        } else {
    ...
    boolean sweepImpl_() {
        ...
        for (int eventQnode = m_event_q.getFirst(-1); eventQnode != -1;) {
            ...
            if (edgesToDelete.size() > 0) {
                m_b_continuing_segment_chain_optimization = (edgesToDelete.size() == 1
                        && m_edges_to_insert_in_sweep_structure.size() == 1);
    ...
    void setEditShape_(EditShape shape) {
        ...
        for (int geometry = m_shape.getFirstGeometry(); geometry != -1; geometry = m_shape.getNextGeometry(geometry)) {
            boolean bMultiPath = Geometry.isMultiPath(m_shape.getGeometryType(geometry));
