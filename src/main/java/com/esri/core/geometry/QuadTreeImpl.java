...
class QuadTreeImpl implements Serializable {
    ...
    static final class QuadTreeIteratorImpl {
        /**
         * Resets the iterator to an starting state on the Quad_tree_impl. If the input
         * Geometry is a Line segment, then the query will be the segment. Otherwise the
         * query will be the Envelope_2D bounding the Geometry. \param query The
         * Geometry used for the query. \param tolerance The tolerance used for the
         * intersection tests. \param tolerance The tolerance used for the intersection
         * tests.
         */
        ...
        /**
         * Resets the iterator to a starting state on the Quad_tree_impl using the input
         * Envelope_2D as the query. \param query The Envelope_2D used for the query.
        ...
        int next() {
            ...
            while (!b_found_hit) {
                ...
                // If m_current_element_handle equals -1, then we've exhausted our search in the
                // current quadtree node
                if (m_current_element_handle == -1) {
                    // get the last node from the stack and add the children whose extent intersects
                    // m_query_box
                    ...
                    m_current_element_handle = m_quad_tree
                            .get_first_element_(m_quads_stack.get(m_quads_stack.size() - 1));
                }
            }
    ...
    static final class QuadTreeSortedIteratorImpl {
        /**
         * Resets the iterator to a starting state on the Quad_tree_impl. If the input
         * Geometry is a Line segment, then the query will be the segment. Otherwise the
         * query will be the Envelope_2D bounding the Geometry. \param query The
         * Geometry used for the query. \param tolerance The tolerance used for the
         * intersection tests. \param tolerance The tolerance used for the intersection
        ...
        int next() {
            if (m_index == -1) {
                ...
                m_bucket_sort.sort(m_sorted_handles, 0, m_sorted_handles.size(),
                        new Sorter(m_quad_tree_iterator_impl.m_quad_tree));
            }
     ...
     * Envelope_2D, and height of the input height, where the root starts at height
     * 0. \param extent The extent of the Quad_tree_impl. \param height The max
     * height of the Quad_tree_impl.
     */
     ...
     * Envelope_2D, and height of the input height, where the root starts at height
     * 0. \param extent The extent of the Quad_tree_impl. \param height The max
     * height of the Quad_tree_impl. \param b_store_duplicates Put true to place
     * elements deeper into the quad tree at intesecting quads, duplicates will be
     * stored. Put false to only place elements into quads that can contain it.
     */
    QuadTreeImpl(Envelope2D extent, int height, boolean b_store_duplicates) {
        m_quad_tree_nodes = (b_store_duplicates ? new StridedIndexTypeCollection(11)
                : new StridedIndexTypeCollection(10));
        m_element_nodes = new StridedIndexTypeCollection(4);
        m_data = new ArrayList<Data>(0);
        m_free_data = new AttributeStreamOfInt32(0);
    ...
    /**
     * Resets the Quad_tree_impl to the given extent and height. \param extent The
     * extent of the Quad_tree_impl. \param height The max height of the
     * Quad_tree_impl.
     */
    ...
    /**
     * Inserts the element and bounding_box into the Quad_tree_impl. Note that this
     * will invalidate any active iterator on the Quad_tree_impl. Returns an
     * Element_handle corresponding to the element and bounding_box. \param element
     * The element of the Geometry to be inserted. \param bounding_box The
     ...
     * Quad_tree_impl. Returns an Element_handle corresponding to the element and
     * bounding_box. \param element The element of the Geometry to be inserted.
     * \param bounding_box The bounding_box of the Geometry to be inserted. \param
     * hint_index A handle used as a hint where to place the element. This can be a
     * handle obtained from a previous insertion and is useful on data having strong
     * locality such as segments of a Polygon.
     */
    ...
    /**
     * Removes the element and bounding_box at the given element_handle. Note that
     * this will invalidate any active iterator on the Quad_tree_impl. \param
     * element_handle The handle corresponding to the element and bounding_box to be
     * removed.
     */
    ...
    /**
     * Returns the element at the given element_handle. \param element_handle The
     * handle corresponding to the element to be retrieved.
     */
    ...
    /**
     * Returns the ith unique element. \param i The index corresponding to the ith
     * unique element.
     */
    ...
    /**
     * Returns the element extent at the given element_handle. \param element_handle
     * The handle corresponding to the element extent to be retrieved.
     */
    ...
    /**
     * Returns the extent of the ith unique element. \param i The index
     * corresponding to the ith unique element.
     */
    ...
    /**
     * Returns the height of the quad at the given quad_handle. \param quad_handle
    ...
    /**
     * Returns the extent of the quad at the given quad_handle. \param quad_handle
     * The handle corresponding to the quad.
     */
    ...
    /**
     * Returns the number of elements contained in the subtree rooted at the given
     * quad_handle. \param quad_handle The handle corresponding to the quad.
     */
    ...
    /**
     * Returns the number of elements in the quad tree that intersect the qiven
     * query. Some elements may be duplicated if the quad tree stores duplicates.
     * \param query The Envelope_2D used for the query. \param tolerance The
     * tolerance used for the intersection tests. \param max_count If the
     * intersection count becomes greater than or equal to the max_count, then
        ...
        while (quads_stack.size() > 0) {
            ...
            if (query_inflated.contains(current_extent)) {
                ...
                if (query_inflated.isIntersecting(current_extent)) {
                    for (int element_handle = get_first_element_(
                            current_quad_handle); element_handle != -1; element_handle = get_next_element_(
                                    element_handle)) {
                        int data_handle = get_data_(element_handle);
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (child_handle != -1 && getSubTreeElementCount(child_handle) > 0) {
                        ...
                        if (b_is_intersecting) {
                            ...
                            extents_stack.add(new Envelope2D(child_extents[i].xmin, child_extents[i].ymin,
                                    child_extents[i].xmax, child_extents[i].ymax));
                        }
                    }
                }
            }
    ...
    /**
     * Returns true if the quad tree has data intersecting the given query. \param
     * query The Envelope_2D used for the query. \param tolerance The tolerance used
     ...
     * that bounds the input Geometry. To reuse the existing iterator on the same
     * Quad_tree_impl but with a new query, use the reset_iterator function on the
     * Quad_tree_iterator_impl. \param query The Geometry used for the query. If the
     * Geometry is a Line segment, then the query will be the segment. Otherwise the
     * query will be the Envelope_2D bounding the Geometry. \param tolerance The
     ...
     * query. To reuse the existing iterator on the same Quad_tree_impl but with a
     * new query, use the reset_iterator function on the Quad_tree_iterator_impl.
     * \param query The Envelope_2D used for the query. \param tolerance The
     * tolerance used for the intersection tests.
     */
    ...
    /**
     * Gets a sorted iterator on the Quad_tree_impl. The Element_handles will be
     * returned in increasing order of their corresponding Element_types. The query
     * will be the Envelope_2D that bounds the input Geometry. To reuse the existing
     * iterator on the same Quad_tree_impl but with a new query, use the
     * reset_iterator function on the Quad_tree_sorted_iterator_impl. \param query
     * The Geometry used for the query. If the Geometry is a Line segment, then the
     * query will be the segment. Otherwise the query will be the Envelope_2D
     * bounding the Geometry. \param tolerance The tolerance used for the
     * intersection tests.
     */
    ...
    /**
     * Gets a sorted iterator on the Quad_tree_impl using the input Envelope_2D as
     * the query. The Element_handles will be returned in increasing order of their
     * corresponding Element_types. To reuse the existing iterator on the same
     * Quad_tree_impl but with a new query, use the reset_iterator function on the
    ...
    /**
     * Gets a sorted iterator on the Quad_tree. The Element_handles will be returned
     * in increasing order of their corresponding Element_types
     */
    ...
    public long estimateMemorySize() {
        long size = SIZE_OF_QUAD_TREE_IMPL + (m_extent != null ? m_extent.estimateMemorySize() : 0)
    ...
    private int insert_(int element, Envelope2D bounding_box, int height, Envelope2D quad_extent, int quad_handle,
            boolean b_flushing, int flushed_element_handle) {
        ...
        for (current_height = height; current_height < m_height
                && can_push_down_(current_quad_handle); current_height++) {
            set_child_extents_(current_extent, child_extents);
        ...
        return insert_at_quad_(element, bounding_box, current_height, current_extent, current_quad_handle, b_flushing,
                quad_handle, flushed_element_handle, -1);
    }

    private int insert_duplicates_(int element, Envelope2D bounding_box, int height, Envelope2D quad_extent,
            int quad_handle, boolean b_flushing, int flushed_element_handle) {
        assert (b_flushing || m_root == quad_handle);

        if (!b_flushing) // If b_flushing is true, then the sub tree element counts are already accounted
                         // for since the element already lies in the current incoming quad
        {
            if (!quad_extent.contains(bounding_box))
                return -1;
        ...
        double bounding_box_max_dim = StrictMath.max(bounding_box.getWidth(), bounding_box.getHeight());
        ...
        while (quads_stack.size() > 0) {
            ...
            if (current_height + 1 < m_height && can_push_down_(current_quad_handle)) {
                double current_extent_max_dim = StrictMath.max(current_extent.getWidth(), current_extent.getHeight());
            ...
            if (b_subdivide) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (b_contains) {
                        ...
                        set_sub_tree_element_count_(child_handle, get_sub_tree_element_count_(child_handle) + 1);
                        set_contained_sub_tree_element_count_(child_handle,
                                get_contained_sub_tree_element_count_(child_handle) + 1);
                        break;
                    }
                }
                ...
                element_handle = insert_at_quad_(element, bounding_box, current_height, current_extent,
                        current_quad_handle, b_flushing, quad_handle, flushed_element_handle, element_handle);
                b_flushing = false; // flushing is false after the first inserted element has been flushed down, all
                                    // subsequent inserts will be new
            }
        }
    ...
    private int insert_at_quad_(int element, Envelope2D bounding_box, int current_height, Envelope2D current_extent,
            int current_quad_handle, boolean b_flushing, int quad_handle, int flushed_element_handle,
            int duplicate_element_handle) {
        // If the bounding box is not contained in any of the current_node's children,
        // or if the current_height is m_height, then insert the element and
        ...
        if (b_flushing) {
            ...
            disconnect_element_handle_(flushed_element_handle); // Take it out of the incoming quad_handle, and place in
                                                                // current_quad_handle
    ...
    private boolean has_children_(int parent) {
        return get_child_(parent, 0) != -1 || get_child_(parent, 1) != -1 || get_child_(parent, 2) != -1
                || get_child_(parent, 3) != -1;
    }
    ...
    /*
     * m_quad_tree_nodes 0: m_north_east_child 1: m_north_west_child 2:
     * m_south_west_child 3: m_south_east_child 4: m_head_element 5: m_tail_element
     * 6: m_quadrant_and_height 7: m_local_element_count 8: m_sub_tree_element_count
     * 9: m_parent_quad 10: m_height
     */

    /*
     * m_element_nodes 0: m_data_handle 1: m_prev 2: m_next 3: m_parent_quad
     */
