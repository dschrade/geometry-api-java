...
class OperatorCutCursor extends GeometryCursor {
    ...
    OperatorCutCursor(boolean bConsiderTouch, Geometry cuttee, Polyline cutter, SpatialReference spatialReference,
    ...
    private void generate_polyline_cuts_() {
        ...
        Cutter.CutPolyline(m_bConsiderTouch, (Polyline) m_cuttee, m_cutter, m_tolerance, cutPairs, null,
                m_progressTracker);
        ...
        for (int icut = 0; icut < cutPairs.size(); icut++) {
            ...
            } else if (cutPair.m_side == Side.Right || cutPair.m_side == Side.Coincident) {
        ...
        if (!uncut.isEmpty() && (!left.isEmpty() || !right.isEmpty() || m_cuts.size() >= 3))
            m_cuts.add(uncut);
