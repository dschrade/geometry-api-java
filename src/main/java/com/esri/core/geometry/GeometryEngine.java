...
/**
 * Provides services that operate on geometry instances. The methods of
 * GeometryEngine class call corresponding OperatorXXX classes. Consider using
 * OperatorXXX classes directly as they often provide more functionality and
 * better performance. For example, some Operators accept GeometryCursor class
 * that could be implemented to wrap a feature cursor and make it feed
 * geometries directly into an Operator. Also, some operators provide a way to
 * accelerate an operation by using Operator.accelerateGeometry method.
 */
public class GeometryEngine {
    ...
    /**
     * Imports the MapGeometry from its JSON representation. M and Z values are not
     * imported from JSON representation.
     * 
    ...
    /**
     * Exports the specified geometry instance to it's JSON representation. M and Z
     * values are not imported from JSON representation.
     * 
     * See OperatorExportToJson.
     * 
     ...
     *            Geometry.Type.Unknown if the geometry type needs to be determined
     *            from the buffer content.
     * @return The geometry or null if the buffer contains null shape.
     * @throws GeometryException
     *             when the geometryType is not Geometry.Type.Unknown and the buffer
     *             contains geometry that cannot be converted to the given
     *             geometryType. or the buffer is corrupt. Another exception
     *             possible is IllegalArgumentsException.
     */
     ...
     *            The required type of the Geometry to be imported. Use
     *            Geometry.Type.Unknown if the geometry type needs to be determined
     *            from the WKT context.
     * @return The geometry.
     * @throws GeometryException
     *             when the geometryType is not Geometry.Type.Unknown and the WKT
     *             contains a geometry that cannot be converted to the given
     *             geometryType.
    ...
    /**
     * Constructs a new geometry by union an array of geometries. All inputs must be
     * of the same type of geometries and share one spatial reference.
     * 
     * See OperatorUnion.
     * 
     * @param geometries
    ...
    /**
     * Creates the difference of two geometries. The dimension of geometry2 has to
     ...
     */
    public static Geometry symmetricDifference(Geometry leftGeometry, Geometry rightGeometry,
     ...
     * 
     * @param geometry1
     *            The base geometry that is tested for within relationship to the
     *            other geometry.
     * @param geometry2
     ...
     * 
     * @param geometry1
     *            The geometry that is tested for the contains relationship to the
     *            other geometry..
     * @param geometry2
     *            The geometry that is tested for within relationship to the other
     *            geometry.
     * @param spatialReference
     *            The spatial reference of the geometries.
     ...
     */
    public static boolean relate(Geometry geometry1, Geometry geometry2, SpatialReference spatialReference,
            String relation) {
    ...
    public static Geometry clip(Geometry geometry, Envelope envelope, SpatialReference spatialReference) {
        ...
        Geometry result = op.execute(geometry,
                Envelope2D.construct(envelope.getXMin(), envelope.getYMin(), envelope.getXMax(), envelope.getYMax()),
     ...
     * Right cuts and coincident cuts are grouped in the second Geometry, and each
     * undefined cut, along with any uncut parts, are output as separate Polylines.
     * For Polygons, all left cuts are grouped in the first Polygon, all right cuts
     * are in the second Polygon, and each undefined cut, along with any left-over
     * parts after cutting, are output as a separate Polygon. If there were no cuts
     * then the array will be empty. An undefined cut will only be produced if a
     * left cut or right cut was produced, and there was a part left over after
     * cutting or a cut is bounded to the left and right of the cutter.
     * 
     * See OperatorCut.
     * 
     * @param cuttee
    ...
    /**
     * Calculates a buffer polygon for each geometry at each of the corresponding
     * specified distances. It is assumed that all geometries have the same spatial
     * reference. There is an option to union the returned geometries.
     * 
     * See OperatorBuffer.
     * 
     * @param geometries
     ...
     */
    public static Polygon[] buffer(Geometry[] geometries, SpatialReference spatialReference, double[] distances,
        ...
        if (toUnionResults) {
            ...
            GeometryCursor result = op.execute(inputGeometriesCursor, spatialReference, bufferDistances, toUnionResults,
                    null);
    ...
    /**
     * Calculates a buffer polygon of the geometry as specified by the distance
     * input. The buffer is implemented in the xy-plane.
     * 
     * See OperatorBuffer
     * 
     * @param geometry
     ...
     * @param distance
     *            The specified distance for buffer. Same units as the spatial
     *            reference.
     ...
     * 
     *         For a Point - returns the same point. For an Envelope - returns the
     *         same envelope. For a MultiPoint - If the point count is one, returns
     *         the same multipoint. If the point count is two, returns a polyline of
     *         the points. Otherwise computes and returns the convex hull polygon.
     *         For a Segment - returns a polyline consisting of the segment. For a
     *         Polyline - If consists of only one segment, returns the same
     *         polyline. Otherwise computes and returns the convex hull polygon. For
     *         a Polygon - If more than one path, or if the path isn't already
     *         convex, computes and returns the convex hull polygon. Otherwise
     *         returns the same polygon.
     */
     ...
     * @param b_merge
     *            Put true if you want the convex hull of all the geometries in the
     *            array combined. Put false if you want the convex hull of each
     *            geometry in the array individually.
     * @return Returns an array of convex hulls. If b_merge is true, the result will
     *         be a one element array consisting of the merged convex hull.
     */
     ...
     */
    public static Proximity2DResult getNearestCoordinate(Geometry geometry, Point inputPoint,
     ...
     */
    public static Proximity2DResult[] getNearestVertices(Geometry geometry, Point inputPoint, double searchRadius,
            int maxVertexCountToReturn) {
        OperatorProximity2D proximity = (OperatorProximity2D) factory
                .getOperator(com.esri.core.geometry.Operator.Type.Proximity2D);

        Proximity2DResult[] results = proximity.getNearestVertices(geometry, inputPoint, searchRadius,
    ...
    /**
     * A geodesic distance is the shortest distance between any two points on the
     * earth's surface when the earth's surface is approximated by a spheroid. The
     * function returns the shortest distance between two points on the WGS84
     * spheroid.
     * 
     * @param ptFrom
