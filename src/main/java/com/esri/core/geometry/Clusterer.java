...
final class Clusterer {
     ...
     * tolerance; clusterer.m_cell_size = 2 * tolerance; clusterer.m_inv_cell_size =
     * 1.0 / clusterer.m_cell_size; return clusterer.clusterReciprocal_(); }
     */
    ...
    final class ClusterHashFunction extends IndexHashTable.HashFunction {
        ...
        public ClusterHashFunction(EditShape shape, Point2D origin, double sqr_tolerance, double inv_cell_size,
                int hash_values) {
            m_shape = shape;
        ...
        public boolean equal(int element_1, int element_2) {
            ...
            return isClusterCandidate_(m_pt.x, m_pt.y, m_pt_2.x, m_pt_2.y, m_sqr_tolerance);
        }
    ...
    void getNearestNeighbourCandidate_(int xyindex, Point2D pointOfInterest, int bucket_ptr,
        ...
        for (int dx = 0; dx <= 1; dx += 1) {
            for (int dy = 0; dy <= 1; dy += 1) {
                ...
                if (bucket_ptr != IndexHashTable.nullNode()) {
                    ...
                    if (c.vertex != IndexHashTable.nullNode() && c.distance < candidate.distance) {
                        candidate = c;
                    }
                }
            }
    ...
    void collectClusterCandidates_(int xyindex, AttributeStreamOfInt32 candidates) {
        ...
        for (int i = 0; i < bucket_count; i++) {
            collectNearestNeighbourCandidates_(xyindex, m_bucket_hash[i], pointOfInterest, m_bucket_array[i],
                    candidates);
        }
    }

    void collectNearestNeighbourCandidates_(int xyindex, int hash, Point2D pointOfInterest, int bucket_ptr,
    ...
    boolean mergeClusters_(int vertex1, int vertex2, boolean update_hash) {
        ...
        m_shape.setUserIndex(vertex2, m_new_clusters, StridedIndexTypeCollection.impossibleIndex2());
    ...
    static boolean mergeVertices(Point pt_1, Point pt_2, double w_1, int rank_1, double w_2, int rank_2, Point pt_res,
            double[] w_res, int[] rank_res) {
    ...
    static boolean mergeVertices2D(Point2D pt_1, Point2D pt_2, double w_1, int rank_1, double w_2, int rank_2,
            Point2D pt_res, double[] w_res, int[] rank_res) {
        double w = w_1 + w_2;
        boolean r = false;
        double x = pt_1.x;
        if (pt_1.x != pt_2.x) {
    ...
    boolean clusterNonReciprocal_() {
        ...
        double dim = StrictMath.max(env.getHeight(), env.getWidth());
        ...
        m_hash_function = new ClusterHashFunction(m_shape, m_origin, m_sqr_tolerance, m_inv_cell_size, m_hash_values);
                    ...
                    for (int index = 0, nindex = m_shape.getPathSize(path); index < nindex; index++) {
                        ...
                        while (true) {
                            ...
                            for (int candidate_index = 0,
                                    ncandidates = candidates.size(); candidate_index < ncandidates; candidate_index++) {
