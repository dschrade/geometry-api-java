...
class ECoordinate {
    ...
    void add(ECoordinate v) // +=
    {
        ...
        double e = m_eps + v.m_eps + epsCoordinate() * StrictMath.abs(r);
        m_value = r;
        m_eps = e;
    }
    ...
    {
        double r = m_value + v;
        double e = m_eps + epsCoordinate() * StrictMath.abs(r);
        m_value = r;
        m_eps = e;
    }
    ...
    void add(ECoordinate v_1, ECoordinate v_2) // +
    {
        ...
        m_eps = v_1.m_eps + v_2.m_eps + epsCoordinate() * StrictMath.abs(m_value);
    }

    void add(double v_1, double v_2) // +
    {
        m_value = v_1 + v_2;
        m_eps = epsCoordinate() * StrictMath.abs(m_value);
    }

    void add(ECoordinate v_1, double v_2) // +
    {
        m_value = v_1.m_value + v_2;
        m_eps = v_1.m_eps + epsCoordinate() * StrictMath.abs(m_value);
    }

    void add(double v_1, ECoordinate v_2) // +
    {
        m_value = v_1 + v_2.m_value;
        m_eps = v_2.m_eps + epsCoordinate() * StrictMath.abs(m_value);
    }
    ...
    void mul(ECoordinate v) {
        double r = m_value * v.m_value;
        m_eps = m_eps * StrictMath.abs(v.m_value) + v.m_eps * StrictMath.abs(m_value) + m_eps * v.m_eps
                + epsCoordinate() * StrictMath.abs(r);
        m_value = r;
    }

    void mul(double v) {
        double r = m_value * v;
        m_eps = m_eps * StrictMath.abs(v) + epsCoordinate() * StrictMath.abs(r);
        m_value = r;
    }

    void mul(ECoordinate v_1, ECoordinate v_2) {
        double r = v_1.m_value * v_2.m_value;
        m_eps = v_1.m_eps * StrictMath.abs(v_2.m_value) + v_2.m_eps * StrictMath.abs(v_1.m_value)
                + v_1.m_eps * v_2.m_eps + epsCoordinate() * StrictMath.abs(r);
        m_value = r;
    }
    ...
    void div(ECoordinate divis) {
        double fabsdivis = StrictMath.abs(divis.m_value);
        double r = m_value / divis.m_value;
        double e = (m_eps + StrictMath.abs(r) * divis.m_eps) / fabsdivis;
        ...
        m_value = r;
        m_eps = e + epsCoordinate() * StrictMath.abs(r);
    }

    void div(double v) {
        double fabsdivis = StrictMath.abs(v);
        m_value /= v;
        m_eps = m_eps / fabsdivis + epsCoordinate() * StrictMath.abs(m_value);
    }
    ...
    void sqrt() {
        ...
        if (m_value >= 0) { // assume non-negative input
            r = StrictMath.sqrt(m_value);
            if (m_value > 10.0 * m_eps) {
                dr = 0.5 * m_eps / r;
            } else {
                dr = (m_value > m_eps) ? r - StrictMath.sqrt(m_value - m_eps)
                        : Math.max(r, StrictMath.sqrt(m_value + m_eps) - r);
            }

            dr += epsCoordinate() * StrictMath.abs(r);
        } else {
    ...
    void sin(ECoordinate angle) {
        double sinv = StrictMath.sin(angle.m_value);
        double cosv = StrictMath.cos(angle.m_value);
        m_value = sinv;
        double absv = StrictMath.abs(sinv);
        m_eps = (StrictMath.abs(cosv) + absv * 0.5 * angle.m_eps) * angle.m_eps + epsCoordinate() * absv;
    }
    ...
    void cos(ECoordinate angle) {
        ...
        double cosv = StrictMath.cos(angle.m_value);
        m_value = cosv;
        double absv = StrictMath.abs(cosv);
        m_eps = (StrictMath.abs(sinv) + absv * 0.5 * angle.m_eps) * angle.m_eps + epsCoordinate() * absv;
    }

    // Calculates natural log of v and assigns to this coordinate
    void log(ECoordinate v) {
        double d = v.m_eps / v.m_value;
        m_value = StrictMath.log(v.m_value);
        m_eps = d * (1.0 + 0.5 * d) + epsCoordinate() * StrictMath.abs(m_value);
    }
    ...
    {
        return StrictMath.abs(m_value - v.m_value) <= m_eps + v.m_eps;
    }
    ...
    {
        return StrictMath.abs(m_value - v.m_value) <= tolerance || eq(v);
    }
    ...
    boolean isZero() {
        return StrictMath.abs(m_value) <= m_eps;
    }
    ...
    boolean tolIsZero(double tolerance) {
        return StrictMath.abs(m_value) <= StrictMath.max(m_eps, tolerance);
    }

    void setPi() {
        set(StrictMath.PI, epsCoordinate());
    }
