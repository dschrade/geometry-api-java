...
class OperatorProximity2DLocal extends OperatorProximity2D {
    ...
    class Side_helper {
        ...
        void find_analysis_pair_from_index(Point2D inputPoint, SegmentIterator segIter, int vertexIndex,
                int pathIndex) {
        ...
        boolean calc_side(Point2D inputPoint, boolean bRight, MultiPath multipath, int vertexIndex, int pathIndex) {
            ...
            find_analysis_pair_from_index(inputPoint, segIter, vertexIndex, pathIndex);
    ...
    public Proximity2DResult getNearestCoordinate(Geometry geom, Point inputPoint, boolean bTestPolygonInterior,
        ...
        case Geometry.GeometryType.MultiPoint:
            return multiVertexGetNearestVertex((MultiVertexGeometry) proxmityTestGeom, inputPoint2D);
        case Geometry.GeometryType.Polyline:
        case Geometry.GeometryType.Polygon:
            return multiPathGetNearestCoordinate((MultiPath) proxmityTestGeom, inputPoint2D, bTestPolygonInterior,
                    bCalculateLeftRightSide);
        default: {
    ...
    @Override
    public Proximity2DResult[] getNearestVertices(Geometry geom, Point inputPoint, double searchRadius,
        ...
        switch (gt) {
        case Geometry.GeometryType.Point:
            return pointGetNearestVertices((Point) proxmityTestGeom, inputPoint2D, searchRadius,
                    maxVertexCountToReturn);
        case Geometry.GeometryType.MultiPoint:
        case Geometry.GeometryType.Polyline:
        case Geometry.GeometryType.Polygon:
            return multiVertexGetNearestVertices((MultiVertexGeometry) proxmityTestGeom, inputPoint2D, searchRadius,
                    maxVertexCountToReturn);
        default: {
            throw new GeometryException("not implemented");
        }
    ...
    Proximity2DResult multiPathGetNearestCoordinate(MultiPath geom, Point2D inputPoint, boolean bTestPolygonInterior,
            boolean bCalculateLeftRightSide) {
        if (geom.getType() == Geometry.Type.Polygon && bTestPolygonInterior) {
            Envelope2D env = new Envelope2D();
            geom.queryEnvelope2D(env);
            double tolerance = InternalUtils.calculateToleranceFromGeometry(null, env, false);

            PolygonUtils.PiPResult pipResult;

            if (bCalculateLeftRightSide)
                pipResult = PolygonUtils.isPointInPolygon2D((Polygon) geom, inputPoint, 0.0);
            else
                pipResult = PolygonUtils.isPointInPolygon2D((Polygon) geom, inputPoint, tolerance);

            if (pipResult != PolygonUtils.PiPResult.PiPOutside) {
                Proximity2DResult result = new Proximity2DResult(inputPoint, 0, 0.0);
        ...
        Proximity2DResult result = new Proximity2DResult(closest, closestVertexIndex,
                StrictMath.sqrt(closestDistanceSq));

        if (bCalculateLeftRightSide) {
            segIter.resetToVertex(closestVertexIndex, closestPathIndex);
            Segment segment = segIter.nextSegment();
            bRight = (Point2D.orientationRobust(inputPoint, segment.getStartXY(), segment.getEndXY()) < 0);
    ...
    Proximity2DResult multiVertexGetNearestVertex(MultiVertexGeometry geom, Point2D inputPoint) {
        ...
        AttributeStreamOfDbl position = (AttributeStreamOfDbl) mpImpl.getAttributeStreamRef((Semantics.POSITION));
        ...
        result._setParams(closestx, closesty, closestIndex, StrictMath.sqrt(closestDistanceSq));

        return result;
    }

    Proximity2DResult[] pointGetNearestVertices(Point geom, Point2D inputPoint, double searchRadius,
            int maxVertexCountToReturn) {
        ...
        if (distanceSq <= searchRadiusSq) {
            ...
            Proximity2DResult result = new Proximity2DResult();
            result._setParams(pt.x, pt.y, 0, StrictMath.sqrt(distanceSq));
    ...
    Proximity2DResult[] multiVertexGetNearestVertices(MultiVertexGeometry geom, Point2D inputPoint, double searchRadius,
            int maxVertexCountToReturn) {
        Proximity2DResult[] resultArray;
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            if (distanceSq <= searchRadiusSq) {
                Proximity2DResult result = new Proximity2DResult();
                result._setParams(x, y, i, StrictMath.sqrt(distanceSq));
    ...
    /*
     * if (distanceSq <= searchRadiusSq) { if (count >= maxVertexCountToReturn + 1)
     * { count++; double frontDistance = v.get(0).getDistance(); if (frontDistance *
     * frontDistance <= distanceSq) continue; }
     * 
     * Proximity2DResult result = new Proximity2DResult(); result._setParams(x, y,
     * i, StrictMath.sqrt(distanceSq));
     * 
     * count++;
     * 
     * if (count <= maxVertexCountToReturn) { v.add(result); } // else // { // if
     * (count == maxVertexCountToReturn + 1) // MAKEHEAP(v, Proximity2DResult,
     * Proximity2DResult::_Compare); // // PUSHHEAP(v, result, Proximity2DResult,
     * Proximity2DResult::_Compare); // POPHEAP(v, Proximity2DResult,
     * Proximity2DResult::_Compare); // } } }
     * 
     * int vsize = v.size(); Collections.sort(v, new Proximity2DResultComparator());
     * 
     * // SORTDYNAMICARRAY(v, Proximity2DResult, 0, vsize,
     * Proximity2DResult::_Compare); resultArray = new Proximity2DResult[vsize]; for
     * (int i = 0; i < vsize; i++) { resultArray[i] = (Proximity2DResult)v.get(i); }
     * 
     * return resultArray; }
     */
}
