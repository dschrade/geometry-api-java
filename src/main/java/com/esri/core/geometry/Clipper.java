...
class Clipper {
    ...
    MultiPath clipMultiPath2_(MultiPath multi_path_in, double tolerance, double densify_dist) {
        ...
        if (b_is_polygon)
            return clipPolygon2_((Polygon) multi_path_in, tolerance, densify_dist);
        else
    ...
    MultiPath clipPolygon2_(Polygon polygon_in, double tolerance, double densify_dist) {
        ...
        delete_candidates.reserve(StrictMath.min(100, polygon_in.getPointCount()));
                ...
                do {
                    ...
                    if (seg_plane_intersection_status == -1) // intersects plane
                    {
                        ...
                        for (int i = 0; i < split_count; i++) {
                            ...
                            int sub_segment_plane_intersection_status = checkSegmentIntersection_(sub_seg_env,
                                    iclip_plane, clip_value);
                            if (sub_segment_plane_intersection_status == -1) {
                                ...
                                if (!b_axis_x) {
                                    assert ((pt_1.x < clip_value && pt_2.x > clip_value)
                                            || (pt_1.x > clip_value && pt_2.x < clip_value));
                                    double d_1 = StrictMath.abs(pt_1.x - clip_value);
                                    double d_2 = StrictMath.abs(pt_2.x - clip_value);
                                    if (d_1 < d_2) {
                                        pt_1.x = clip_value;
                                    ...
                                    assert ((pt_1.y < clip_value && pt_2.y > clip_value)
                                            || (pt_1.y > clip_value && pt_2.y < clip_value));
                                    double d_1 = StrictMath.abs(pt_1.y - clip_value);
                                    double d_2 = StrictMath.abs(pt_2.y - clip_value);
                                    if (d_1 < d_2) {
                                        pt_1.y = clip_value;
                                        m_shape.setXY(split_vert, pt_1);
                                    } else {
                                ...
                                sub_seg.queryEnvelope2D(sub_seg_env);
                                sub_segment_plane_intersection_status = checkSegmentIntersection_(sub_seg_env,
                                        iclip_plane, clip_value);
                            }
    ...
    void resolveBoundaryOverlaps_() {
        ...
        for (int path = m_shape.getFirstPath(m_geometry); path != -1; path = m_shape.getNextPath(path)) {
            ...
            for (int ivert = 0,
                    nvert = m_shape.getPathSize(path); ivert < nvert; ivert++, vertex = m_shape.getNextVertex(vertex)) {
    ...
    void densifyAlongClipExtent_(double densify_dist) {
        ...
        for (int path = m_shape.getFirstPath(m_geometry); path != -1; path = m_shape.getNextPath(path)) {
            ...
            do {
                ...
                int num = (int) StrictMath.min(StrictMath.ceil(len / densify_dist), 2048.0);
                if (num <= 1) {
                    vertex = next_vertex;
                    continue;
                }
                ...
                for (int ia = 0, na = active_intervals.size(); ia < na; ia++) {
                    ...
                    if (n_1 == 1) {
                        ...
                        if (!pt_1.isEqual(pt)) {// Split the active segment
                            double active_segment_length = Point2D.distance(pt_0, pt_1);
    ...
    void resolveOverlaps_(boolean b_axis_x, double clip_value) {
        ...
        for (int ivert = 0, nvert = m_vertices_on_extent.size(); ivert < nvert; ivert++) {
            ...
            if (b_axis_x ? pt.y == clip_value : pt.x == clip_value) {
                m_shape.getXY(next_vertex, pt);
                if (b_axis_x ? pt.y == clip_value : pt.x == clip_value) {
                    assert (m_shape.getUserIndex(next_vertex, m_vertices_on_extent_index) != -1);
    ...
    void removeOverlap_(AttributeStreamOfInt32 sorted_vertices, int v, int nextv, int v_1, int prevv_1,
            int sorted_index) {
    ...
    static Geometry clip(Geometry geometry, Envelope2D extent, double tolerance, double densify_dist) {
        ...
        case Geometry.GeometryType.MultiPoint: {
            ...
            AttributeStreamOfDbl xy = (AttributeStreamOfDbl) ((MultiPointImpl) multi_point._getImpl())
        ...
        case Geometry.GeometryType.Polyline:
            return clipMultiPath_((MultiPath) geometry, extent, tolerance, densify_dist);
        default:
            assert (false);
            throw GeometryException.GeometryInternalError();
        }
    ...
    static final class ClipperVertexComparer extends AttributeStreamOfInt32.IntComparator {
        Clipper m_clipper;
